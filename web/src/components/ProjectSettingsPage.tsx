...
export interface ProjectSettingsPageProps extends UseProjectSettingsOptions {
  ...
  title?: string
  /**
   * Project id — only used as the Advanced-tier sticky-state key (G5).
   * Optional: a host that never passes one gets a single shared key rather
   * than a per-project one, same fallback `readRevealed`'s caller would hit
   * if it ever forgot to pass a project id.
   */
  projectId?: string
}

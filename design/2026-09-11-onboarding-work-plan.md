...
- **Depends on:** —
- [ ] done
- Notes: (2026-09-11) Commit 32b28cc merged to main (rebased on cd44582). `projectSettingsHolder` (atomic.Pointer; SIGHUP + `AGENTKIT_PROJECT_MAP_RELOAD`, default 60s) and `projectKeysHolder`; login handlers take a `userDirectory`; git-token-env closure reads the holder per call; embed-CSP origin list still a boot snapshot (DI 6); docs/ops.md switched to a mounted `projects.json` (DI 7). Gates incl. `-race` green on the branch. (2026-09-12) **Deliberately still unticked, and the only one.** Its code is merged and its unit tests pass, but the ticket's own status asks for a *live* check and D2 could not give one: the e2e stack carries the inline `AGENTKIT_PROJECT_MAP`, not `AGENTKIT_PROJECT_MAP_FILE` (confirmed by reading the env inside the running `agentd`), so the SIGHUP/timer reload path is never exercised there. Contriving a file into the container would prove the code path while proving nothing about the claim that matters — that Kai can add a friend to the OVH box without a restart. **That check belongs to the box, i.e. thread 01's ops work:** write the new email into the map file, `docker compose kill -s HUP agentd`, and confirm the new project appears without a restart and without dropping a session. Until someone runs it there, A6 is merged-and-plausible, not verified. (2026-09-12, relayed from thread 01 and **verified here line by line**, not taken on trust) Thread 01 has accepted this live check as `docs/ops.md` step **11g** (commit `f5c482b`) and will report when it passes on the real box. It independently reached the same diagnosis as D2 did, and added one hazard worth having in writing: **(a) inline silently disables the whole feature.** `loadProjectSettings` (`go/cmd/agentd/googleauth.go:250-253`) returns on `AGENTKIT_PROJECT_MAP` before it ever looks at `AGENTKIT_PROJECT_MAP_FILE` — its own comment says "inline JSON, **wins**" — and `docker-compose.yml:168` passes that variable straight through from `.env`. So one leftover inline value in a real `.env` turns off SIGHUP and the timer with no warning at all, which is exactly why the e2e stack could not exercise the path. 11g therefore checks `grep '^AGENTKIT_PROJECT_MAP=' .env` prints nothing BEFORE testing anything. **(b) a moving counter is not a pass.** The proof is the account count rising in `[agentd] project map reloaded from …: N mapped account(s)` **and** the new account actually logging in; a counter that moves while the login path is broken is a false pass. **(c) a typo looks like "no reload".** A malformed or unreadable file keeps serving the previous map and says so — `googleauth.go:395` and `:400`, both logging "keeping the previous map" — so check for that line before concluding the reload is broken. **(d) it covers embedders too:** a successful reload recomputes the API-key index (`main.go:648-650`), because `api_key_env` lives in the same file's `projects` section, so adding an embedder's key has the same no-restart property as adding a person.
...
**38. DI29's root cause is closed: `examples/web` now has a test runner.** (orchestrator, 2026-09-12) DI29's own diagnosis named three gaps that had to line up, and the first was structural — the app shell the stack actually serves had no `test` script and no test files, so none of its logic was reachable by any offline gate. Added `examples/web/vitest.config.ts`, `src/test-setup.ts` and a `test` script, all mirroring `web/`'s shape (node environment by default, jsdom opted into per file, same setup file). The config **merges** `vite.config.ts` rather than restating it, because `src/onboarding.ts` imports `@agentkit/chat-ui` and must resolve it through the same three aliases the build uses — `web/dist`, the emitted package (O12) — so `yarn test` here needs `npm ci && npm run build` in `web/` first, exactly as `yarn typecheck` already did. Seeded with `src/onboarding.test.tsx`: four cases over `useInterviewState`, the first of which is DI29 itself — no `onboard` session and no architect at mount, then a session appears, and the hook must notice. **Proved by reverting:** restoring the old `if (!inInterview) settled.current = true` fails that case ("expected false to be true") and leaves the other three green; the fix passes all four. Wired into CI as a `yarn test` step in the `examples-web` job, before `yarn build`. The job's display name is unchanged — it is a branch-protection check name (see that file's header), and the `web` job likewise runs `verify-package.sh` without saying so. The second and third gaps DI29 named are untouched: A2's unit tests still take `inInterview` as a prop, and `onboarding.stack.spec.ts` still skips without `--mock-script`.

**39. DI37 happened again, to DI38's own commit — and this time it took the message.** (orchestrator, 2026-09-12) The six files of **38** were staged and verified, and between `git add` and `git commit` another session committed the shared checkout with a broad add. They landed in `8be169b` ("The booking system does not use Elasticsearch, so NoCode and Franchise Cloud can go off on schedule") alongside four files belonging to two other threads. Nothing was lost — `git show HEAD:<path> | md5sum` matches the working tree for all three new files — and history is left as found, because rewriting a commit you did not author is worse than a wrong message. The cost is that the repo's record of why `examples/web` gained a test runner is a sentence about a booking system. The message that belongs to those six files is this one: *"Give examples/web a test runner, and the test that would have caught DI29 — the app shell the stack serves had no `test` script and no test files, which is the first of the three gaps DI29 names."* Two further notes from the same episode. `yarn add -D --exact=false …` is not a yarn flag form: yarn read `false` as a **package name** and installed `false@0.0.4`, which rode into `8be169b` in `package.json` and `yarn.lock` before being removed here — check a lockfile diff for junk after any `yarn add`. And vitest 4 wants vite >=6 while this app builds on vite 5, so yarn gives vitest a nested vite 8; tests and the production build therefore transform through different vite majors, documented in `vitest.config.ts` and deliberately not resolved, since the alternative is a vite major bump on the shell the stack serves.

**40. C4 turned the repo-wide hygiene gate red, and this thread never ran it.** (orchestrator, 2026-09-12) Found by running `./scripts/check-no-nul-bytes.sh` for the first time in the whole fleet: `web/src/components/MemoryBrowserPage.tsx` carried **three raw NUL bytes** on one line, introduced by C4's merge `39a709b` and still present after C2's `2357287`. They were a deliberate separator — a template literal joining `title`, `initialLabels` and `initialContent` with NUL, keying a render-phase re-seed — written as the byte instead of the escape, which is the precise defect doc 22 RD25 records and the `hygiene` job exists to stop; CI would have gone red on the next push, and every `grep` over `web/src` was silently missing that file. Fixed by writing the escape `\u0000` (runtime string identical) and commenting why. **The lesson is about the gate list, not the byte:** D1's gate set was the three packages' own scripts plus verify-package, docker and the stack e2e — it did not include the repo-wide `hygiene` job, so a wave of 22 tickets ran to completion with a red CI check nobody had executed. Read `.github/workflows/ci.yml`'s job list, not a remembered gate list, when deciding what "green" means.

**41. DI10 is closed: the server states "applied" instead of the console guessing it.** (orchestrator, 2026-09-12) Kai picked this out of the triage list, and it is the one open finding that could strand a real human. `GET /agent/charter/current` now answers `applied` (and `applied_at`), read from the config log's `topology_apply` bracket, which already named the interview session in its answers — so the fact was durable, authoritative and unforgeable all along, and simply was not being read. No migration and no second source of truth. `examples/web/src/onboarding.ts` lost `ARCHITECT_WORKER_NAME` entirely; `useInterviewState` now reads the session, then that session's charter, and only `applied: true` ends the watch. `web/src/useCharter.ts`'s `applied` became "this screen's own apply **or** the server's record of one", which also fixes a smaller thing nobody had logged: a reload mid-onboarding used to forget the approval had happened and offer "Approve" on an already-approved charter. **The failure mode was reachable, not theoretical** — `TestGetCharter_AppliedIgnoresWhatTheArchitectIsCalled` asserts `valid: true` on a charter whose `architect_name` is `head-of-newsletter`, so a legal charter really could have hung the console forever. Proved at three levels: seven Go cases including a **live-Postgres round trip** (append the deposit, approve it through the real `ApplyTopology`, read `applied: true` back out of jsonb — the one claim the fakes cannot make, because `answered`'s type switch exists precisely because the nested `answers` object arrives as a different Go type off the database than off the write path); five `examples/web` cases, one of which asserts `/agent/workers` is never called at all, so a name check cannot creep back without failing; and one `OnboardingPage` case proved by reverting the `useCharter` line first. Also in the same sitting, from the same triage: `web/vitest.config.ts` gained `testTimeout: 20_000` (DI8 — the suite then ran 1713/1713 on a loaded machine, where the 5s default had been reporting 2–6 phantom failures), and the guide's hard word cap moved 450 → 500 with the reason written beside it (DI35), which brings all four over-running pages into compliance without truncating a verbatim quote. `field-notes.md` (894 words) and `glossary.md` (633) are still over any cap; DI35 never listed them and this did not change their status — they are a note collection and a term list, not prose pages, and whether the cap is meant to bind them is Kai's call. And one small embarrassment worth recording: writing DI40 put a **raw NUL byte** into this file, because the `\u0000` in its text was interpreted as an escape by the tool that wrote it rather than kept as six characters. The entry describing the defect introduced the defect, and `c02a749` shipped with it. Fixed here. The lesson is the same one, one layer up: after writing about a control character, run the gate that looks for it.

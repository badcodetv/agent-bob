...
(`api/src/bob/client.ts:255-270`) **always supplies one — the upstream's**. So an Orange **502**
...
| `api/src/bob/client.ts` | Typed Orange client |
...
- **Acceptance criteria:**
  ...
  - `wolf-web` joins Orange's compose network as **external** (`agent-bob_default`), publishes
...
- **Acceptance criteria:**
  ...
  - **`embed_url` is `${BOB_PUBLIC_URL}/embed/session/hyp-<id>`** — the *browser-reachable*
    Orange origin (`http://localhost:8080` in the compose stack), read from `BOB_PUBLIC_URL` in
...
- **Acceptance criteria:**
  ...
  - **Two build args, because `wolf-web` ships as a built nginx image and cannot read runtime env.**
    `VITE_BOB_PUBLIC_URL` (default `http://localhost:8080` — Orange's *browser-reachable*
...
- Notes: **Wave 14, 2026-08-26 — PASSED, two fix rounds, one BLOCKING defect. Merged as `a155c96`.** api 36 files / **1696** tests (from 1649), web 31 / 347, both typechecks and the web build clean on `main`. Six files: `hypothesis/store.ts`, `routes/hypotheses.ts`, `app.ts` and their test twins, +2255/−68.
  ...
  **Left open, all logged, none W22's to fix:** **R188** — `kind=report-candidate` has **no reader anywhere**, so half this ticket's own isolation criterion is vacuous as written. **R189** — the detail route makes 19–20 Orange reads where ~11 would do, because `composeFor` reads before consulting its own cache; the fix touches W21's interface. **R190** — `BOARD_LIMIT = 100` is decoration because both fakes ignore `limit`, so the board silently truncates past 100 hypotheses while the detail page does not. **R187** — the per-name follow-up read is `O(attacked)` and the attacker picks N; W27's restated budget was amended for it. And one note: a **live, template-less** hypothesis now answers `422 "author one before going live"` where it used to answer `409 "cannot go live from live"` — no HTTP test covers the 409 path, so nothing broke, but any hypothesis that went live before W22 gets a misleading refusal.
...
- **Acceptance criteria:**
  ...
  - `e2e/bob-override.yml` is passed to Orange's compose with `-f` and bind-mounts
...
- Notes: **Wave 17, 2026-08-26 — PASSED, four fix rounds, ZERO blocking. Merged as `ac17884`.** api 37 files / **1826** tests on `main` after merge (W29 alone: 1732 → 1750), web 36 / **479** (from 462), all five gates green. **25 landing-gated mutations by the verifier, 22 killed**; all seven criteria PASS.
  ...
  **Left open, none W29's:** 🔴 **Orange's `ListArtifacts` is unbounded end to end** (`go/agentdb/artifacts.go:72-85` — `Order("created_at DESC").Find(...)`, no LIMIT, no pagination), while every other proxied list in this project is capped and a hypothesis is validated daily for its whole life — **R225's shape, filed with R190's cap work**. `getSessionArtifactFile` is **unreachable from any route** (proved empirically: making it throw killed exactly 3 of 1750) with its **501** branch untested — the one a developer without Postgres hits first. And 🔴 **Orange's own published package disagrees with its Go source about `source`**: `"registered"` is written by **no Go code anywhere**, `"tool"` appears **only in Orange's tests**, and production writes `"capture"`, which is in **neither** half — **the two halves intersect on exactly `'auto'`**. Wolf's wire preserves the true value verbatim and `source` is read zero times in the rendered subtree, so the mapping loses nothing; widening the union is Orange's change.
- Notes:
...
- **Why, and it was found sideways.** W31's audit hit this while mutating for a different purpose;
  ...
  page zero is not an error, it is an infinite loop. 🔴 **And Orange will do exactly that:**
...
- **Severity, as called by the verifier:** **latent, not live** — nothing triggers it today. But a
  one-line Orange regression, a proxy that strips a query parameter, or a future route change turns
  **every board read, every detail read and the poller's enumeration** into an unbounded request
  loop against Orange, with **no timeout, no error and no log** — a hung request plus a
...
- **Why this exists, and why it is not part of the consolidation.** **R184** records that fidelity is
  ...
  `user_email: "*"` from `orange/client.ts` went from killing **zero** tests to killing **76**. Twelve
...
- **Acceptance criteria:**
  - The document enumerates **all fourteen** fakes by path: `app`, `bootstrap/bootstrap-project`,
    ...
    `marketdata/stooq`, `orange/client`, `routes/auth`, `routes/embed`, `routes/hypotheses`,
...
| **R268** | 🔴 **THE PROMPT TOLD THE MODEL TO READ SOMETHING IT CANNOT SEE, AND THE RESULT WAS A GOOD INTERVIEW THAT COULD NOT BE TAKEN LIVE.** `prompts/interviewer.md` said the candidate memory's `name` label is "this hypothesis's bare id (the session you are running in is named `hyp-<id>`; use the id without that prefix)". **A session container is given `SESSION_ID` — Orange's 32-hex id — and `SESSION_TOKEN`, and nothing else. The session's NAME never reaches it** (`go/runner.go:2735-2751`; `SessionEnv` is a fixed map). So the instruction was unfollowable, the model did the only thing left and invented a slug from the thesis (`gold-m2`), Wolf looks candidates up by `name=<id>`, found none, and the user's Go Live button never appeared. What he saw instead was the server's honest-but-now-false sentence *"no spec has been proposed yet — the interview has to deposit a hypothesis-spec-candidate memory first"*, while two perfectly good candidates sat in the database under a name nothing reads. **THE RULE: an instruction that refers to state is only as good as the model's access to that state — so when you write "you can see X", go and check that it can.** The tell was available the whole time and I walked past it twice: I had already read `sandbox/src/config.ts`'s env list and printed the container's environment (`SESSION_ID=<set>`, `SESSION_TOKEN=<set>`) an hour earlier while answering a question about the model, and did not connect it. Fixed Wolf-side by putting the id in the interview's FIRST MESSAGE, attributed to Wolf so a thesis claiming a different id reads as user text (§ 6.2.4); an Orange `SESSION_NAME` would be cleaner and touches the engine and every embedder. |

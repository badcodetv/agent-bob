...
2026-08-24); see the Discovered Issues Log, entries R1–R141. Two owner rulings on 2026-08-21 added
W8b and W2b; revision 5 adds O12, O13, W27, W28, W29 and W30, so the ticket count is **47**, not 39.
**33 are done**; W13 landed 2026-08-24 and the UI chain continues W14 → W23 → W24.
...
- [ ] done
- Notes: **Hand-off from W13 (2026-08-24), read before starting.** (1) **`ChatRail` already exists** and owns sticky / `100vh` / `clamp()` / collapse / the below-`md` tab — take a bare id and do not re-implement the column. (2) **`HypothesisDetail.tsx` already fetches `GET /api/hypotheses/:id`** and holds `spec_validation`; **widen `HypothesisDetail` in `web/src/api/types.ts`** rather than declaring a second type or adding a second fetch. (3) The two-column flex row already wraps below `md`; W14's left column is just a child. (4) `HypothesisRow` is shared with the archive. (5) 🔴 **`title_truncated`'s `degraded` sentence is W14's** — the board deliberately carries only an ellipsis affordance (**R141**). (6) `testUtils.tsx`'s `stubFetchRoutes` **throws on unrouted paths**, so any test rendering the detail page must stub the embed-token route, and anything rendering the rail needs fake timers. (7) 🔴 **Stub `window.matchMedia`** for every responsive criterion — jsdom defines none, so `useMediaQuery` is always `false` and the below-`md` branch runs in no test and fails silent (**R139**).

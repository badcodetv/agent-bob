...
> dependencies say otherwise. **Only the orchestrator writes to this file** —
> Status, checkboxes, Notes and the Discovered Issues Log alike. 🔴 **Workers
> and verifiers must NOT edit the plan; report every finding to the
> orchestrator, who writes it up** (amended 2026-08-26, **R175**: two to four
> agents run per wave, concurrent appends conflict in the one artefact that has
> to survive the wave, and only the orchestrator sees a whole wave's findings at
> once — which is also what has caught four defects in orchestrator-written text
> that a worker appending directly would have had no occasion to find). A
> ticket's checkbox is checked only after its Validation commands have been
> re-run by the orchestrator and pass. Do not expand scope; report surprises
> instead.
>
> 🔴 **THE HARNESS RULES ARE NOW FIVE, NOT FOUR** (amended 2026-08-26, **R196**). Any ticket
...
> ran, which a mutation sweep reads as "undefended" and acts on.
>
...
W8b and W2b; revision 5 adds O12, O13, W27, W28, W29 and W30, so the ticket count is **49**, not 39
(**W31**, the test-fake fidelity audit, was authored at the wave-15 cut on 2026-08-26 — see R184 and
R190 — and **W32**, the unbounded-page-walk fix, was authored the same day from a defect W31 found
sideways, so the count is **49**).
**42 are done.** W13 and W14 landed 2026-08-24; W30, W17, W19, W21, W25, W22, **W23** and **W31**
landed 2026-08-26. **Seven remain: W24, W26, W27, W29, W32, O10, X1.** W24 needs W23 (done);
W27 needs W22 (done); W29 is ready and 🔴 **must not run alongside W23-touching work — both modify
`web/src/pages/HypothesisDetail.tsx`** (R194); **W32 is independent of everything** and should run
early, being two lines and the only open finding that can hang production.

⚠️ **Revision 5 reverses a standing decision and rewrites five criteria.** `web/` **is** installable
— proved by building, packing and consuming it (see **R125**) — so Wolf now imports Bob's types
...
single branch carried any of those combinations. Both trunks are gated green — agent-bob
...
`design/2026-08-06-embeddable-agent-bob.md`, `docs/18-workers-memory-events.md`,
...
Agent Bob is the runtime. `docs/19-embedding.md` was written for exactly this integration and
names Agent Wolf as its driving use case; its § 8 "two-bot pattern" is the shape this plan
follows. Wolf owns the page, the vocabulary and the user allowlist; Bob owns prompts,
sessions, schedules, memories and (after this plan) datasets.

### The two problems Bob cannot solve today

**1. There is nowhere to put a shared numeric time series.** Bob has memories (append-only
...
**2. An application embedding Bob cannot write a memory.** Verified:
...
- **Tiered UI reuse — REVISED 2026-08-24, revision 5 (was "iframe-only").** `web/` *is*
  ...
  foreign app (**R125**). Wolf therefore **imports** Bob's types and pure logic (tier 1) and its
  ...
  tier where a Bob change would force every client app to move. Rationale and the tier table:
  `design/2026-08-24-agent-wolf-ui.md` § 1.
- **Dataset atom in Bob, not a file store in Wolf.** It matches Bob's existing atom grain
...
- **One `wolf` project, slug owner labels.** Bob label values are the K8s charset
...
| | Path | What it is |
| --- | --- | --- |
| **agent-bob** | `/home/kai/projects/badcode/agent-bob` | The existing engine. Go module `github.com/badcodetv/agent-bob`, plus a TypeScript in-image agent (`sandbox/`) and a React component library (`web/`). |
| **agent-wolf** | sibling directory `agent-wolf`, created by W1 | The new product. Node/TS API + React UI. Imports **nothing** from agent-bob. |

Ticket ids beginning `O` change agent-bob; ids beginning `W` change agent-wolf; `X1` spans
both.
...
| Test runner | **vitest 4** in both `api/` and `web/` | Matches Bob's JS packages |
...
🔴 **A KIND IS NOT A STATUS, and the two travel independently — clarified 2026-08-26 (R230), because
the plan's own prose reads as a promise the code does not keep.** `DEFAULT_STATUS` (`api/src/errors.ts:28`)
maps each kind to a status **only when no status is supplied**, and `defaultErrorFor`
(`api/src/bob/client.ts:255-270`) **always supplies one — the upstream's**. So a Bob **502**
reaches the browser as **502 with `kind: "unavailable"`**, not as 503. That is existing behaviour on
**every** Wolf route and it is the right behaviour — the upstream status is information a caller may
want — but anything branching on **status** rather than **kind** will be surprised. **Branch on the
kind.** W10's poller does; it is correct.
...
**Bob refusing a session because its host port pool is exhausted maps to `unavailable`**, and
here the retryable semantics are correct rather than a bug: deleting a finished session frees a
port, so the condition genuinely clears. Surface Bob's message verbatim — "host port pool is
...
| `api/src/bob/client.ts`, `client.test.ts` | W2, W15, **W2b**, **W29** | Strictly serial in that order. W2's route list was declared exhaustive and closed; **W2b adds the twenty-third, `GET /agent/workers/{name}`, by owner decision 2026-08-21 (R91)** — the list is closed against casual addition, not against an owner ruling |
...
| `api/src/app.ts` | W1, W7, W8, W11, W21, **W22**, **W29** | **W22 added 2026-08-26**: wiring W21's `composeReportStats` into the hypotheses router needs one line here and one in `CreateHypothesesRouterOptions`, and neither this row nor W22's Files line carried `app.ts` — W22 would have been blocked mid-ticket. 🔴 **W21 and W29 must NOT run concurrently** — both modify this file, and wave 13 ran W21 alongside W25 for exactly that reason. | Strictly serial; every router is mounted here, by the ticket that creates it |
...
| `web/src/pages/*.tsx` | **W13**, W14, **W23**, **W29**, W24 | W13 creates `HypothesisList`, `NewHypothesis`, `Archive`, `SignIn` and a **working** `HypothesisDetail` placeholder (back-link, title, chip, tamper alerts, go-live gate, rail, two-column layout). **W14 replaces the LEFT COLUMN's contents only**; W24 adds `/hypotheses/:id/golive` to the same `<Routes>` in `App.tsx`. 🔴 **W23 and W29 added 2026-08-26 at the wave-15 cut — BOTH modify `HypothesisDetail.tsx` and they must NOT run concurrently** (W23 replaces the `report-placeholder` seam; W29 adds the artifacts panel below it). Neither ticket's own Files line was wrong; the row simply did not list them, and the wave was cut as `W23 ‖ W29` before the check caught it. **R194** — the thirteenth time this table has been found short, and the second time in three waves that the collision was between two tickets neither of which is the row's author |
...
### The three standing pre-wave checks (run all three at every wave cut)

1. **The citation grep (R154).** Normalise every `§ "…"` citation and every heading; prefix-match one
   against the other. Catches a ticket citing a section that does not exist — which has happened
   three times and blocks the executor outright, since they are told this document is sufficient.
2. **The vocabulary sweep (R163).** Every pending ticket's enumerated values against the code's
   exported unions, checking **completeness** and not only correctness — an omission has no token to
   grep for.
3. 🔴 **The Files-line pass (R194, R206), which is two steps and the second is the valuable one.**
   First **intersect the proposed wave's Files lines pairwise** — two tickets naming one file must not
   run together, whatever the dependency graph allows, and the ownership table goes stale because it
   records who *created* a file and is never revisited when a later ticket *modifies* one. Then
   🔴 **reconcile each Files line against that ticket's own acceptance criteria and ask what the
   criteria need that the line does not name.** On its first run the intersection caught an
   approved-and-illegal wave, and the reconciliation caught an acceptance criterion that could not be
   built at all because a route it assumed had never existed. **The reconciliation is the half that
   finds the bigger defect.**

### Committing and reporting — the order matters (R201)

🔴 **Commit first, then run Validation on the clean tree, then paste the sha above the output.**
Not the other way round. Validating a dirty tree and committing afterwards is honest and it is what
several tickets have done — but it leaves a window in which `git status` shows modified files on
work that is finished, and an orchestrator polling git state cannot tell that apart from an agent
reporting completion over uncommitted work. Committing first makes the pasted output provably *of*
the named commit. It also removes a real hazard: a worktree pruned during that window destroys the
work with no trace.

🔴 **Before you MERGE: `git log --oneline <branch>` and compare the tip against the sha the review
quoted.** If they differ, read what arrived in between *before* merging. A merge that lands ahead of
the review it is acting on puts a state in trunk that a reviewer may have explicitly rejected, and
nothing records the disagreement (R214, merge-side half).

🔴 **Orchestrator: never prune a worktree, or delete a branch, on a stale reading of another
agent's state.** Before either, re-read `git status --porcelain` **and** `git log --oneline -1`
inside the worktree — the reading and the deletion are not atomic, and an *idle notification is not
"finished"*: an agent can be idle between two of its own actions. Before `git branch -D`, confirm
ancestry with `git merge-base --is-ancestor <branch> <target>`; `-D` deletes regardless and only
prints the sha as it goes. **If you have already deleted one, that printed sha recovers it** —
`git cat-file -t <sha>` proves the object survives, `git branch <name> <sha>` restores it. A prune
mid-command returns exit 1 with no output, which reads exactly like a genuine failure — one agent
lost a `yarn build` this way and correctly refused to report a green it had not seen. **Both
hazards were walked into within an hour of this section being written (R201, R205), so treat the
class — "a destructive step on a stale reading" — as the rule, not the two procedures.**
...
| Term | Meaning here |
| --- | --- |
| **Project** | Bob's tenancy boundary. Wolf uses exactly one, id `wolf`. The `customer` claim in a token *is* the project. |
...
```
  ...
  │                                                                      │
  │  wolf-web (React + MUI, own components, imports no Bob code)      │
  │    │  /api/*              <iframe src=bob/embed/session/…#token>  │
  │    ▼                                     │                           │
  │  wolf-api (Node/TS)                      │                           │
...
Bob's append-only posture while giving mutable-file ergonomics.
...
- *A blob store in Wolf* — keeps machinery out of Bob, but makes Wolf a stateful service and
...
Bob's memory is a genuine shared bus: project-scoped, append-only, **no per-worker permissions
...
1. One request:
   ...
   (R90).** Without it Bob's `notRetractedSQL` hides Wolf's newest state row, and `latest_per`
...
```
   browser
      ...
      ▼
   wolf-web  (nginx; on network agent-bob_default)
      │  proxy_pass http://dind:8100   ← wolf-api shares dind's netns
      ▼
   wolf-api  network_mode: "container:${BOB_DIND_CONTAINER}"
...
- `api/src/config.ts` resolves the gateway **at boot**: read **DinD's own `docker0` bridge address**
  — the **`docker0` row** of `/proc/net/route` (its Destination is the bridge's network; the bridge
  itself sits at network **+1**) — and 🔴 **never the destination-`00000000` default route, which
  under compose is the OUTER compose-network gateway.** Fall back to the literal `172.17.0.1` when
  discovery fails, and `pino`-log which of the two was used, at info, once.
  ⚠️ **CORRECTED 2026-08-27 (R236). This bullet previously said "read the default route", and that
  is the bug X1 found (R235) and W33 fixed.** The parenthetical that made it convincing — *"wolf-api
  shares that namespace, so this reads DinD's own table"* — was **true**, and is exactly what made
  the error survive: DinD's own table has **two** gateways, and the sentence silently assumed the
  interesting one was the default. 🔴 **THE GENERAL LESSON: a discovery spec must name the ROW, not
  the file.** "Read X from Y" is under-specified whenever Y holds more than one plausible answer —
  and that under-specification is **invisible on a default install**, where the candidates collapse
  to one and every host-side check agrees.
...
hypothesis id), and the current state is re-read from Bob *inside* the critical section. There
...
| `api/src/bob/client.ts` | Typed Bob client |
| `api/src/bob/types.ts` | Response shapes, timestamp units in the type names |
...
| `web/src/components/{BobChatFrame,Scoreboard,MetricChart,Timeline,StatusChip,ConditionTable,TamperWarning}.tsx` | Components |
...
is what Bob returns, and `tMs`/`fetched_at_ms` are unix **milliseconds**. An earlier draft wrote
...
type UnixSec = number;   // Bob sessions, embed-token expiry
...
**Embed-token expiry is `expires_at_sec`.** Bob returns unix **seconds**; W13 computes a T-120s
...
⚠️ **`Authorization: Bearer ${WOLF_MCP_TOKEN}` cannot work.** Bob rejects partial interpolation
...
### Memory kinds (the Wolf vocabulary, all in the `wolf` Bob project)
...
- **Fixing Bob hazards H1–H13** (`docs/19-embedding.md`). H1 — an embed token carrying
...
27 tickets: **O1–O11** (O6 split into O6a/O6b) in agent-bob (front-loaded — Wolf's data path *and* its state path both
depend on them), **W1–W14** in agent-wolf, **X1** spanning both.

W1, W3 and W6 have no Bob dependency and may start immediately in parallel.
...
- **Validation** — start the throwaway instance first (§ "The throwaway Postgres"), export
  ...
  - `cd go && ! go list -deps ./agentdb | grep -q 'agent-bob/extension$'` — exits 0
...
### O10: Document datasets and the memory append route   [Status: done | Model: sonnet — run on opus]
...
- **Validation** — each command has a stated expected output; run from the repo root. Docs-only
  ...
  # 6. The core-tools table gained the three tools. Must print >= 4.
  #    (Corrected 2026-08-26, R207: this said "must print 3" and 3 is ARITHMETICALLY
  #    UNREACHABLE. `grep -c` counts matching LINES, not occurrences, and the criteria
  #    require a table row naming all three tools — one line — PLUS "each with its
  #    one-line contract", three more. The minimum is 4, and 3 is only reachable by
  #    violating the criterion the check exists to enforce.)
...
- [x] done
- Notes: **Wave 16, 2026-08-26 — PASSED, two fix rounds. Merged as `27570ca`.** `docs/20-datasets.md` created (459 lines, twelve headed sections); `docs/19-embedding.md`, `docs/18-workers-memory-events.md` and `CLAUDE.md` amended. Exactly four files, no Go change, no test change, `go build`/`go vet` exit 0. Verified **PASS on all 14 criteria**, with **16 spot-checks re-derived from the Go source and 16 of 16 agreeing** — including both directions of the inverted `0`, the shrink guard's exact boundary, the CAS race and the full status vocabulary.
  🔴 **THE TICKET NAMED THREE STALE CLAIMS. THE EXECUTOR AND ITS VERIFIER FOUND NINE, IN FOUR DISTINCT CLASSES, AND ONLY ONE CLASS HAD A LENS.** (1) **A retracted fact surviving as somebody else's JUSTIFICATION** — § 5.1 asserted the shared-secret claim H12 retracts, *as the reason* the `sid` is empty, so a reader treats it as load-bearing. Neither a grep for the hazard nor for the section finds it; only a grep for the claim's **consequence** does (**R213**, F9). (2) **A bare cardinal** — "two ways to carry state between runs", "One middleware accepts all three", "The three credentials" when there are four (**R208**). (3) **Plausible-mechanism drift** — two claims *about* the code that the code does not make, both inside the brand-new document, both surviving an author who verified every value (**R211**). (4) **Citation rot** — five `file:line` refs broken by this plan's own inserts, every one still pointing at *something* so nothing looked broken. 🔴 **The sharpest single instance: O8's insert pushed a citation onto a line O8 ITSELF ADDED**, so the doc pointed at the dataset reaper while claiming to describe console login.
  **New hazard `H14`, and it was verified rather than asserted.** A dataset download token travels in the **query string** — the only credential in the tree that does; every other rides in a header or a fragment browsers never transmit. The executor checked the logging consequence instead of assuming it: **agentd writes no access log**, but nginx proxies `/agent/` (`deploy/web.nginx.conf:54-62`) and its default format includes the query string, so a download fetched through the web origin lands in a log with the token in it. Bound stated as a bound (300s default, 900s ceiling, one `(project,name)`), with three imperatives.
  **Three defects in the plan's own text, all found by the executor and all corrected:** check 6's expected value was **arithmetically unreachable** (**R207**); the default was documented in a syntax the parser **rejects** — `64MiB` is a boot error, so an operator copying the doc would fail to start; and the two reaper knobs read the same digit **oppositely** (`0` = never sweep vs `0` = keep everything), which `gc.go:182-215` argues at length would delete every non-current version if inverted.
  **Honest gaps documented rather than smoothed:** there is **no HTTP write path for datasets at all** (verified independently — `httpapi.DatasetStore` declares four read methods and no create or delete), so an embedder cannot seed one from outside a container; `allow_shrink` is unverified; nothing enforces the canonical CSV; reaping is not archiving. And **the guarantee is stated as a limit, not a promise** — *"the plan never **requires** bytes in context"*, with both caveats, the credential's reach spelled out, and the 200-character transcript truncation named as **a rendering detail, not a security boundary**.
  **Left open:** **R212** (two env tables enumerate an incomplete "subset" by name); **F10** — `go/cmd/agentd/embedtoken.go:158-172`'s comment still carries the retracted shared-secret fact in its pre-split wording, citing lines that no longer exist, and is the likely **source** of § 5.1's sentence; and doc 19's ~60 remaining citations into files this plan never touched are a **known unknown**, reported as such rather than implied clean.
- Notes:
...
- **Acceptance criteria:**
  - `wolf-api` uses `network_mode: "container:${BOB_DIND_CONTAINER}"` (default
    `agent-bob-dind-1`), so it shares DinD's namespace: it reaches agentd at
    `http://localhost:8099` and session containers reach it at
    `http://172.17.0.1:${WOLF_API_PORT}`.
  - `wolf-web` joins Bob's compose network as **external** (`agent-bob_default`), publishes
  ...
  - `.env.example` documents every variable `api/src/config.ts` reads, each with a comment, and
    `BOB_DIND_CONTAINER`, `WOLF_API_PORT`, `WOLF_WEB_PORT`, `WOLF_MCP_URL`.
  - **`web/` imports nothing from agent-bob — enforced by a test, and the test is graded on
  ...
  - **`api/` is held to the same boundary.** The architecture forbids it importing Bob code
    just as firmly, and nothing asserted it. Reuse the same checker; do not write a second one.
  - `README.md` gives the exact two-repo start sequence: bring Bob up, then Wolf, in that
...
- Notes: **Implemented 2026-08-20; 10 of 11 criteria verified PASS; ESCALATED on the eleventh.**
  ...
  **Round 4 (2026-08-21), against the rewritten criteria — 19 of 22 sub-criteria PASS, 3 fail.**
  ...
  caught**, plus two it invented (a symlinked directory into agent-bob, and `export * from` an
...
### W2: Bob client   [Status: done | Model: sonnet]
- **Scope:** A typed client for every Bob route Wolf touches. **The list is exhaustive and
  closed** — no later ticket may edit `api/src/bob/client.ts` except W15, which is strictly
...
- **Repo:** agent-wolf
- **Files:** `api/src/bob/client.ts`, `api/src/bob/types.ts`,
  `api/src/bob/client.test.ts`, `api/src/bob/types.test.ts`.
- **Acceptance criteria:**
  ...
  - **The client reads no environment variable.** It is constructed as
    `createBobClient({ baseUrl, apiKey })`; the caller supplies both. A test reads
  ...
  - **Timestamp units are branded types**, declared once in `api/src/bob/types.ts`:
    ...
    `expiresAtSec` are `UnixSec`. The proof is compile-time, in `api/src/bob/types.test.ts`: a
  ...
  - **Non-2xx responses become `WolfError`s** carrying Bob's status and body verbatim in
    ...
    a timeout `→ unavailable` (Bob being down is an upstream outage, not a Wolf bug); anything
  ...
  - **One documented exception to that table:** a `404` from `POST /auth/verify-google` is
    `misconfigured` naming **`GOOGLE_CLIENT_ID`**, not `not_found` — Bob 404s that route when
    the variable is unset on *Bob*, and W8 must surface it as a configuration error rather than
    an auth failure.
  - **A `403` refusing a session create because the host port pool is exhausted maps to
    `unavailable` and preserves Bob's message verbatim** — "host port pool is exhausted" is
...
- **TDD:** yes.
- **Validation:**
  - `cd api && yarn test src/bob` — exits 0. `vitest` exits **1** when a path filter matches no
    files, so a green run here does mean the suite ran
  - `cd api && grep -c "it(" src/bob/client.test.ts` — at least 22, one per route, plus the
  ...
  - **The brand proof, two steps:** delete the `& { readonly __unit: "ms" }` suffix from `UnixMs`
    in `src/bob/types.ts` and re-run `yarn typecheck` — it must **fail** with "Unused
...
- **Depends on:** W1 *(and O7 for the append route's 201, O11 for `include_retracted` — the unit
  tests mock the HTTP layer with `MockAgent`, so neither Bob ticket blocks this one)*
- [x] done
- Notes: **Delivered on branch `W2-bob-client`, commit `c71d97d`, based on
  `W1-wolf-repo-scaffold` (not merged to `main`). One fix round.** Orchestrator re-ran the
  Validation on 2026-08-21: `yarn typecheck` clean, `yarn test src/bob` green (**52 tests**,
...
### W2b: The twenty-third Bob route — worker read   [Status: done | Model: sonnet]
- **Scope:** Add `getWorker(name)` to the Bob client and repoint W12's bootstrap at it, removing
  the one raw `fetch` in the repo. Owner decision 2026-08-21 (**R91**): W2's route list was
  declared "exhaustive and closed" and had no worker **read**, while Bob serves one
...
- **Repo:** agent-wolf
- **Files:** modify `api/src/bob/client.ts`, `api/src/bob/client.test.ts`,
...
- **TDD:** yes.
- **Validation:**
  - `cd api && yarn test src/bob/client src/bootstrap --reporter=verbose` — confirm both files'
    test counts went up.
...
- **Depends on:** W2, W12, **and W15** *(W15 holds `api/src/bob/client.ts` immediately before
...
- Notes: **Implemented and independently verified 2026-08-21 — 7 of 7 criteria PASS, zero fix
  rounds.** Branch `W2b-bob-worker-read`, commit `56f110c`, merged to `main`. api/ 644 → **648
  ...
  around. `readWorker` swallows only `not_found` and **rethrows every other kind**, so a Bob
...
- **Files:** create `api/src/hypothesis/lifecycle.ts`, `api/src/hypothesis/store.ts`,
  ...
  `api/src/hypothesis/__fixtures__/` (Bob response bodies captured verbatim from a running O11
...
- **Acceptance criteria:**
  ...
  - **All six self-pairs are no-ops**, not errors and not writes (**B3**): they return the current
    ...
    A test asserts the Bob client received no `POST /agent/memories` for a self-pair.
  ...
  - Transitions are serialised per hypothesis id by an in-process async mutex, and the current
    state is **re-read from Bob inside the critical section**. A test drives two concurrent
  ...
  - **`Tamper` is exactly the shape pinned in § "Shared shapes"**:
    ...
    Bob sets **both** for any session that has a worker (`go/cmd/agentd/mcpserver.go`), which is
...
- Notes: **Implemented, FAILED independent verification, fixed, re-verified 2026-08-21 — 17 of 17
  ...
  board.** The board's fast path read without `include_retracted`, so Bob's `notRetractedSQL`
...
- **Scope:** An HTTP MCP server named **`wolf`** exposing `series_search` and `series_fetch`,
  ...
  per-hypothesis, JSON points over Bob *datasets*), and a `download_url` pointing at nothing
...
- **Acceptance criteria:**
  ...
  - **The credential is sent as the bare header `X-Wolf-Mcp-Token: <token>`, with no scheme
    prefix**, and compared in constant time. `Authorization: Bearer …` **cannot** work: Bob
...
- **Scope:** `POST /api/auth/google`, `GET /api/hypotheses`, `GET /api/hypotheses/:id`,
  ...
  Raised to **opus**: the allowlist, the cookie guard, the board parser and Bob's *asynchronous*
...
- **Acceptance criteria:**
  - **The allowlist is `WOLF_ALLOWED_EMAILS`**: comma-separated, case-insensitive,
    ...
    empty allowlist must not silently mean "everyone". An identity Bob verifies that the list
    does not contain is rejected **403, kind `forbidden`**: Bob verifying a token is necessary,
    never sufficient.
  - Wolf calls Bob's `POST /auth/verify-google` server-side with `WOLF_API_KEY` (that route is
    ...
    `registerVerifyGoogle` mounts nothing when that variable is unset *on Bob*, and a
  ...
  - **Bob's session create is asynchronous and reports no provisioning failure on the POST.**
  ...
  - **`BOB_BASE_URL` is documented in `.env.example` and read through the typed `WolfConfig`**
    ...
    can be given a home. This is where R49's "Bob's base-URL variable, pinned nowhere" finally
  ...
  - `atoms` names the four Bob atoms from § "Bob atoms per hypothesis": the `hyp-<id>` session
...
- Notes: **Implemented and independently verified 2026-08-21 — 21 of 21 criteria PASS, zero
  ...
  that would otherwise have surfaced inside a container at X1. R92 is closed: `BOB_BASE_URL` and
  ...
  **Three findings handed forward.** (1) Two defects in `api/src/bob/client.ts`, which W8 does
  not own: `classifyStatus` has no 401 case, so a Bob 401 reads as kind `internal` (W8 detects
  ...
  W14's detail page (W5's known limit), and W12's bootstrap still reads `BOB_BASE_URL` from
...
- **Scope:** The mechanism that actually moves a hypothesis to `challenged`, and the writer of the
  board's numbers. Nothing else does either: Bob cannot call Wolf (no webhook; subscriptions
...
- **Acceptance criteria:**
  ...
  - **Tick sessions are swept without consulting status.** List
    ...
    `pending` or `running` for that worker**. Bob has no "completed" session status: a finished
...
- **Scope:** `GET /api/hypotheses/:id/embed-token` and `GET /api/hypotheses/:id/series/:metric` —
  ...
  from Bob. Both are read-only; neither changes hypothesis state.
- **Repo:** agent-wolf
- **Files:** create `api/src/routes/embed.ts`, `api/src/routes/embed.test.ts`,
  ...
  routers), `api/src/config.ts`, `.env.example` (`BOB_PUBLIC_URL`), **`docker-compose.yml`** and
...
- **Acceptance criteria:**
  - The embed token is minted through `POST /agent/embed-token {session: "hyp-<id>"}` with
    `WOLF_API_KEY`, **sending no `ttl_seconds` at all**, so Bob applies its own 900s default:
  ...
  - **The response is `{ token, expires_at_sec, embed_url }`, and `expires_at_sec` is unix
    SECONDS.** Bob returns the token's own `exp` claim in seconds (`embedTokenResponse.ExpiresAt`
  ...
  - **`embed_url` is `${BOB_PUBLIC_URL}/embed/session/hyp-<id>`** — the *browser-reachable*
    Bob origin (`http://localhost:8080` in the compose stack), read from `BOB_PUBLIC_URL` in
    ...
    breaks every non-compose deployment. `BOB_PUBLIC_URL`'s origin must also appear in O8's
  ...
  - A hypothesis whose `hyp-<id>` session does not exist is **404 `not_found`**. Bob answers 404
    for absent and for another project alike and Wolf does not distinguish either — the route is not
    an existence oracle.
  - **The series route proxies bytes server-side and never redirects the browser to Bob.** Bob
  ...
  - **The response is `{ points, unit, version, fetched_at_ms, state }`**, where `points` is
    ...
    - a `404` from Bob (the dataset has never been written) returns **200** with `points: []`,
...
- **Acceptance criteria:**
  - `installations/wolf/Dockerfile` begins `ARG BASE_IMAGE=agent-bob-core:dev` / `FROM
  ...
  - **`scripts/load-image-into-dind.sh` builds the image inside DinD, not on the host.** A
    ...
    (`tar -C installations/wolf -cf - . | docker exec -i "$BOB_DIND_CONTAINER" docker build …`),
    defaults the container name to `agent-bob-dind-1`, builds `agent-bob-core:dev` into DinD
    from `$BOB_REPO/installations/core` (default `../agent-bob`) when that tag is absent,
    fails naming the exact command when that path does not exist, and is safe to re-run.
  - The script's last step is `docker exec "$BOB_DIND_CONTAINER" docker run --rm <tag> python3
    -c "import pandas, numpy, duckdb"`, and a non-zero exit fails the script. A comment states
    that Bob's compose `BASE_IMAGE` must **never** be set to the Wolf tag: `init-sandbox`
  ...
  - `mcp_config` is written as `{"wolf": {"url": "<WOLF_MCP_URL>", "headers":
    ...
    config value, never re-derived here. A test asserts the value satisfies Bob's whole-value
    `${VAR}` rule (no `Bearer` prefix, no partial interpolation).
  - **`attention_channel` is written explicitly EMPTY (`{}`), and that is the criterion.** Bob's
  ...
  - **A prompt-contract test** (in `bootstrap-project.test.ts`, so `yarn test src/bootstrap` grades
    ...
    `interviewer.md`.
  - The idempotency test runs the script twice against an `undici` `MockAgent` Bob and asserts
...
- **Validation:**
  ...
  - with agent-bob's stack up: `./scripts/load-image-into-dind.sh` — must exit 0 and print the
    `import pandas, numpy, duckdb` success line
  - `docker exec "${BOB_DIND_CONTAINER:-agent-bob-dind-1}" docker image inspect
    --format '{{json .Config.Cmd}}|{{.Config.WorkingDir}}|{{json .Config.ExposedPorts}}'
    agent-wolf:dev agent-bob-core:dev` — the two lines must be identical (house rule 4)
- **Depends on:** W2, W7 *(W2 owns `api/src/bob/client.ts` and its route list is closed, so the
...
- Notes: **Implemented, FAILED independent verification, fixed, re-verified 2026-08-21 — 16 of 16
  ...
  `bob/client.ts` because W2's closed route list has no worker **read** and idempotency is
...
- **Scope:** `HypothesisList`, `NewHypothesis`, `Archive`, `BobChatFrame`, `StatusChip`,
...
- **Files:** create `web/src/pages/HypothesisList.tsx`, `web/src/pages/NewHypothesis.tsx`,
  `web/src/pages/Archive.tsx`, `web/src/components/BobChatFrame.tsx`,
...
- **Acceptance criteria:**
  ...
  - **Two build args, because `wolf-web` ships as a built nginx image and cannot read runtime env.**
    `VITE_BOB_PUBLIC_URL` (default `http://localhost:8080` — Bob's *browser-reachable*
  ...
  - 🔴 **`BobChatFrame` is a full-height RAIL, not a block in a document** (UI design § 5).
  ...
  - `BobChatFrame`'s `src` is `${VITE_BOB_PUBLIC_URL}/embed/session/hyp-<id>#token=<token>`,
    ...
    or Bob's CSP `frame-ancestors` blocks the frame outright.
  - **The embed token's expiry is `expires_at_sec`, unix SECONDS**, passed through from Bob
    verbatim (`go/cmd/agentd/embedtoken.go:62-68` states the unit). `BobChatFrame` re-mints and
  ...
  - `NewHypothesis` posts `{ title }`, expects **201**, and surfaces Bob's create error verbatim
...
- **Validation:**
  ...
  - `docker build -f web/Dockerfile --build-arg VITE_BOB_PUBLIC_URL=http://localhost:8090
...
- **Acceptance criteria:**
  ...
  - **Status and staleness never come from Bob's delivery status.** A delivery parked at
    `awaiting_human` never clears — a known Bob wart — so status comes from the trusted
  ...
  - **Amendment actions post the pinned body:** `POST /api/hypotheses/:id/amend` with
    ...
    Bob memory id of the `kind=spec-amendment` row, carried by the detail response as
...
- **Acceptance criteria:**
  ...
  - **Three defects in `api/src/bob/client.ts` that earlier tickets each found and each declined
    ...
    1. **`classifyStatus` has no case for 401**, so a Bob 401 becomes kind `internal` with the
...
- Notes: **Implemented and independently verified 2026-08-21 — 16 of 16 criteria PASS, zero
  ...
  **The three standing `client.ts` defects are closed**: 401 now classifies as `forbidden` (status
  ...
  `unavailable` at status **503** with Bob's message verbatim and the upstream 403 kept in
...
POST /api/hypotheses/:id/report-amendment    { amendment_id, decision, rationale }
       ...
       → 200                                 REJECTING writes NO template — only the decision
                                             row. Added 2026-08-26: the block documented only
                                             the accept path while printing `decision` in the
                                             body, so W21 had to invent this and the 404 below
       → 404                                 no amendment with that id for this hypothesis
       → 409                                 that proposal ALREADY has a decision. A decision
                                             row carries `amendment: <memory-id>` as a LABEL,
                                             so "already decided" is a query, not a guess —
                                             without it a replayed accept silently REVERTS the
                                             locked template past a review that superseded it
                                             (R176)
       → 422                                 the proposed template failed validation, and
                                             NO template is written — not even a decision row,
                                             because a validator rejection is not a human's
                                             decision
```
...
- **`drift` is `null` when no `kind=report` memory exists at all** — the empty state, which is
  **not** drift and must stay distinguishable from `{orphan_slots: [], unfilled_slots: []}` (a tick
  ...
  🔴 **Since R185, `drift: null` means the empty state and NOTHING ELSE** — read it with
  `unreadable` beside it: `{drift: null, unreadable: false}` is "no tick has run yet",
  `{drift: null, unreadable: true}` is "a tick ran and its report cannot be read".
- 🔴 **`unreadable` was ADDED 2026-08-26 (R185), and it exists because `drift: null` was carrying
  two meanings.** A `kind=report` body that is not a flat `{slotId: html}` map makes `readLatestReport`
  throw `invalid` (W15, deliberate, and right for the frame route). The detail route cannot let that
  400 the page — it is the page carrying the verdict buttons, and model-authored content must never
  be able to take the human's controls away — so it catches and degrades. But the degraded shape was
  **indistinguishable on the wire from "no tick has run yet"**, and W22's verifier proved that the
  degrade also **discarded tamper already witnessed on the other rows**, so a cross-hypothesis
  forgery went silent on the detail page while the board still named it. `unreadable: true` is the
  third state: `true` when the report body did not parse **or** a stored template no longer
  validates; `false` otherwise. 🔴 **Tamper survives the degrade** — that is the blocking half, and
  `unreadable` is what stops the empty state from absorbing it.
...
`docs/19-embedding.md` records as hazard **H3** in Bob's own UI — so `allow-same-origin` must
...
### W21: Report routes   [Status: done | Model: opus]
...
- **Depends on:** W19, W8, W11 *(~~W20~~ removed 2026-08-26: `routes/report.ts` imports nothing from `drift.ts` and computes no drift — that is W22's.)*
- [x] done — verified 2026-08-26, **three fix rounds plus an addendum**. Merged to agent-wolf `main` as `58e5eed`; main gated green after (api **36 files / 1649 tests**, web 31 / 347, typechecks clean, `yarn build` clean).
- Notes: Implemented `3f6c38e` → `85dcc1f` → `fd352a2` → `f85ec6e` → `4f984b1`. **Five files, 3416 insertions.** The most defect-dense ticket in the project, and **most of what it found was not in its own code**: three live defects in **merged** code (`createErrorHandler` echoing an `internal` error's message AND `details`; `express.json()`'s 100kb default making `WOLF_REPORT_MAX_BYTES` unreachable through an opaque 500; `entity.too.large` classified as our bug), **three defects created by orchestrator rulings** (**R176** replay, **R177** page sizing, **R179** selector injection — all three from R167, none found by review), **two errors in newly-authored plan text** (the 422 was unreachable — **R166**; `rationale` had nowhere durable to go — **R167**), and **five defects in test infrastructure** (**R178**/**R180**: the fake ignoring `limit`, `include_retracted` and `?version=`; **R181**'s wrong-reason test; **R184**'s fourteen-fakes class). 🔴 **The last blocking defect is the one to remember: a whitespace-only `rationale` on ACCEPT locked a template and then answered 400** — `z.string().min(1)` accepts `"   "`, and R171's template-first ordering put the deep guard after the durable write. Closed as a **class** (**R183**): the verifier fuzzed the whole write window, **31 rationale shapes × 15 amendment-id shapes, 46 cases, zero torn writes**, and showed the revert tears **ten** distinct inputs where the shipped test covers one. **Two deviations approved, both better than the instruction**: `composeReportStats` withholds `html` because the document is safe only inside the sandboxed frame (**R172**), and the amendment lookup is `getMemoryById` rather than a paged scan, because `GET /agent/memories` has **no `offset`** and a cursor walk over second-granularity timestamps on millisecond rows drops rows silently. **Totals: 79 valid implementer mutations across five sweeps + the verifier's own re-derivations, 0 surviving, 3 implementer reds and 4 verifier reds rejected as unattributable.** Both agents measured their own error rates — **~1 in 20 mutations invalid on first attempt, ~1 test in 20 passing for a reason other than the one it states** (**R182**). 🔴 **Open, not blocking:** `series.test.ts` and `poller.test.ts` have their own fakes that still ignore `?version=`, and ten more are unmeasured — **R184** is the wave-sized consolidation that answers it.
- Notes:

### W22: Board integration and cross-hypothesis defence   [Status: done | Model: opus]
...
- **Repo:** agent-wolf
- **Files:** modify `api/src/hypothesis/store.ts`, `api/src/routes/hypotheses.ts`, and their tests. **Also modify `api/src/app.ts`** — one line passing W21's `composeReportStats` into the hypotheses router (added 2026-08-26; see the `api/src/app.ts` ownership row).
- **Acceptance criteria:**
  ...
  - `headline` comes from line 1 of the `kind=report` snippet; a hypothesis with no report yet gets
    ...
    *(Clarified 2026-08-26, R186: the "never `""`" is scoped to the **no-report** case. `""` is a
    legal and meaningful value when a report exists whose line 1 was blank — `web/src/api/types.ts:104`,
    which W13 authored, says exactly that, and W22 implements both. W22's executor read this sentence
    as forbidding `""` outright and flagged the apparent conflict; the trailing clause carries the
    meaning, and it should not have needed a careful reader to find it.)*
...
- **Depends on:** W21
- [x] done
- Notes: **Wave 14, 2026-08-26 — PASSED, two fix rounds, one BLOCKING defect. Merged as `a155c96`.** api 36 files / **1696** tests (from 1649), web 31 / 347, both typechecks and the web build clean on `main`. Six files: `hypothesis/store.ts`, `routes/hypotheses.ts`, `app.ts` and their test twins, +2255/−68.
  🔴 **The blocking defect is R185 and it is the wave's whole lesson: a graceful degradation swallowed the security signal it was standing next to.** `readLatestReport` computes `picked.tamper` before it parses the winning row; `parseReportContent` throws `invalid`; the catch returned `{report: null, tamper: []}` — discarding anomalies witnessed on **other** rows. Result, proved end-to-end through the real `createApp` by the verifier and reproduced by it again after the fix: a cross-hypothesis forgery named on the board and **silent on the detail page**, which is the page carrying the verdict buttons. The two surfaces now carry byte-identical `tamper` arrays. `report.unreadable` was added to the pinned block in the same stroke (**R192**(1) records why it stays one boolean rather than splitting its two causes).
  **Verification was the strongest this project has run.** Round 1: 59 mutations, 57 gated, all four harness rules, opening control red and three closing controls green, final manifest diff `[]`. It re-derived the implementer's two headline claims independently and got 6 and 76 kills against the claimed 5 and 73 — then **attributed the gaps** rather than reconciling them (the sixth kill is a test W22 itself added). Its own invalid-mutation rate was 2/59, holding **R182**'s ~1-in-20 across a different agent on different code. Round 2: 34 mutations, zero regressions across a 16-mutation slice, and the two families most at risk — cross-hypothesis and hostile-retraction — **grew hardest** (M57 18→24, M1 16→22, M8 10→13): both read paths changed shape without one assertion loosening.
  **Three guards were decoration and one was not — that distinction is R191, the wave's other keeper.** S1 (the follow-up read's `include_retracted`), S2 (the error narrowing) and N1 (the headline `.trim()`) were genuinely untested and now die. `mergeTamper`'s dedup was **unreachable**, not untested: a retraction carries a single `retracts=<id>` label and both production call sites join reads over disjoint kinds. Kept, exported, unit-graded with both near-misses, and each call site now names its pairing — because reachability there is a property of **Bob's** data model, which this function can neither see nor enforce.
  **Four things the implementer got right that were not asked of it.** It read `app.ts` properly and found the "one line" was a **hoist** — `createReportRouter` constructed at `:239`, after the hypotheses router at `:207` — and moved the construction while leaving `app.use(report.router)` exactly where it was (verified against `main`). It chose `stripped_count: null` over `0` for "nobody counted", because `0` is a real answer and reporting a stripped XSS attempt as clean is the failure that avoids. It caught **its own** wrong-reason test by mutation (guards unreachable through `reportOwnerFor`, rewritten to pin a degenerate owner) — R182's rate again, on the nose. And it **fixed three permissive fakes** and measured the difference: deleting `user_email: "*"` went from killing **zero** tests to killing 76. That is R184's mechanism run forwards for the first time — decoration converted into defence, and the conversion measured.
  **Rulings on record:** the go-live gate's 422 wins over spec errors when both halves fail (a spec pre-validation in the route would duplicate W9's validator, and both are refusals that write nothing) — the `draftStub()` change this forced was enumerated across all ten call sites and weakened nothing, proved by M22 killing only the two template tests and never the spec ones. The frame route's 400 body gaining a `tamper` key is accepted, **R192**(2). Board tamper is widened where `readEvaluationSummaries` deliberately only logs, and that is right *here* because the detail page reports the identical anomaly — which is exactly the claim B1 falsified before the fix, and which now holds.
  **Left open, all logged, none W22's to fix:** **R188** — `kind=report-candidate` has **no reader anywhere**, so half this ticket's own isolation criterion is vacuous as written. **R189** — the detail route makes 19–20 Bob reads where ~11 would do, because `composeFor` reads before consulting its own cache; the fix touches W21's interface. **R190** — `BOARD_LIMIT = 100` is decoration because both fakes ignore `limit`, so the board silently truncates past 100 hypotheses while the detail page does not. **R187** — the per-name follow-up read is `O(attacked)` and the attacker picks N; W27's restated budget was amended for it. And one note: a **live, template-less** hypothesis now answers `422 "author one before going live"` where it used to answer `409 "cannot go live from live"` — no HTTP test covers the 409 path, so nothing broke, but any hypothesis that went live before W22 gets a misleading refusal.
- Notes: 🔴 **Hand-off from W21 (2026-08-26) — read before you write a line.** (1) **`stripped_count` has exactly ONE producer and you must not become a second.** It comes out of `composeFrame`, which sits behind W21's frame cache. W21 exports **`composeReportStats(id, sessions?)`** from `createReportRouter`, bound to that same router instance and therefore that same cache — call it. A second `sanitiseSlot` pass would produce a second number that can disagree with the document actually served, and the two agree almost always, which is what makes it dangerous rather than obvious. Pass the session index your board already holds. (2) 🔴 **`composeReportStats` deliberately does NOT return `html`, and you must not widen it.** The composed document is safe only inside the sandboxed frame the CSP header applies to; a caller holding the bytes could put them in a JSON payload the SPA renders with **no sandbox, no `frame-ancestors` and no opaque origin**. The bytes leave this process through `GET …/report/frame` and nowhere else. That is a security boundary, not a payload-size preference — ruled 2026-08-26. (3) **Your criterion about a forged `report-template` serving 404 is ALREADY DONE and tested** by W21 (mutation-proved against a store that trusts any row). Do not rebuild it; only the tamper-surfacing half on the detail payload remains. (4) A decision on a report amendment now appends a **second `kind=report-amendment` row** with `status: accepted|rejected` — so a selector on that kind returns proposals AND decisions, and only `status=proposed` is decidable. (5) The frame route pays for its own session-index read per request; `readTemplate`/`readLatestReport` both accept a `sessions` option, so a page fetching detail and frame together can share one.

### W23: VerdictBand and ReportPanel   [Status: done | Model: sonnet — run on opus]
...
- **Files:** create `web/src/components/{VerdictBand,ReportPanel,ReportDrift}.tsx` and tests;
  modify `web/src/pages/HypothesisDetail.tsx`, **`web/src/api/types.ts`**, `web/src/pages/HypothesisDetail.test.tsx`.
  *(Corrected 2026-08-26 at the wave-15 report, **R195**. This line named `web/package.json`, which W23
  must NOT touch — the testing library was already installed by W13 and the jest-dom pin is exact,
  R135 — and it OMITTED `web/src/api/types.ts`, without which the ticket cannot be completed at all
  because `ReportBlock` did not exist on the web side. Wrong in both directions in one line.)*
- **Acceptance criteria:**
...
- **Depends on:** W22, W14
- [x] done
- Notes: **Wave 15, 2026-08-26 — PASSED, three fix rounds, ZERO blocking defects. Merged as `6da986c` + `574f87c`.** web 34 files / **416** tests (from 347), api 36 / 1696, both typechecks and the web build clean on `main`. Nine files; `web/package.json` and `yarn.lock` untouched, jest-dom still pinned exactly `6.9.1`.
  **Verified across 61 mutations with all four harness rules, and the highest-value test is not vacuous.** Deleting the `sandbox` attribute *entirely* still kills 3, because of an `expect(sandbox).not.toBeNull()` guard — so the negative assertion cannot pass on an absent attribute. The exact-token-list assertion catches **any** added token, not only `allow-same-origin`. The `dangerouslySetInnerHTML` scan's self-exclusion is exactly one resolved path: a needle planted on a *different line of the same file* still kills, and forcing the file walker to return `[]` kills too, so the scan reads real files rather than a stale list. **R185's hiding shape does not come back**: moving the `unreadable` check below the empty states — the exact defect W22's verifier found — kills 3 at two levels.
  🔴 **Every defect this ticket produced was one shape: a check that cannot fail on its own, and three fix rounds were needed to find them all.** Round 1 closed two self-referential sentence constants — one of which, emptied, rendered a blank panel and defeated criterion 3's own "never a blank frame". Round 2 swept all three components for assertion-list divergence and closed **five more**, four of which had the thinner half written second. Round 3 closed the positive control on the second listener test. See **R197** for what the three rounds have in common and **R198** for the rule the executor derived from its own third instance.
  **Two decisions were left to the executor deliberately and both were ruled correct.** `stripped_count: null` renders as a plain sentence on the always-on provenance channel, **not** `Severity` and **not** silence — because `null` is a property of how the deployment is wired, constant across every report, and a permanent warning triangle is exactly what UI § 2's opening paragraph exists to prevent; a real `degraded` would lose its force. And `unreadable: true` **withholds the frame**, which the verifier confirmed independently against the API: `composeFor` returns `null` when `template === null` → `not_found`; `readLatestReport` at `report.ts:516` is uncaught; `parseStoredTemplate` rethrows as `internal`. All three states that set the flag make `GET …/report/frame` **error**, so a mounted frame would render the API's error body inside the report panel — a worse lie than an honest empty state.
  **The `has_template: false` + populated `drift` state is real and was found by mutation, not by reading.** A mutation killed one test where the executor expected two; rather than banking it, it worked out that every fixture with no template also had no drift, so a second gate was masking the first. `reportBlockFor` sets `declared = []` when `template === null` and `detectDrift([], reportSlots)` makes every filled slot an orphan, so the state is emitted and the frame route 404s for it. Verifier's own mutation then kills 2. **A mutation that kills fewer tests than you expected is a finding, not a pass.**
  **Ruled and closed:** the `Provenance kind` deviation (`"model"` only when `has_template === true`, so the empty states sit on no ground) is **accepted** — an empty state has no author, and stamping one would be a claim about content that does not exist. The two co-firing tests are **over-broad assertions, not wrong-reason tests**: they *fail* for extra reasons, which is the harmless direction, and their neighbours were diffed with no third case found.
  **Left open, logged, none W23's:** 🔴 **R199** — the report block carries no writer, so `Provenance` falls back to `UNKNOWN_WRITER` and every healthy report on the detail page reads "unknown writer"; UI § 2's signal table is unsatisfied in substance and the fix is one field on the API side. Also **R200** — `stubFetchRoutes` lets a test declare a route it never requests, and its "fails loudly" claim is weaker than it reads, because the throw is caught twice on the way up and stays loud only because every page test asserts positive content; an absence-only test would pass against a page that never loaded.
- Notes: 🔴 **Hand-off from W22 (2026-08-26) — the report block gained a THIRD state on each of two fields, and your criteria as written fall through both.** (1) **`stripped_count` is `number | null`, not `number`.** `null` means *nobody counted* — W22 does not call `composeReportStats` at all when there is no locked template, and emits `null` rather than `0` when the producer is unwired, because `0` is a real answer ("the sanitiser removed nothing") and reporting a stripped XSS attempt as clean is the failure this avoids. Your criterion says "`stripped_count > 0` renders as degraded"; in JavaScript `null > 0` is `false`, so today the third state silently renders as healthy. **Decide what `null` shows and test it** — the honest rendering is neither the degraded notice nor the clean state, and `Severity` refuses to render without a cause sentence, so whatever you choose needs words. (2) **`report.unreadable: boolean` is new** (R185, pinned in § "The detail route's report block, pinned"). `drift: null` no longer means only "no tick yet" — read `unreadable` beside it: `{drift: null, unreadable: false}` is the empty state your criterion already covers; `{drift: null, unreadable: true}` is "a tick ran and its report cannot be read", which is a **degraded** condition with its own cause sentence, not an empty state. Rendering the second as the first is what W22's verifier proved can hide a cross-hypothesis attack. (3) The `tamper` inside the report block survives that degrade, so a report block can carry `unreadable: true` **and** a populated `tamper` at once — do not let one branch swallow the other.

- Notes: 🔴 **Hand-off from W21 (2026-08-26): the composed report document must never reach you as data.** It leaves the API only through `GET …/report/frame`, and `ReportPanel` renders that URL in an iframe carrying `sandbox="allow-scripts"` with **no `allow-same-origin`**. Do not request the HTML as JSON, do not accept it as a prop, and do not ask for `composeReportStats` to be widened to return it — the document is safe only inside the frame the CSP header applies to, and rendering those bytes in the SPA gives them no sandbox, no `frame-ancestors` and no opaque origin. The `stripped_count` you gate the degraded notice on comes from the detail payload (W22), whose **sign is the contract and whose magnitude is not**.
- Notes (original): **Hand-off from W14 (2026-08-24), read before starting.** (1) 🔴 **`ReportPanel` is `ReportFrameHost`'s CHILD, not a peer.** W14 ships the host, which owns `height: clamp(480px, 70vh, 900px)`, internal scroll, the expand dialog and the proof that a `postMessage` height is ignored — the ordering deadlock between the two tickets was resolved that way (both tickets previously said the other owned the height rule). W23 owns `sandbox="allow-scripts"` with **no `allow-same-origin`**, still the highest-value test in the feature. The host mounts its child in **exactly one place at a time**, so you get one live frame; `HypothesisDetail.tsx` marks the seam with two comments and a `report-placeholder` `<Typography>` that W23 deletes. (2) 🔴 **`VerdictActions` (W14) is a different component from `VerdictBand` (W23)** — compose the band above it; do not fold the buttons in. (2b) 🔴 **`stripped_count`'s SIGN is the contract; its magnitude is not** — it counts DOMPurify *records*, so a library upgrade moves the number with no behaviour change. Render it magnitude-agnostically ("content was removed from this report (3 items)", never "3 items removed" as though it were an inventory). **R149**. (3) 🔴 **Import `format.ts` and `reasons.ts`** rather than rolling a second date helper or a second gloss table. (4) `Provenance`/`Severity` come from `web/src/components/trust/` — both **default** exports; `Severity` **throws in dev without a non-empty cause**, so `stripped_count` and drift each need their sentence. (5) `ConditionTable` takes a `testId` prop and **two instances already coexist** on the page (`condition-table` and `case-conditions`) — do not assume one. (6) The provenance stamp for the report panel is the **caller's** wrapper: the host does not know the report's writer. (7) `ReportFrameHost` renders in **every** lifecycle state including `draft`, so its empty state — which W23 owns — is what a pre-go-live hypothesis shows. (8) 🔴 **Stub `window.matchMedia`** for any responsive assertion (**R139**), and remember `testUtils.tsx`'s `stubFetchRoutes` **throws on unrouted paths**. (9) The testing library is already installed — W23's "adds `@testing-library/react`" criterion is a **no-op**; do not add a second copy or change the exact `6.9.1` jest-dom pin (**R135**).

### W24: Go Live review screen   [Status: done | Model: sonnet — run on opus]
...
- **Files:** create `web/src/pages/GoLiveReview.tsx` and its test; modify `web/src/App.tsx`,
  **`web/src/components/GoLiveButton.tsx`** (a SECOND PROP for the accepted-template half — never a
  second gate; see the ownership row), **`web/src/api/client.ts`**, **`web/src/api/types.ts`**,
  **`web/src/testUtils.tsx`**; and create the read this screen needs in
  **`api/src/routes/report.ts`** + **`api/src/routes/report.test.ts`**.
  *(Corrected 2026-08-26 at the wave-16 cut, **R206**. The line named two files and the ticket cannot
  be done in two: `GoLiveButton` is on the ownership row for this ticket and was absent here — the
  R195 shape again — and the first acceptance criterion was **unbuildable as written**, see the new
  criterion below.)*
- 🔴 **NEW CRITERION, added 2026-08-26 (R206) — the candidate read does not exist and this ticket
  builds it.** The screen's first criterion says it "reads the newest `kind=report-candidate`", and
  **nothing in the tree can**: `grep -rn "report-candidate" api/src` returns only the constant and
  its label builder in `report/kinds.ts`, and `web/src/api/client.ts` has no method. **This closes
  R188's open question** — that entry asked whether a reader was missing from an earlier ticket or
  whether W22's criterion should have said `kind=report` only; the answer is the former, and **W24 is
  the reader**. Add `GET /api/hypotheses/:id/report-candidate` to **`api/src/routes/report.ts`**, not
  to `routes/hypotheses.ts` — it is a report-layer read, `createReportRouter` already owns the other
  three, and `routes/hypotheses.ts` is W27's file this wave. It returns the newest candidate's HTML
  plus the `script_srcs` / `remote_origins` this screen must list, or a documented empty state.
  🔴 **Reuse W15's store reads and the same trust rule the other report kinds get** — a
  `report-candidate` whose provenance does not name this hypothesis's own researcher or its
  `hyp-<id>` session is a cross-hypothesis write (W22's defence, `isOwnReport`), and a screen that
  renders a forged candidate for a human to approve is the worst possible place to skip it.
- **Acceptance criteria:**
...
- **Depends on:** W23, W13
- [x] done
- Notes: **Wave 16, 2026-08-26 — PASSED, one fix round, ZERO blocking. Merged as `ba167fb`.** api 36 files / **1732** tests (from 1696), web 35 / **462** (from 416, +1 file), both typechecks and the web build clean. Eleven files, +2721/−13. **74 mutations attempted by the verifier, 69 valid, 59 killed** — invalid rate 6.8%, R182's ~1-in-20 holding on a seventh agent.
  🔴 **THE TICKET GREW AT THE WAVE CUT AND THAT IS WHY IT WORKED (R206).** Its first criterion said the screen "reads the newest `kind=report-candidate`" and **nothing in the tree could** — the kind had been a transport with a writer and no reader since W15, nine tickets, and the only thing that ever noticed was a consumer trying to consume it. **W24 built the read**, in `api/src/routes/report.ts` (not `routes/hypotheses.ts`, which was another ticket's file that wave), 🔴 **carrying W22's `isOwnReport` cross-hypothesis rule — a screen rendering a forged candidate for a human to approve is the worst place in the product to skip it.** Deleting the check kills 3; inverting it kills 20. It reuses the **shared exported primitives** and re-implements only `pickSurvivingRow`'s 12-line walk, which is private to `createHypothesisStore`; the verifier diffed that copy against `store.ts:1528-1555` **line by line** and found it faithful.
  **The three lists above the preview are the ticket's real content, and the labelling is the security property.** `script_srcs` renders as *"Remote code this template references"* and **never** as "permitted script origins" — W19's CSP maps only the **`https:`** entries, and `scriptSrcs` is not https-only (a CSS `@import url(data:…)` puts a `data:` URL in it, **R155**), so that labelling **would lie to the human approving it**. Substituting the raw list for `codeOrigins()` kills 3. 🔴 **The verifier grepped every added line for a second URL→origin derivation — `new URL`, `.origin`, `hostname`, `protocol` appear only inside comments. There is no third mapping.** "Everything else" is the set difference, routinely **empty**, and the empty case renders a sentence with `severity` asserted null — explicitly not an error. The caveat says the list is *"an inventory, not a guarantee"* (R173), and rewording it into a completeness promise dies.
  **An extra preview-frame route was built that the amendment did not name, and it was judged NECESSARY.** The criterion demands the **real CSP header**, and a header only applies to a document the browser fetched; `GET …/report/frame` reads the **locked** template, which by definition does not exist at review time. `srcdoc` carries no policy, and a `<meta http-equiv>` copy silently ignores `sandbox` and `frame-ancestors` — the two directives that make the document safe. **The criterion could not be met without it.** It is not a duplicate: it shares `composeFrame`, the same `send()` header helper and `requireKnownHypothesis`, differing only in the source row and `slots: {}` — and the empty-slots rule is sentinel-tested, so splicing yesterday's report in dies.
  🔴 **The fix round's defect is R220 and the way it was found is the entry: THE VERIFIER'S OPENING CONTROL WENT GREEN, AND THAT WAS THE FINDING.** Two of the three list headings — `CODE_ORIGINS_HEADING` and `OTHER_ORIGINS_HEADING` — had **no assertion anywhere in the repo**. Measured: renaming "everything else" to *"Origins allowed to execute code"* left **457 tests green**, so the screen would show the **non-executable** host list under the **executable** label, defeating the one distinction the screen exists to draw. Every *value* was pinned; only **the words telling the human which list is which** were unguarded. **Fourth consecutive ticket with the R182/R198 shape.** The fix pins all three as literals **and** adds a test that the three headings are three *different* sentences — which catches two headings converging, a case pinning each separately would not. Second fix item: the preview route's trust rule had **no test of its own** (giving `readCandidate` a `trust` flag and passing `false` from only the frame route left the suite green at 1723) — closed with a forged-candidate test, plus the two notes on `created_at_ms` and the non-discriminating provenance fixture.
  **Boundaries all held.** `store.ts` untouched (W32's that wave). `report/frame.ts` changed by **8 comment lines plus one `export`** — the W19 precedent, and necessary, since the alternative was the third mapping criterion 3 forbids. `ReportPanel`/`ReportFrameHost` neither edited nor re-implemented; `REPORT_SANDBOX` **imported**, so `allow-same-origin` cannot arrive through a second spelling. **W24 mints no embed token**; the only occurrence in the diff asserts a sentinel token does *not* leak into the frame, and the two new log lines carry ids, booleans and counts only.
  **Left for W29 (R219):** the Go Live gate's template half is wired on this screen and **not** on the detail page, whose comment already claims otherwise. W24 was right not to reach into another ticket's file; W22's server-side `422` is the backstop.
...
- **Depends on:** W17, W12
- [x] done — verified 2026-08-26, one fix round. Merged to agent-wolf `main` as `7e1a1d6`; main gated green after (api **35 files / 1573 tests**, typecheck clean). No new dependency.
- Notes: Implemented `953e3ea`, fix round `712f7f8`. **Five files, 1178 insertions, ZERO deletions — no existing test was modified**, which is what proves nothing was relaxed to accommodate the prompt edits. Its verifier measured **every contract claim against the code by execution rather than reading, and refuted none**, then ran 30 mutations in round 1 and 34 in round 2. 🔴 **The worked example was EXECUTED in jsdom across four states** and the mandatory-fallback criterion — the one most likely to be prose that was never run — is genuinely demonstrated: scripts-off leaves it PRESENT, a drawn chart REMOVES it (proved by a real SVG path `d` and a caption naming 5 observations), day-one with no points REMOVES it with "No observations yet", and a draw that fails halfway KEEPS it with the table still filled. The example is **fully self-contained** — `scriptSrcs: []`, `remoteOrigins: []` — so it also demonstrates § 6b's common case, `script-src 'unsafe-inline'` with no host at all. **One real gap in round 1: an acceptance criterion with NO assertion behind it** — deleting the `**Line 1**: the headline` bullet left all 1562 tests green while its two siblings in the same criterion were pinned (R133 reached from the other end). Closed, plus five discriminating prose guards, each reddening **alone** with a message naming which rule went missing; `bulletAt()` scopes the raw-text guard to one bullet because a whole-document match would still pass with the rule deleted (`<style>` also appears in the allow list further down) — the R146 failure mode caught by the implementer before the verifier could find it. The two `SLOT_PROFILE` list guards are **two-way**: the prose can no longer drift from the constant in either direction. **Findings recorded, not fixed here:** **R173** (SVG `fill`/`filter` are an unscanned URL channel — W16/W30's walker, and W25 correctly narrowed its prose instead of claiming a rule that does not exist) and **R174** (`api/` depends at test time on an undeclared `jsdom`; the comment now says "That is a BYPASS, not a clearance").
...
### W26: End-to-end verification   [Status: done | Model: opus]
...
- [x] done
- Notes: Merged at `8ebbbda` (merge commit `5ce12d4`), bringing `fix-runsh-sigpipe` (`1c70c09`)
  with it. **Files: FOUR, not the one this ticket's Files line names** — `report-layer.spec.ts`,
  `helpers/x1.ts` (R249's `UnretryableError` fix), `mock/build-script.py` and its generated
  `mock/script.json`; plus `run.sh` via the merge. See **R253** — I made that line stale at dispatch.
  **Seven legs, each proved able to fail by its own mutation** (7/7, every one exit 1, six distinct
  legs; `sandbox-attr` lands on leg 3 because `reportFrame()` guards the attribute first).
  🔴 **The direct-navigation leg is proved NON-DUPLICATE**: delete `"sandbox allow-scripts"`
  from `frameCsp` and leg 4 stays GREEN while leg 5 goes RED, same run — and the verifier stripped
  leg 5's header assertions to check it fails for the right reason, finding the directly-navigated
  document genuinely reaches Wolf's own origin (`http://localhost:8091`, not `"null"`) and genuinely
  gains origin-keyed `localStorage` (`GRANTED`, not `denied: SecurityError`). A real privilege
  escalation the CSP really prevents. Validated by the orchestrator on merged `main`: full suite
  **27 passed / exit 0** with `Running 27 tests` shown, api **37 files / 1834 tests**, web
  **36 / 479**, `go build`+`vet`+`test` clean (*Go live-Postgres cases skip without
  `AGENTKIT_TEST_POSTGRES_URL` — that green does not cover them; W26 touches no Go*). The tamper
  mutation was re-run by the orchestrator after the implementer was stopped mid-proof: see **R256**.

---

### X1: End-to-end verification   [Status: done | Model: opus]
...
- **Repo:** agent-wolf (with agent-bob running alongside)
- **Files:** create `e2e/run.sh`, `e2e/playwright.config.ts`,
  ...
  `e2e/features/tamper-resistance.spec.ts`, `e2e/mock/script.json`, `e2e/bob-override.yml`.
  **No file in agent-bob is edited by this ticket** — the mock script reaches agentd through
  the override file, not through an edit to Bob's `docker-compose.yml`.
...
- **Acceptance criteria:**
  - **Runs offline against the mock model** — no `ANTHROPIC_API_KEY`, no `CLAUDE_CODE_OAUTH_TOKEN`,
    ...
    🔴 **CORRECTED 2026-08-27 (R234) — the string this criterion originally pinned CAN NEVER APPEAR
    in an X1 run, and a `run.sh` written to it fails every time.** `go/cmd/agentd/modelproxy.go:58-63`
    branches on whether a mock **script table** was configured: `table == nil` prints
    `MOCK model proxy (set it for a real agent)` (`:59`), otherwise it prints
    `SCRIPTED mock model proxy (N rule(s))` (`:62`). **They are exclusive, and X1 always configures a
    table.** Assert the **SCRIPTED** form, and separately assert that **neither** `real model proxy →`
    **nor** `subscription mode →` appears — a positive plus two negatives, because the positive alone
    would pass on a boot line that changed wording. The original text, now wrong, pinned
    `[agentd] ANTHROPIC_API_KEY unset → MOCK model proxy (set it for a real agent)` present in
  ...
  - **Ports are pinned explicitly for both stacks, because the defaults collide.** `run.sh` starts
    Bob with `WEB_PORT=8090` and Wolf with `WOLF_WEB_PORT=8091`, and builds `wolf-web` with
    `VITE_BOB_PUBLIC_URL=http://localhost:8090`. Wolf's default 8081 is exactly the port
    `README-stack.md`'s known-good mock invocation gives Bob, and `docker compose up` then fails
    on port allocation.
  ...
  - `e2e/bob-override.yml` is passed to Bob's compose with `-f` and bind-mounts
...
- **Validation — all five must pass:**
  - `cd go && go build ./... && go vet ./... && go test ./...`   *(agent-bob; note this
...
- [x] done
- Notes: Merged at `f009f58` (merge commit `3d29dd5`). Verified by a FRESH verifier that
  re-ran the validation itself: two back-to-back 17/17 runs, exit 0, on a board seeded with 5
  hypotheses of which **3 carried the lifecycle spec's exact title** — the precise condition that
  broke the old locator. **Both claimed fixes were broken by mutation and observed failing**: the
  id-keyed locator reverted to `hasText` produced a strict-mode violation resolving to 4 elements
  (exit 1), and `LIST_LIMIT` lowered to 1 fired `assertNotTruncated`'s named message against a real
  listing (exit 1). Gates on merged `main`: api **37 files / 1834 tests**, web **36 / 479**,
  `go build`+`go vet` clean — all at baseline. Five defects that only running the product could find
  are recorded in R249-R252 and the entries above. 🔴 **PRE-FLIGHT, run by the orchestrator 2026-08-26 at the wave-16 mid-point (R216) — every mechanism this ticket depends on is BUILT, and here is the evidence, so you do not spend a round rediscovering it.** The reconciliation check (R206/R215) was run against your criteria and your Files line is **correct as written**: you create only `e2e/` files, and the three ownership rows that list X1 (`api/src/config.ts`, `.env.example`, `docker-compose.yml`) turn out **not** to need you, because earlier tickets already landed what your criteria ask for. Verified on `main`:
  - **`WOLF_WEB_PORT` is already honoured** — `docker-compose.yml:123` is `"${WOLF_WEB_PORT:-8081}:80"`, so `run.sh` can pin 8091 with an env var and **no compose edit**. The default really is 8081, which is why the port-collision criterion exists.
  - **`WOLF_TEST_LOGIN` is already parsed** (`api/src/config.ts:337-360`) **and the dev-login route is already built and gated** (`api/src/routes/auth.ts:159-174`), with `secretEquals` on both halves.
  - 🔴 **B6's "asserted absent from the production build by test" is ALREADY DONE** — `api/src/routes/auth.test.ts:279`, `auth_dev_login: is NOT mounted when WOLF_TEST_LOGIN is unset`. Do not rebuild it.
  - **`e2e/` does not exist yet.** You create the whole directory; there is no rig to extend, and W26 extends *yours*.
  🔴 **The one thing the pre-flight could NOT settle, and it is yours to prove first:** every mechanism above is proved *in unit tests against a fake*. This ticket is the first thing in the project that runs the real stack, so **treat each of them as unverified in a container until you have seen it work there** — that is the entire point of X1 and it is why R41 assigns the reachability proof here. A `docker compose config` proves a declaration and nothing else (**R41**: W1's nginx bug survived a pass for exactly that reason), and 🔴 **`docker compose config` prints interpolated secrets to stdout — use the `dcc()` redactor from § "Executor orientation", never the bare command.**
...
- **Acceptance criteria:**
  ...
  - 🔴 **`web/scripts/verify-package.sh` reproduces the R125 probe as a repeatable check** and CI
    ...
    dot computes to the CONSUMER's `success.main`, not Bob's** — that last assertion is the whole
    reason the package exists and is the one a refactor will silently break.
  - **Bob's own source imports are NOT changed.** The probe isolated this: rebuilding with the
...
- **Depends on:** —
- [x] done — verified by the orchestrator 2026-08-24, zero fix rounds. Merged to agent-bob `main` as `437abb1`; main gated green after (go build + vet clean, `web` **72 files / 1395 tests** up from 71 / 1381, typecheck clean, `examples/web` builds against `dist`).
...
- **Scope:** Convert Bob's seven remaining `@mui/icons-material` barrel imports to deep imports,
...
- **Acceptance criteria (all met):**
  ...
  - Bob stays green: `web` 72 files / 1395 tests, `typecheck` clean, `verify-package.sh` PASS,
...
- **Depends on:** O12
- [x] done — merged to agent-bob `main` as `e22e6d8` and re-vendored into agent-wolf `main`. Wolf `web` **42.93s → 1.18s**.
...
### W27: The attention model   [Status: done | Model: opus]
...
- **Acceptance criteria:**
  ...
  - **`attention_requests` reaches the board.** It is currently detail-only. It is **one
    ...
    🔴 **Amended 2026-08-26 (R187) — that restatement is now incomplete, and W22 landed the reason.**
    W22 issues **one further per-name read for each hypothesis whose newest `kind=report` row is a
    forgery** (`store.ts:1594`, verified). It is deliberate: without it a forged newest row erases the
    victim's real headline, which is the attack criterion 2 exists to stop. It is **not** a `latest_per`
    request — it sends no `latestPer` — so it honours W22's criterion literally, and your twelve-row
    test must be written against `latest_per` requests specifically, not against a total request count.
    State the budget as **"three `latest_per` requests, plus one project-wide attention read, plus one
    per-name read per ATTACKED hypothesis"**. 🔴 And note the cost honestly: that last term is
    `O(attacked)`, and the attacker chooses it — one forged row per hypothesis turns a 3-request board
    into `3 + N`. It is the same pattern `readBoard` already uses, so it is consistent rather than
    novel, but it is an amplification under hostile control and it belongs on the record here rather
    than being discovered by whoever first sees a slow board.
...
- **Depends on:** W22
- [x] done
- Notes: **Wave 17, 2026-08-26 — PASSED, one self-sweep round and two review rounds, ZERO blocking. Merged as `e5d4c04`.** api 36 files / **1808** tests on `main` after merge (W27 alone: 1700 → 1776), web 35 / 462, all gates green. **52 mutations by the verifier, 50 killed, both survivors deliberate controls**; all nine criteria PASS, graded clause by clause against UI § 4's table.
  **R163's gap is closed**: the `draft` branch has three fixtures plus an end-to-end board row, and two mutations kill it. **R187's amended budget is tested as amended**: `latest_per` counted *specifically* by a path filter, never a total, with the attacked case asserting per-name reads **by selector name rather than by count**. `attention` and `stale` are absent-stays-absent, pinned on the **key set** (`"attention_count" in row`) rather than `toBeUndefined()`, which also passes against an explicit `0`.
  **Two deviations from the ticket's literal wording, both ruled correct — R223.** `challenge_reason` is gated on `status === "challenged"`, which R143 did not say: a `confirmed` row's `rationale` is **the free text a human typed with their verdict**, and serving it under a name the UI renders as a *machine's* reason would put a person's words in a machine's voice, on the page whose whole job is telling those apart. And **`poller.ts` is untouched** — deriving `stale` inside `evaluationSummaryLine`, beside `tallyConditions`, keeps one source of truth for a number the board reads; the Files line implied an edit that would have been worse.
  🔴 **Review round 1 closed a defect worth more than the ticket: a failed rationale read 503'd the ENTIRE detail page** — no spec, no scoreboard, no verdict, no tamper — for `challenged` hypotheses, the one status meaning a person is needed now. Every sibling read on that handler degrades deliberately and has a test saying so, including one W27 wrote correctly in the same ticket. Now degrades to `null` at `error` level, and the test **names every other field that survives**, because a test asserting only the null would pass against a page that returned nothing.
  🔴 **Review round 2 closed R225's instance — a limit that became a defect without its line changing.** `readDetailRows` had always read `limit: 50`; harmless for eight tickets because only the newest surviving row mattered. W27 made those rows the **whole timeline**. Reported rather than raised, and the reasoning is the right one: raising it moves the cliff without removing it, and the read is shared with a caller that needs only the newest row. Documented in four places, so the *meaning* change is written down where R190's was not. **Truncation is measured off the rows READ, never the survivors** — because Wolf rejects forged rows *after* the read, so a padding attack that pushes real history off the end would, on a survivor-counting implementation, **also hide that it did**. And it **over-reports at the exact boundary** deliberately: a full page is not proof there is no next one, and claiming a complete timeline we cannot verify is the worse error on a page a human decides from.
  🔴 **THE FINDING OF THE TICKET (R227): a test whose TITLE named a distinction its BODY never exercised.** The test was called *"truncation is reported off the ROWS READ, not off the rows that survived"* — and the survivor-counting mutation **survived it**, because every fixture had survivors equal to rows read, so both implementations agreed on every case that existed. **Invisible to every reading; findable only by mutation.** Rebuilt on the discriminating case, which is the attack itself. **Three review rounds each found something the sweep over *existing* tests could not, and the defect got subtler each time** — a missing assertion, then a test asserting half its title, then a test whose title named an unexercised distinction.
  **R226 confirmed in place**: the first of three sibling cases was the thin one, and the executor wrote *why it missed it* into the comment — the rule as stated ("the thinner half is written second") is what walked its diff past it. **A group sweep must compare every member against every other.** The same corrected sweep then found a live instance of the count-assertion trap: `toHaveLength(10)` is satisfied by **the wrong ten**, which is exactly what a padding attack produces. Survivors are now named by memory id at both ends.
  **Left open, none W27's:** **R224** (a `live` row with no trusted state row tiers `holding`, which is collapsed by default); **R190**, sharpened — W27 adds a *second* consumer of the truncated board set, so past 100 hypotheses a row needing a human vanishes from the board **and from its tier count**, which is a different severity from a missing row; and the `web/` half of `restated_from` (the server half shipped; `types.ts` does not declare it and `Archive.tsx` still pays a detail read per terminal row).
...
- **Acceptance criteria:**
  - `@agentkit/chat-ui` is installed from a **local tarball** under `web/vendor/`, referenced as
    ...
    repository** — O12 leaves it at `agent-bob/web/agentkit-chat-ui-<version>.tgz` via
    `npm run build && npm pack`, and this ticket copies that file into `agent-wolf/web/vendor/` and
    commits it — the tarball is **gitignored in agent-bob**, so it is not in a fresh clone. An
  ...
  - `web/src/theme.ts` implements `design/2026-08-24-agent-wolf-ui.md` § 2b in full: the light and
    ...
    Bob rail follows the OS and cannot be told otherwise, so Wolf follows the same rule or the two
...
- Notes: Implemented `1061764`, gaps closed in `72c8d2d`. Its verifier ran **29 mutations and caught 26**, each checked in **both light and dark** — the provenance ground set to `warning.main`, `error.main` and `info.main` in turn; `error` given to `tripped`, `holding`, `indeterminate` and `degraded` in turn; a default `cause` on `<Severity>`; a `<Box>` around `kind="machine"`; the glyph removed from `degraded` and from `attacked` with colour left alone; `deps.inline: []`; `spacing`, `MuiTableCell` and `tabular-nums`. It also checked the easy fake on the production path — forcing `isProductionBuild()` to `false` **fails** the two production tests, so they genuinely execute that branch — and confirmed the tier-2 import resolves to `node_modules/@agentkit/chat-ui` rather than a relative path, with the `live` dot taking **Wolf's** `success.main`. **Three gaps found and closed before merge:** (1) the MUI `Alert` `severity` prop was unasserted — flipping `error`→`warning` changed the colour and the a11y role with nothing going red; now asserted on `MuiAlert-colorError` + `role="alert"` and proved to fail in both modes, which matters because four tickets consume this component; (2) the `web/package.json` R134 note described a repro that does not reproduce (see R134, draft 3); (3) `package.json`'s description still read *"imports nothing from agent-bob"*, which revision 5 reversed. **The implementer logged 29 guesses** — the largest single deliverable of this wave — of which two are load-bearing for later tickets: `web/src/setupTests.ts` exists because of **R135**, and the glyph maps plus `conditionColor()` live in `theme.ts` so W14 and W23 cannot invent their own. Scope was clean: `web/src` holds 12 files, zero pages, zero router, `App.tsx` untouched.

### W29: Wolf's artifact surface   [Status: done | Model: sonnet — run on opus]
- **Scope:** Read a hypothesis session's artifacts and render them with Bob's own panel — the
  concrete case that motivated revision 5's tier decision.
- **Repo:** agent-wolf
- **Files:** modify `api/src/bob/client.ts`, `api/src/bob/types.ts`, `api/src/app.ts`; create
  ...
  **`web/src/api/client.ts`** and **`web/src/api/types.ts`**.
  `api/src/bob/client.ts` is shared with W2 and W15 — strictly serial.
  *(The two `web/src/api/*` files added 2026-08-26 at the wave-16 cut, **R215**. They were absent
  while the criteria require the panel to fetch `GET /api/hypotheses/:id/artifacts` — and
  `client.ts` is **the only module in `web/` that calls `fetch`**, by the ownership row, so there is
  nowhere else the call could go. 🔴 **The omission also HID A COLLISION**: W24 is editing both files
  right now, so `W24 ‖ W29` read as legal on the Files lines and is not. W29 waits for W24.)*
- **Acceptance criteria:**
  - The Bob client gains reads for `GET /agent/sessions/by-name/{name}/artifacts` and
  ...
  - `GET /api/hypotheses/:id/artifacts` proxies the metadata list server-side, guarded by
    `requireSignedIn`. **Never redirect the browser to Bob** and **never log a `download_url`** —
  ...
  - The UI renders `ArtifactPanel` **imported from `@agentkit/chat-ui/components`**, under Wolf's
    ...
    `success.main` — the same assertion `verify-package.sh` makes in Bob, made again at the point
    of use, because that is what proves the shared component is themed by its host.
  - The panel renders inside `Provenance kind="machine"`: artifact **metadata** is Bob's record
...
- 🔴 **ADDED 2026-08-26 at W24's close (R219) — the Go Live gate's template half is missing from the detail page, and you own that file.** W24 built `GoLiveButton`'s second prop (`templateAccepted`) and wired it on the review screen, but `web/src/pages/HypothesisDetail.tsx:162` still renders the button with `specValidation` only — **so the detail page's Go Live button is enabled with no template accepted** — and its comment already claims otherwise: *"The spec half of the Go Live gate (W24 adds the template half)."* W24 was right not to touch it: that file is yours this wave (R194) and reaching into it would have been the collision the ownership rule exists to prevent. **W22's server-side `422` backstop means this is not a hole**, only a button that offers an action the server will refuse. **Pass `templateAccepted` from `report.has_template` on the detail payload, correct the comment, and test both halves** — `GoLiveButton.test.tsx` already covers all four combinations, so this is the page-level wiring test, the one W24's own M17 kills on the review screen.
- **TDD:** yes for the routes and the theming assertion; no for layout.
- **Validation:**
  - `cd api && yarn test src/routes/artifacts src/bob/client && yarn typecheck` — confirm
    **2 test files**
  - `cd web && yarn test && yarn typecheck`
- **Depends on:** O12, W28, W14
- [x] done
- Notes: **Wave 17, 2026-08-26 — PASSED, four fix rounds, ZERO blocking. Merged as `ac17884`.** api 37 files / **1826** tests on `main` after merge (W29 alone: 1732 → 1750), web 36 / **479** (from 462), all five gates green. **25 landing-gated mutations by the verifier, 22 killed**; all seven criteria PASS.
  🔴 **THE FINDING OF THE PROJECT'S SECURITY WORK — R228: the credential-absence test COULD NOT FAIL.** The criterion is that no `download_url` ever reaches the log. The executor mutated the log line to dump the **whole record**, expecting the leak — **and nothing appeared, because `mapArtifactRecord` had already dropped `blobPath` and `download_url` upstream.** Every `not.toContain` assertion was green over data that could never have contained the thing, and would have stayed green while the log line silently grew a field nobody sanctioned. Verified **on both sides**: at `fe8a8aa` the mutation ran green (10 tests, exit 0); at the fix it dies — **and only the SHAPE assertion fires while the value prohibitions stay silent**, which is itself the proof that pinning the key set is what carries the defence. Two corollaries: 🔴 **the fake must carry the forbidden value or the criterion is untestable** — Bob sends no `download_url` on this route, so the fake supplies one deliberately, and without it an allow-list and a passthrough are indistinguishable; and **every prohibition needs a positive half**, since a route that logs nothing satisfies every "must not contain" trivially.
  **Criterion 3 is the ticket's reason to exist and it is properly proven.** Bob's `ArtifactPanel` uses the MUI token `'success.main'`, resolved against the **host** provider — so there is no palette to "carry", and the verifier's answer to *what would have to be true for this to pass falsely* is **nothing; there is no such state**. Three independent guards anyway: Wolf's colours pinned as **literals** (never read back through the theme object the component read), an explicit negative against MUI's stock `success.main`, and a third case asserting light and dark paint **different** greens — which no hard-coded colour satisfies. Mutating Wolf's `positive` to MUI's default painted `rgb(46,125,50)` and the literal caught it.
  🔴 **R229 — a new door into R133's family, found here: a route that is mounted, reachable, and SILENTLY DEGRADING.** `App.test.tsx`'s detail-route case did not stub the new artifacts fetch; `stubFetchRoutes` threw as designed, the client caught it, the component rendered its degraded marker — **and all three route assertions passed while a user-visible error banner sat on the page** (verified, with the rendered text). R133's failure is an assertion also true when the thing is **absent**; this is one also true when the thing is **present but broken** — and harder to find, because R133's discriminator (a malformed-id probe) cannot see it.
  **Four fix rounds, and the method improved across them.** Round 3's three self-reported asymmetries were graded **separately** by the verifier rather than credited together: one was a **closed hole the executor had understated** (a hand-rolled route-local 401 with a different kind kills exactly one test, which `session.test.ts` structurally cannot see); one was **levelling that rescued nothing** (two of its three kills were pre-existing exact-path checks — logged as such so R226's tally is not inflated); and one was **half hole, half tidiness**, proved by **reverting only the new assertion while keeping the mutation** — a component rendering an error banner *and* "No artifacts yet" simultaneously passed all ten tests. **The executor adopted that method for round 4**, and its two new mutations kill one test each in opposite directions.
  **Its own summary of the R211 lesson, after correcting a comment and getting the new count wrong too:** *"the failure was not the count — it was treating 'correcting a comment' as a different activity from 'verifying a claim about the code'."* And on understating asymmetry 1: *"writing down what an assertion catches is not the same as testing that it catches it"* — it had written the discriminating sentence into the comment and then not built the mutation its own comment described.
  **Left open, none W29's:** 🔴 **Bob's `ListArtifacts` is unbounded end to end** (`go/agentdb/artifacts.go:72-85` — `Order("created_at DESC").Find(...)`, no LIMIT, no pagination), while every other proxied list in this project is capped and a hypothesis is validated daily for its whole life — **R225's shape, filed with R190's cap work**. `getSessionArtifactFile` is **unreachable from any route** (proved empirically: making it throw killed exactly 3 of 1750) with its **501** branch untested — the one a developer without Postgres hits first. And 🔴 **Bob's own published package disagrees with its Go source about `source`**: `"registered"` is written by **no Go code anywhere**, `"tool"` appears **only in Bob's tests**, and production writes `"capture"`, which is in **neither** half — **the two halves intersect on exactly `'auto'`**. Wolf's wire preserves the true value verbatim and `source` is read zero times in the rendered subtree, so the mapping loses nothing; widening the union is Bob's change.
- Notes:

### W32: The unbounded page walk — an iteration budget for every paging loop   [Status: done | Model: sonnet — run on opus]
- **Scope:** Give `readSessionIndex` (and every sibling that walks pages) a hard iteration and row
  budget that **throws** rather than looping forever. Two lines of behaviour; the tests are the work.
- **Repo:** agent-wolf
- **Files:** modify `api/src/hypothesis/store.ts` and `api/src/hypothesis/store.test.ts`. Audit the
  other paging loops in the same pass and fix any that share the defect.
- **Why, and it was found sideways.** W31's audit hit this while mutating for a different purpose;
  W31's verifier confirmed it by reading and reproduced the hang under a bounded 90s / 1.5 GB
  harness. `readSessionIndex` (`store.ts:1242-1275`) is `for (;;)` at `:1253` with **no iteration
  cap, no page budget, no request counter and no deadline**. Its sole exit is
  `if (page.length < limit) break;` at `:1271` — *a short page*. So a server that keeps returning
  page zero is not an error, it is an infinite loop. 🔴 **And Bob will do exactly that:**
  `go/httpapi/history.go:107-125` parses `offset` with `strconv.Atoi` and **silently falls back to
  `0` on any parse error** — it never rejects a bad offset, it hands back the first page. The same
  missing guard means `readSessionIndex({sessionPageSize: 0})` also loops forever (`:1243` takes a
  caller's `0` as-is).
- **Severity, as called by the verifier:** **latent, not live** — nothing triggers it today. But a
  one-line Bob regression, a proxy that strips a query parameter, or a future route change turns
  **every board read, every detail read and the poller's enumeration** into an unbounded request
  loop against Bob, with **no timeout, no error and no log** — a hung request plus a
  self-inflicted flood on the dependency. **The cost of closing it is far
  below the cost of it happening**, which is the whole argument for doing it early.
  ⚠️ *This paragraph and the Scope line said "the fix is two lines"; it landed as **68 lines of
  implementation and 136 of test**, and the Scope line's own "the tests are the work" contradicted
  it two lines earlier. Corrected 2026-08-26 (**R209**) — a two-line framing sets the wrong
  expectation for whoever sizes the next one of these.*
- **Acceptance criteria:**
  - `readSessionIndex` throws a `WolfError` (taxonomy: `internal` — this is our invariant breaking,
    not a caller's bad input) after a bounded number of pages **and** a bounded number of rows.
    Both bounds, not one. ⚠️ *The reason originally given here was **wrong**, and W32's verifier caught
    it (**R217**): "a server returning a full page of duplicates exhausts rows before pages" is false —
    `rows` and `pages` advance in lockstep regardless of duplication. At the default
    `SESSION_PAGE_SIZE = 200` the **page** budget fires first, at 10 000 rows, so
    `SESSION_INDEX_MAX_ROWS` is **unreachable on every production path today** and bites only for
    `sessionPageSize ≥ 409`. **Keep both bounds anyway**: a cap that is unreachable today but correct
    is exactly what you want in place before a caller passes a larger page size, and the implementer's
    own docstring (`store.ts:1235-1240`) states the true relationship. The criterion was right; only my
    argument for it was not.*
  - The message names what happened well enough to debug it from a log line alone — the route, the
    page count reached, and that the walk did not converge. 🔴 A timeout that says only "timeout"
    reproduces the silence this ticket exists to remove.
  - **A test drives the exact failure**: a fake that ignores `offset` and always returns page zero.
    It must terminate, and it must terminate by **throwing**, not by exhausting a test timeout —
    assert the thrown error, never merely that the test finished.
  - `sessionPageSize: 0` (and any non-positive value) is rejected at the edge rather than looping.
  - **Every other paging loop in `api/src` gets the same treatment or an explicit note saying why it
    cannot loop.** Enumerate them; do not fix only the one this ticket is named after. A cap on one
    loop while its three siblings stay unbounded is the R94 class arriving in code.
- **TDD:** yes.
- **Validation:** `cd api && yarn test src/hypothesis/store && yarn typecheck`. 🔴 **State the
  test-file count** — a positional filter matching nothing runs zero files and exits **0** (R196's
  neighbour).
- **Depends on:** — (independent of every other open ticket; can run in any wave)
- [x] done
- Notes: **Wave 16, 2026-08-26 — PASSED, one fix round, ZERO blocking. Merged as `f635fca`.** api 36 files / **1700** tests (from 1696), typecheck clean. Two files, +218/−1.
  **Both bounds delivered and both independently load-bearing**: `SESSION_INDEX_MAX_PAGES = 50`, `SESSION_INDEX_MAX_ROWS = 20_000`, module-private so no test can assert a literal through the constant. The **short-page exit is still checked first**, so a walk that legitimately converges on its fiftieth request is not refused for reaching the cap on it — the off-by-one with a real consequence, and mutation-pinned from both sides. Non-positive and non-integer `sessionPageSize` refused **before any request is issued**, asserted by counting requests to zero **with a positive control on the same stub**, because "no requests were made" would otherwise pass against a stub that was never wired up.
  🔴 **The Bob-side premise was verified in the Go source by both agents, not assumed:** `go/httpapi/history.go:107-109` keeps `offset` at `0` on any `strconv.Atoi` error and returns the first page — no rejection. The verifier reproduced the hang under a bounded 90s / 1.5 GB harness. **Reachable, not hypothetical.** (`limit` falls back to 50 the same way, but that yields a *short* page, so it terminates.)
  **The enumeration is criterion 7's deliverable and lives in these Notes rather than in code** (R110 — W32's Files line names two files). Three passes: grep every loop form; an awk sweep for **every `await` lexically inside any loop** in every non-test file, which is what proves completeness; then all 23 `client.list*`/`search*` call sites read individually. The verifier reproduced it independently and agreed: **`readSessionIndex` was the only paging loop in `api/src`.**
  🔴 **Rule five's second live catch, and by a different route than R196's — see R209.** The executor's harness snapshotted with `git archive <sha> api`, a **pathspec**; `api/tsconfig.json` extends `../tsconfig.base.json` at the repo root and vitest lives in the root `node_modules`, so all 36 files failed to **transform** and the runner printed `Test Files 36 failed (36)` / `Tests no tests`. **The file count matched the baseline while zero tests ran.** Under rule four all ten mutations would have been admitted with an **empty kill attribution** — ten fabricated "undefended" findings on a ticket whose whole deliverable is a kill table. Two new harness rules followed and are adopted: **the first run of any copy-based harness must be the closing control**, and **`git archive <sha> <path>` is not a safe snapshot of a workspace package.**
  **Three defects in the plan's own text, all found here:** ⚠️ the ticket said *"the fix is two lines"* twice while its own Scope line said *"the tests are the work"* — it landed as 68 lines of implementation and 136 of test; the Note now records that 🔴 **a vitest test timeout is NOT a bound on a non-terminating loop** (it fails the *test* but cannot cancel the promise — measured at exit 143 after 180s under an external `timeout`); and 🔴 **criterion 1's stated reason was false** — R217, `rows` and `pages` advance in lockstep, so at the default `SESSION_PAGE_SIZE = 200` the page budget fires first and the row cap is unreachable on every production path today, biting only for `sessionPageSize ≥ 409`. **Both bounds kept**: a cap unreachable today but correct is what you want in place before a caller passes a larger page size, and the implementer's docstring already stated the true relationship beside my wrong instruction.
  **Fix round:** the edge-guard test was the short one of three (R198 again — the executor had levelled T1 against T2 itself and the third sibling was outside that diff), and the `details` bag on all three throws was **inert** — an `internal` error's `details` reaches neither the wire (`app.ts:78` answers `{kind, message}`) nor the log (`app.ts:68` logs `{kind, path, msg}`), so under R148 it was a comment written as code. **Dropped**, with the reasoning left in the docstring. The ceiling is now on the record: the largest converging walk is 49 full pages plus a short one — **9 999 sessions** at the default page size.
- Notes: 🔴 **Do NOT reproduce the hang unbounded.** If you run it at all, bound it with a wall clock
  and a heap cap, as W31's verifier did, and never leave it running. The code claim is readable
  without executing it, and reading is the cheaper proof here.
  🔴 **THE PAGING-LOOP ENUMERATION, from W32's executor — this is the durable record, and criterion 7
  is satisfied by it rather than by comments in files W32 does not own (R110).** Method, three passes:
  grep every `for (;;)` / `while` / `do`; an awk sweep for **every `await` lexically inside any loop**
  in every non-test file (the pass that proves completeness — a loop with no await cannot spin on a
  server's answer); then all 23 `client.list*` / `client.search*` call sites read individually.
  **Exactly four `for (;;)` loops exist in `api/src`, and one had the defect.**
  (1) `store.ts:1298` `readSessionIndex` — **the defect, capped by this ticket.**
  (2) `provision.ts:559` `drain` — **cannot loop**: its exit is a wall-clock deadline checked every
  iteration and, the load-bearing part, **its exit does not depend on the server's answer at all**.
  It is a *poll*, not a page walk — it deliberately re-reads page one and never advances an offset,
  so there is no offset for a server to ignore.
  (3) `routes/hypotheses.ts:615` `waitForSession` — **cannot loop**, same shape: polls until a status
  leaves `creating`, exits on a deadline with `unavailable`. No paging.
  (4) `report/template.ts:369` `parseAttributes` — **cannot loop**: a string scanner over a fixed
  input, no I/O, every branch returns or advances, and it already carries an explicit zero-progress
  guard at `:391-395`.
  Cleared as bounded-by-input: `template.ts:502/:554/:580/:839`, `frame.ts:356`. `routes/report.ts:339`
  `cachePut` is `while (map.size > max)` and its body deletes, so the guard strictly decreases. Every
  other `for…of`/`for(let)` containing an `await` (30 sites) iterates an array obtained **before** the
  loop and none appends to the collection it iterates. No `do…while` anywhere; no recursive
  pagination. 🔴 **`readSessionIndex` is the only place in Wolf that walks pages at all.**
  🔴 **And the non-obvious half, measured by W32's executor: the RUNNER'S OWN TIMEOUT IS NOT A BOUND.**
  Vitest's per-test timeout fails the *test* but **cannot cancel the running promise**, so the loop
  keeps hammering the mock until an **external** wall clock kills the process — measured at exit 143
  after 180s under `timeout -k 5 180`, and 2.8 GB RSS when W31 hit it with no external bound at all.
  An executor assuming `testTimeout` is the bound will reproduce the hang in full.

### W33: The DinD gateway probe reads the wrong routing table   [Status: done | Model: sonnet — run on opus]
- **Scope:** One discovery function. `api/src/config.ts`'s DinD-gateway probe resolves an address
  **nothing inside a session container can reach**, so every `mcp__wolf__*` call from inside a
  session times out with no error and no log line.
- **Repo:** agent-wolf
- **Files:** modify `api/src/config.ts`, `api/src/config.test.ts`.
- **Why, and it was found by running the thing (X1, R235).** R43 prescribed discovering DinD's
  gateway from its **default route**. In the compose stack that route is the **outer compose-network
  gateway**, not DinD's inner `docker0` — which is the address a session container actually uses, and
  which § "Local topology and networking" **names correctly** two paragraphs above the probe it
  prescribes. Measured by X1 from a nested container inside DinD:
  ```
  curl 172.17.0.1:8100/mcp  → 401      reachable; credential refused
  curl 172.26.0.1:8100/mcp  → exit 7   could not connect
  ```
  and `config.ts` discovered **`172.26.0.1`**, writing
  `WOLF_MCP_URL=http://172.26.0.1:8100/mcp` into the project's `mcp_config`.
  🔴 **R43 warned that the VALUE drifts. The defect is that the DISCOVERY reads the wrong route** —
  a different failure from the one that was guarded against, in the same function.
- **Acceptance criteria:**
  - The probe derives the gateway from **DinD's own `docker0`**, not from its default route.
  - 🔴 **An explicit `WOLF_MCP_URL` still wins** — the design says so, X1's rig depends on it, and it
    is the escape hatch that made this bug survivable.
  - **A test pins the distinction**: given a routing table where the default route and `docker0`
    differ — which is the compose stack's normal state, not an edge case — the probe returns the
    `docker0` address. A fixture where the two agree cannot fail, and is the reason this survived.
  - The failure mode is **named in a comment**. ⚠️ **CORRECTED 2026-08-27 (R238): this said "a
    TIMEOUT". It does not time out.** X1's own recorded measurement is **`curl exit 7` — could not
    connect** — a timeout of curl's own would be exit 28. ⚠️ **AND THE FIRST CORRECTION WAS ITSELF AN
    INFERENCE (R244): "fails FAST" is not supported either.** Exit 7 means "could not connect" and
    says nothing about latency — measured: a refusal returns in **4ms**, a black hole returns exit 7
    after **minutes**, and the black hole is the likelier shape here. X1 recorded the code and not the
    elapsed time. 🔴 **Do not diagnose this by how long the call took.** The
    conclusion — that it is invisible — still holds, but **for a different reason: not latency, but
    that the failure lands on the far side of the container boundary**, where nothing Wolf logs can
    see it. Say that, not "timeout".
- **TDD:** yes.
- **Validation:** 🔴 **`cd api && yarn test && yarn typecheck` — the WHOLE api suite, not `src/config`.**
  *(Corrected 2026-08-27, **R239**. The line originally read `yarn test src/config`, which runs **one
  file** and does **not** touch `api/src/mcp/server.test.ts` — the probe's SECOND consumer, and a file
  this ticket had to edit. **Eight of W33's verifier's nineteen mutations kill a test that lives only
  there**, so a regression in it would pass the ticket's own gate.)* **State the test-file count** for
  whatever you run — a positional filter matching nothing runs zero files and exits 0.
- **Depends on:** — (independent; X1's rig sets `WOLF_MCP_URL` explicitly and is unaffected)
- [x] done
- Notes: **Wave 18, 2026-08-27 — PASSED, five rounds, ZERO blocking. Merged as `210cc9e`.** api 37 files / **1834** tests (from 1826), web 36 / 479, all gates green. **31 mutations by the verifier across three graded commits, 25 killed**; the implementer ran the full 15-mutation sweep **from a pristine rebuilt at every final sha** rather than reusing a table, four times.
  🔴 **THE FIXTURE IS THE TICKET, and both axes were proved by the verifier reverting the code itself.** The two addresses **agree on a plain Docker install and differ only under compose**, so a host-side check passes and a fixture where they agree **cannot fail** — which is exactly the fixture the original tests had. Restoring the pre-W33 default-route probe kills **11**; removing the interface check kills **7**, and that second one only bites because the fixture carries a directly-connected `eth0` row **ordered before `docker0`** — which the implementer caught **in its own first fixture** by reasoning about a mutation before running it. Its own words: *"my fixture differed on the axis the ticket named and agreed on an axis it did not."*
  🔴 **The escape hatch had a hole in it.** An **empty** `WOLF_MCP_URL` is load-bearing and was **completely untested**: compose forwards `${WOLF_MCP_URL:-}`, so an operator who has not set it gets `""` **inside the container, not absence**, and discovery runs at all only because `if (explicit)` treats it as falsy. Tightening that to `!== undefined` gives every session an **empty address**, failing the same silent way — and **would have read in review as a tidy-up**. One test now defends it and **it is the sole defender**, proved by a mutation killing exactly it and nothing else in 1834.
  **The old function is DELETED, not left beside the new one** (grep across both repos returns nothing), and the tests **stopped importing `DEFAULT_GATEWAY_FALLBACK`** and hardcode the literal — under the old import-the-constant style, mutating the constant would have **survived**; it now kills 5.
  🔴 **Rounds 3–5 are a COMMENT, and they are the most instructive part of the ticket — see R244/R245.** On one claim, in two hours: **the plan** said the failure *"times out"*; **the orchestrator's correction** said *"fails fast, immediately"*; **the verifier's objection** said *"exit 7 covers the slow case too"* and offered three probes; **and its own third probe, still running as it wrote, refuted it** and it withdrew, unprompted. **Four inferences, all plausible, all internally consistent.** The two measurements settled it — reproduced independently by the orchestrator: **exit 7 in 0.00035s** on a refusal, **exit 28 at 2.0s** with a connect timeout, **exit 28 at 132s** on kernel SYN exhaustion (the verifier measured 131s). **Exit 7 is the immediate-negative case and every slow shape returns 28.** The shipped comment states those numbers and tells the reader to **diagnose by the exit code, not the clock**.
  **What the implementer did that is worth copying:** it **refused the inference the orchestrator offered it** — the reasoning about why the black-hole shape is likelier here — on the grounds that *"it is convincing and it is still an inference, and the comment I am fixing is a comment that broke twice by adding one."* And it re-ran the whole sweep at each final sha because *"comment-only is a claim about a diff, and the sweep is what grades the tree I am actually handing you."*
  **Left open as notes:** the **`+1` inference** — the bridge address is derived as the docker0 network **plus one**, correct for every default configuration and for R43's reproduced shift, wrong only under an explicit `dockerd --bip` with a non-`.1` host part, and **that limitation is still not stated in the file**. A **boot race**: wolf-api joins DinD's netns with no health gate, so if it reads the table before `dockerd` creates `docker0` it silently takes the fallback — the same failure shape, and `source: "fallback"` in the boot log is the operator's only signal. And the **definitive measurement nobody has**: X1 recorded the exit code, not the elapsed time, **on the real failure from inside a nested container**.
- Notes: 🔴 **Do not "verify" this by reading the routing table on the host.** The two addresses agree
  on a plain Docker install and differ under compose, which is exactly why a host-side check passes
  and a container-side one fails. X1's measurement was taken **from a nested container inside DinD**
  and that is the only reading that counts.

### W31: The test-fake fidelity audit   [Status: done | Model: opus]
- **Scope:** Measure, for all fourteen hand-rolled fakes, which parameters the production code sends
  through them and which the fake actually implements. **Produce a document. Fix nothing.**
- **Repo:** agent-bob (the output file only); reads `agent-wolf` **read-only**
- **Files:** create **and COMMIT** `design/2026-08-26-agent-wolf-fake-audit.md`. **No other file in
  either repo may be modified** — not a fake, not a test, not a source file. That constraint is the
  ticket. *(“and COMMIT” added 2026-08-26, **R201**: this line said only “create”, the executor
  created it and never committed, and 999 lines of measured work sat untracked in a worktree due to
  be pruned. A deliverable that is not a commit is not a deliverable.)*
- **Why this exists, and why it is not part of the consolidation.** **R184** records that fidelity is
  a per-file property with no shared floor and no test. **R180** measured two instances. **W22
  measured a third, forwards**: it fixed three ignored parameters in two fakes, and deleting
  `user_email: "*"` from `bob/client.ts` went from killing **zero** tests to killing **76**. Twelve
  fakes remain unmeasured. 🔴 **We do not know the size of the problem, and the consolidation cannot
  be scoped until we do** — so this ticket buys the number, and nothing else. Fixing as you go would
  make the audit's own findings unreproducible and would collide with every ticket in flight.
- **Acceptance criteria:**
  - The document enumerates **all fourteen** fakes by path: `app`, `bootstrap/bootstrap-project`,
    `hypothesis/poller`, `hypothesis/provision`, `hypothesis/store`, `marketdata/fred`,
    `marketdata/stooq`, `bob/client`, `routes/auth`, `routes/embed`, `routes/hypotheses`,
    `routes/report`, `routes/series`, `testing/undici-mock.smoke`. A fake omitted is the failure
    mode this ticket exists to prevent, so the list is checked against
    `grep -rln "MockAgent\|setGlobalDispatcher" api/src --include=*.test.ts` and the count stated.
  - For each fake, a table: **route → parameter → IMPLEMENTED or IGNORED → evidence (`file:line` in
    the fake's own code)**. 🔴 **Read the fake's own body. Never infer from a test name, a comment,
    or what the parameter obviously ought to do.**
  - Parameters are enumerated from the **production** side — what `api/src` actually sends — not from
    what the fake happens to parse. A parameter the fake never sees is the interesting case, and
    reading only the fake cannot find it.
  - 🔴 **Every IGNORED entry carries a CONSEQUENCE line: which assertions are therefore decoration.**
    An ignored parameter is not itself a defect; it is a defect *when a test claims to cover the
    behaviour that parameter controls*. Name those tests by `file:line`, or write "none today" —
    which is a real and common answer.
  - **Each IGNORED entry is PROVED, not asserted**, by the R184 method: mutate the production line
    that sends the parameter, run the suite, and record the kill count. **Zero kills proves the
    decoration.** A non-zero kill count means the parameter is defended by something and the entry
    is wrong — say so.
  - The document ranks the findings: **which ignored parameters guard a security property**
    (`include_retracted`, `user_email`, provenance and tenancy filters) versus which guard a
    correctness or cost property (`limit`, `offset`, `latest_per`, `?version=`). The first class is
    the consolidation's first wave of work.
  - **R190 is closed or contradicted by name**: `BOARD_LIMIT = 100` (`store.ts:1170`) is claimed to be
    decoration because both board fakes ignore `limit`. Prove it or refute it.
  - The document ends with a **scoping recommendation for the consolidation**: what a shared fake
    would have to implement to raise every file to one floor, which fakes could adopt it unchanged,
    which are load-bearing enough to need their own, and an honest estimate of how many existing
    tests would change meaning. That recommendation is this ticket's deliverable, not a nice-to-have.
- **TDD:** no — the deliverable is a measurement, and its evidence is the mutation kill counts.
- **Validation:**
  - The mutation harness obeys all four rules (**R153/R168/R170/R178**): exit-code decisions with
    "did not run" classified separately, an opening control, a closing control, and a per-mutation
    landing gate. **State them in the document.**
  - `cd api && yarn test && yarn typecheck` at the end, proving the tree was left exactly as found —
    36 files / 1696 tests, exit 0.
  - `cd /home/kai/projects/badcode/agent-wolf && git status --porcelain` prints **nothing**.
- **Depends on:** W22
- [x] done
- Notes: **Wave 15, 2026-08-26 — PASSED, one fix round. Merged as `b34a6fd`** (document committed as `fd03af6`; agent-wolf untouched throughout, `git status` clean, `git diff a155c96 HEAD -- api/` empty). The deliverable is `design/2026-08-26-agent-wolf-fake-audit.md`, 1015 lines.
  🔴 **THE HEADLINE IS ≥ 42 UNDEFENDED SENDS — 7 SECURITY-CLASS, 6 EVIDENCE-INTEGRITY (after the `rationale` ruling below), 29 CORRECTNESS/COST — AND THE SHAPE IS NOT WHAT R184 AND R190 PREDICTED.** Only **2** currently-passing assertions are decoration. **40 of the 42 have no claimant at all.** The permissive fake is not mostly making tests *lie*; it is making tests that cannot fail **unwritable**, and that is what nobody noticed. That reframe changes what the consolidation buys and how it must be sequenced, and it was attacked by the verifier — which hunted claimants by title across all fourteen files, mutated **all sixteen** `includeRetracted` sends (the seven named kill 0; the other nine kill 1–13) and confirmed the class-1 list is complete — before it was accepted.
  🔴 **The audit's own verdict on itself is the most useful sentence in it: the count went 24 → 40 → 42 across two passes, every increase from splitting an aggregated row, and the last increase found a call site TWO INDEPENDENT PASSES HAD MISSED.** §2.5 originally folded five `limit: DETAIL_LIMIT` callers into one row reported as *defended*; four are undefended, including `readTemplate` and `readLatestReport` — the two reads every report in the product goes through. **Under-counting by aggregation is exactly what a permissive fake does to a test suite, and it is what the audit did to itself, in the file it examined most closely.** The granularity rule now in §1.2 — *measure once per call site, never once per parameter name* — is the correction, and every count in the document is stated as a **floor**.
  **Verification was reproduction, not review.** The verifier rebuilt the experiment from a fresh `git archive` and ran 83 of its own mutations: both controls to the digit **with identical per-file breakdowns** (78 = 2/41/1/34 and 39 = 1/27/1/6/4), all 24 original zero-kill sends, 152 for `selector` with an identical distribution, 162 for `offset`, the two `BOARD_LIMIT` survivors at the exact test lines, and **all fourteen per-file test-count denominators**. It also proved the audit's "three chokepoints" premise independently rather than accepting it.
  **Three findings that outlive this ticket.** (1) 🔴 **R196**, the fifth false-green direction, found by the verifier as a deliberate self-check and then **strengthened by the auditor beyond what the orchestrator specified** — assert the collected test *total*, not just the file count, and retro-validate (68 runs, 0 violations). (2) 🔴 **The inertness rule** (§2.6): a matcher-based fake defends a parameter **only when the test's expected outcome differs from the outcome of "no interceptor matched"**. With `disableNetConnect()` a miss throws and `fred.ts:100-104` catches any throw into `unavailable`, so every test asserting `rejects → unavailable` cannot distinguish match from miss however precise its matcher looks. **It is a rule the consolidation needs, not a module**, and the verifier checked it does not generalise further — its reach is fred + stooq, both already covered. (3) **W32**, the production hang, found sideways while mutating for a different purpose.
  **Corrections the fix round accepted, all re-measured rather than taken on trust:** three inert stooq matchers, not four (an over-count in the *unflattering* direction); the 78-vs-76 gap **measured across all four W22 commits** rather than guessed — the +2 arrives entirely at fix round 1 and entirely in `routes/hypotheses.test.ts`; **≈116 carried as a range ~30–116** with both criteria named, because the exclusion criterion sits on a continuum and a ticket must not be scoped off it as though it were measured; and 🔴 **the invalid-mutation rate restated as an ARTEFACT** — an occurrence check that *refuses* a stale or ambiguous anchor at apply time converts would-be invalid mutations into rejections rather than runs, so neither 3.0% nor the verifier's 0-of-83 is comparable to **R182**'s 1-in-20. The transferable claim is about the **guard**, not the rate.
  **Orchestrator rulings recorded here because the consolidation reads them:** the four `rationale` sends move to **class 2, evidence integrity** — they write the *explanation* field of Bob's config log for every teardown and rollback, and dropped, the log records that things were deleted and no longer records why. Not provenance or tenancy, so not class 1; but a record narrowed to its empty form is a **claim**, and the claim "this teardown had no stated reason" is false and unfalsifiable afterwards — **R185's shape, in an audit log**. They head class 2 and are early work. And the two `fetchImpl`-seam fakes (`mcp/server.test.ts`, `mcp/seriesdownload.test.ts`) are in scope for the consolidation's framing even though they are invisible to the ticket's grep — 🔴 **a fake that replaces `fetch` by injection cannot be found by grepping for the dispatcher**, which is a hole in the completeness argument, not in the count.
  **Sequencing, verified twice and adopted:** the **six missing retraction tests come FIRST**, before any shared-fake work, because four of the seven class-1 holes are *missing fixtures* that no shared fake can close; `bob/client.test.ts` **stays bespoke** (its fake *is* the request-shape oracle and is the only defence anywhere for five parameters); `bootstrap/bootstrap-project` is **already stricter** than the proposed floor and is left alone; and **W32 runs alongside step one**, being two lines and the only finding that can hang production.
- Notes:
...
| **R5** | **Nothing ever triggered `live → challenged`,** and `horizon_days` fired nothing. Bob cannot call Wolf. | New ticket **W10** |
...
| **R20** | **The owner-slug mapping handled only `@`** (`kai+test@gmail.com` produces an illegal label); `/auth/verify-google` 404s unless `GOOGLE_CLIENT_ID` is set on Bob; archive-and-relaunch was a free scoreboard re-roll. | W5's total mapping; W8's config error; `restated_from` label |
...
- **The DinD topology in § "Local topology" is verified sound.** A service on
  ...
  hard-binds to Bob's compose project name and dind must already be up (compose `depends_on`
...
| **R104** | 🟡 **MITIGATED IN WOLF, still open in Bob (reviewed 2026-08-24).** `getCurrentMemory` asserts the expected `kind` client-side and throws `not_found` when the newest memory with that name is a different kind (`api/src/bob/client.ts:695-718`), so Wolf **fails loudly rather than returning the wrong memory**. The route stays unusable whenever a different kind was written more recently; fixing Bob is the owner's call and nothing in this plan is blocked on it. Original finding: **`GET /agent/memories/current?name=` cannot do what four tickets assume it does, and for Wolf it is close to useless.** Bob builds the selector as literally `"name=" + name` (`go/httpapi/memories.go:391`) and accepts no other query parameter — no `kind=`, no `selector=`, no `include_retracted=1`. **In Wolf's vocabulary EVERY kind shares `name=<hypothesis id>`** — `hypothesis`, `hypothesis-spec`, `verdict`, `evaluation`, `report-template`, `research-note`, `report` — so the route answers with whatever the researcher happened to write most recently, whatever kind that was. It is also blind to retractions, which makes it unusable for anything the trust model touches. W15 implemented its `kind` argument as a **client-side assertion** (mismatch → `not_found`) and used `listMemories` with a `kind=,name=` selector everywhere it actually mattered, which is correct but means the route is doing almost none of the work its callers expect. **Owner decision wanted: either add a `kind=` (or `selector=`) parameter to that Bob route, or strike it from the tickets that assume it** — leaving it as-is invites a later ticket to call it and silently read the wrong row. Note this route is one of the embeddable-Bob features `docs/19-embedding.md` advertises, so the fix is not Wolf-local. | **Open** — a Bob-side ticket, or the tickets that call it |
...
| **R170** | 🔴 **A THIRD direction on R153, and the worst of the three: a concurrent peer overwrote the MUTATOR, so a mutation was never applied and "no test failed" was read as "the mutation survived."** Wave 13 ran four agents at once, and every brief pointed them at the same session scratchpad — **"session-specific" is not "agent-specific"**, a fact none of the briefs stated because the orchestrator had not noticed it. Two agents chose the generic subdirectory `mut` and generic script names (`run.sh`, `mutate.py`, `reset.sh`). One had its `mutate.py` silently replaced mid-sweep and reported a **false GREEN**; the other, finding its harness files gone, ran `rm -rf api/src` in what it believed was its own directory and **destroyed the first agent's working copy unrecoverably** — the scratch tree had no `.git`. Compare the three known directions. **R153**: the runner crashed and its output was grepped for a failure marker that never appeared. **R168**: the shell killed the driver mid-sweep, so the next snapshot baked a mutation into its own baseline. **R170**: every component the agent controlled worked perfectly, and a *different process* removed the step that does the work. That is why it is the worst — there is no local symptom at all. **The rule this forces, now standing in every brief: a mutation harness must PROVE its mutation landed before running the suite** — diff the file, or assert the mutated token is present in the source — because "the edit never happened" and "the edit happened and nothing noticed" produce identical exit codes, and only one of them is a finding. An opening control (R153) proves the harness can go red; a closing control (R168) proves the tree ended as it began; a **per-mutation landing check** is the only one that catches this. 🔴 **STATED LIMIT, added 2026-08-26 after the gate failed to catch a real case: the land-set gate proves an edit landed in exactly the intended file, and says NOTHING about whether the edit was the one you meant.** A verifier's mutation was truncated mid-arguments by a `head -16`, so the replacement string became the next line of its own script and a download call was replaced with a shell assignment — landing cleanly in the right file and killing **26** tests. **"Didn't land" and "landed as garbage" are different failures**, and only R146 catches the second: the kill count was implausible on its face for a version-pin mutation, which is what made the verifier look. The gate is necessary and not sufficient; **read the kill set and ask whether it makes sense** remains the check no automation replaces. Three further lessons. (1) 🔴 **Concurrency turns a shared temp directory into a shared namespace, and generic names are then a collision waiting to happen.** Standing rule: `scratchpad/<agent-name>-<unique>`, never `mut`, `work`, `tmp` or `copy`. (2) **Verify a commit from `git archive`, not from the worktree.** The worktree was legitimately dirty with a fix round in flight; the verifier that exported the commit measured something reproducible, and one that had copied the directory would have produced a verdict about a commit that does not exist. (3) **A scratch tree with no `.git` cannot be restored after exactly this mistake** — copy the git metadata, or never `rm -rf` a path you did not create in this task. *(Both agents reported the collision immediately and precisely, including the parts that made them look bad, which is the only reason the blast radius was knowable. The shared path was the orchestrator's omission in all four briefs, and no real repository was touched.)* |
| **R171** | **"A lie about state is worse than a repeatable action" — the ordering principle for two appends with no transaction between them.** W21's accept path writes two memories: the new `report-template`, and the decision row recording the human's rationale. There is no transaction across them, so one order or the other must be chosen for what a crash between them leaves behind. **Decision-first** leaves a durable row saying a human accepted a template that was never locked — the report keeps rendering the old one while the record says otherwise, and nothing surfaces the contradiction. **Template-first** leaves the proposal still `status=proposed`, so a human simply decides it again. The implementer chose template-first and stated the principle in those words; it is recorded here because it generalises past this route to every append-only pair in the product. Two corollaries fell out of the same reasoning. (1) **A validator rejection writes NOTHING, not even a decision row** — a template failing validation is not a human's decision, and recording it as one would put a fictional human act in an append-only log that has no delete. (2) 🔴 **A decision row IS a `kind=report-amendment` row**, which the ruling that ordered it failed to name: the amendment selector now returns decisions alongside proposals, and without a guard a caller could "accept" a decision, whose empty body would surface as a **422 about invalid HTML** rather than as what it is. Only `status=proposed` is decidable; anything else is **409** naming the status. The transferable lesson is about rulings rather than about routes: **an instruction to add a row to an existing kind changes every reader of that kind**, and the person issuing it is the one least likely to enumerate them. |
| **R172** | 🔴 **The orchestrator said "return the composed frame" and the implementer returned everything except the document — correctly, on a security argument the instruction had not considered.** W21's `composeReportStats` hands W22 `{structureHash, strippedCount, reportMemoryId}` and deliberately **not** `html`. The reasoning: the composed document is safe only inside the sandboxed frame the CSP header applies to, and a caller holding the bytes creates a path where the SPA renders them with **no sandbox, no `frame-ancestors` and no opaque origin** — turning a locked-down iframe into innerHTML. "The bytes leave this process through `GET …/report/frame` and nowhere else" is the invariant, and it is a **security boundary, not a payload-size preference**. Approved as issued, and the implementer's own corollary was taken too: the prohibition is now written into **W22's and W23's Notes**, so a later ticket cannot widen the accessor without meeting the argument. Three lessons. (1) 🔴 **An accessor's return shape is part of a security boundary whenever the data is only safe in one rendering context.** The frame's safety lives in the CSP header and the sandbox attribute, both of which belong to the *response*, not to the bytes; hand the bytes to anyone else and every protection is silently absent. (2) **A ruling is an instruction, not a specification**, and an implementer who declines the literal wording on a stated argument is doing the job. This was the second time on one ticket the same agent refused the orchestrator's wording and was right — the first being its refusal to invent a field in a pinned content format. (3) **The corollary matters as much as the decision.** A security property held only in one implementer's head is one refactor from gone; writing the prohibition into the two tickets that would otherwise ask for it is what makes it durable. |
| **R173** | 🔴 **SVG presentation attributes are an UNSCANNED URL channel — `<svg><path fill="url(http://evil.example/x)">` validates clean with `remoteOrigins: []`.** `fill` and `filter` are absent from `URL_ATTRIBUTES` (`api/src/report/template.ts:751`), so the attribute walker never sees them. Measured by W25's verifier. The same shape as **R118(1)** and **R118(2)**, which is the point: this is now the *third* time the URL inventory has been found short, and each time by someone building a consumer of it rather than by anyone reading the walker. **Direction of failure, stated precisely:** the *contract* over-promises strictness — `report-authoring.md` said the rule applies to "every URL channel" — so no legitimate author is rejected, and `default-src 'none'` blocks the fetch at render time. What breaks is the **review screen**: W24 shows a human `remoteOrigins`, and a host reachable only through `fill=` would never appear on it. That is R118's own fail-open half, unchanged. **Belongs to W16/W30's walker, NOT to W25** — W25 correctly narrowed its prose to the CSS half. 🔴 **Open: add `fill` and `filter` to `URL_ATTRIBUTES` (and re-audit the SVG presentation-attribute set) whoever next opens `template.ts`.** The transferable lesson: **an allow-list of URL-bearing attributes is a list of things someone remembered**, and SVG is where memory fails, because its URL channels look like paint rather than like links. |
| **R174** | **`api/` depends at test time on a package it does not declare, and the workaround that made it compile also concealed that.** `fixture.test.ts` reaches jsdom through `createRequire(...)("jsdom")`. W25's verifier proved by conversion that a plain `import { JSDOM } from "jsdom"` makes `import-boundary.test.ts` fail with *"package `jsdom` is not declared in a package.json"* — exit 1 with the import, exit 0 with the `createRequire` form. `jsdom` resolves from the **repo-root** `node_modules` as a transitive of `isomorphic-dompurify`. Acceptable as shipped, because it fails loud the moment resolution breaks rather than silently doing something else — but two things are worth recording. (1) 🔴 **The dodge concealed a true finding.** The file's comment gave only the typecheck reason, so a reader learns that the import style is unusual and not that the dependency is undeclared. **A workaround should document what it is working around, not merely that it is one.** (2) This is **R145's class again, fifth instance**: a guard that greps raw source text can be satisfied by rewriting the text rather than by fixing the thing, and it teaches authors to reword rather than to resolve. `tools/import-boundary`'s specifier regex remains open from R145. Either declare `jsdom` as a dev dependency of `api/` and use a plain import, or make the guard's message say "declare it or explain here" — the current state rewards evasion. |
| **R175** | **The orchestrator's briefs overrode the plan's own EXECUTION RULES for eleven tickets without noticing, and the plan is now the thing that is wrong.** The header says workers "may only append to Notes and the Discovered Issues Log". Every brief written from wave 4 onward has said the opposite — *do not touch the plan file, ticket Status, checkboxes, or the Discovered Issues Log; report findings to me instead* — and W25's verifier flagged the implementer for not appending, correctly by the document and incorrectly by the instruction it was given. **The instruction is the right one and the header should follow it**, for a reason worth stating: with two to four agents in flight per wave, concurrent appends to one file are a merge conflict in the artefact that has to survive the wave, and the orchestrator is the only party who can see all of a wave's findings at once and knows which are duplicates of each other. Routing findings through the orchestrator is also what produced R151, R155, R162 and R166 — corrections to *orchestrator* text, which a worker appending directly to a log would have had no occasion to make. **Ruled 2026-08-26: the EXECUTION RULES header is amended to match practice** — workers report findings to the orchestrator, who writes them up. The general lesson: **when instructions and their governing document disagree for eleven tickets and nothing breaks, the document is the stale one** — and the tell here was a verifier faulting a worker for obeying its brief. |
| **R176** | 🔴 **A ruling that said "append a decision row" and did not say the row must POINT AT WHAT IT DECIDED — so a decided proposal stays decidable for ever and a replayed accept silently reverts the locked template.** Measured on W21: accept amendment B, accept amendment C, re-accept B → **200**, and the frame's CSP carries B's origins again. A human who approved C now sees B's template, above a fresh `status: accepted` row asserting they chose it. Nothing was forged, no taxonomy rule was broken, and the ticket's criterion — "accept re-validates and writes a NEW `report-template`" — is **literally satisfied**. The design's whole claim is "agent proposes, human decides, enforced by provenance", and a replayed accept walks straight past a review that already superseded it. **Root cause is in R167's ruling, not in its implementation:** the decision row carried no `amendment_id`. The id exists at the moment of the write — it is in the response and in the log line — and was discarded, so the route could not build an "already decided" check from the record, and W23/W24 could not grey out a decided proposal. **Ruled: the id goes on as a LABEL** (`amendment: <memory-id>`, which fits the K8s charset), not in the content — the question is "does a decision exist for this id", which must be a *query*, and a body line is not selectable. Then a proposal that already has a decision is **409** naming the existing status. Three lessons. (1) 🔴 **An append-only log records that something happened; it does not by itself record that something is FINISHED.** Every state machine built on appends needs the terminal fact to be *queryable*, or "has this been decided" degrades into "did I see a decision go past". (2) **The implementer had already found the same class and guarded it** — a decision row cannot itself be decided, 409 naming the status — which is the *named* case; the unnamed one, re-deciding the proposal, was left open because the ruling enumerated one and not the other. **When a ruling introduces a new row into an existing kind, enumerate every question that kind is asked, not the one that prompted the change** (R171 said this of readers; this says it of writers). (3) **A criterion can be literally satisfied by a defect.** "Accept writes a new template" is true of the replay; what was never written down is that accepting twice must not. **Where an action is meant to happen once, the criterion must say once.** |
| **R177** | **R122's class, arriving through a ruling: adding a row type to a shared kind halved a page limit nobody re-sized.** `reportSelector` deliberately omits `status=` (documented: a selector pinning `status=locked` cannot see a row labelled oddly, and "no template" would then be indistinguishable from "labelled strangely" — sound, and left alone). But `ROW_LIMIT = 50` with no pagination was sized when only *proposals* carried `kind=report-amendment`. R167's decision rows now share that kind, so the page fills at **twice** the old rate and a busy hypothesis will **404 a legitimate `amendment_id`** — a lookup failure that reads as "that amendment does not exist". Same shape as **R122** (delivery reads taking a single unfiltered page with no offset loop), reached from a different direction: nobody wrote a bad limit, a ruling changed the population the limit was sized against. **Fix is to page until found, not to raise a constant** — a raised constant is the same bug with a later onset. The transferable rule: **a ruling that adds rows to an existing kind must re-examine every LIMIT, SELECTOR and COUNT that reads that kind**, and the person issuing the ruling is the least likely to know where they are. This is the second consequence of R167 found by measurement rather than by review, after R176. |
| **R178** | 🔴 **A FAKE THAT IS MORE PERMISSIVE THAN THE REAL SERVICE DOES NOT FAIL — IT SILENTLY CONVERTS A DEFENCE INTO DECORATION, AND THE TEST THAT "COVERS" IT KEEPS PASSING.** Two instances in `report.test.ts`'s Bob fake, in two consecutive fix rounds, each invisible to ~1600 green tests and each found only by a mutation refusing to die. **(1) It ignored `limit`** and returned every matching row, so a 50-row page cap was invisible to every test in the file — `ROW_LIMIT`, `DETAIL_LIMIT`, and the test written specifically to prove the pagination fix, which therefore proved nothing. **(2) 🔴 It ignored `include_retracted`** and returned retracted rows to every caller — which did not merely leave the spec path untested, it meant **NO retraction defence anywhere in that file was being tested at all**, including the template path's dedicated *"a template hidden by a HOSTILE retraction is still served"* test. That test passed because the fake never filtered, not because the code asked for the flag. Three lines fixed it, and the mutation that drops `includeRetracted` from the spec read now kills where an hour earlier it would have killed nothing. Four lessons. (1) **A fixture is an oracle, not a backdrop.** Every simplification in a fake is a claim that the difference does not matter, and where the real service *filters* and the fake does not, the claim is false in the one direction that matters — the fake answers "yes" to questions the real thing answers "no" to. (2) 🔴 **Permissiveness fails silently and in the direction of false confidence**, which is why it outlives every green run: a fake that is too strict breaks tests loudly on day one. **Audit fakes for what they DON'T enforce.** 🔴 **And the audit is MECHANICAL, which is what makes it worth writing down: for every filter parameter the client sends, ask whether the fake implements it or ignores it.** Here the client sent exactly two — `limit` and `include_retracted` — and the fake ignored both. That check takes a minute, needs no judgement, and would have caught both instances on the day the fake was written. (3) **The blast radius is not the test you were writing.** Instance (1) invalidated `DETAIL_LIMIT`'s tests, which nobody was looking at; instance (2) invalidated a *security* test in a different part of the file. A fixture defect is a file-wide defect. (4) **This is R146 arriving through the fixture rather than through the assertion** — the test went green for a reason that had nothing to do with the code under test — and it is the fourth distinct channel found for the same failure, after the runner (R153), the shell (R168) and a concurrent peer (R170). |
| **R179** | 🔴 **A ruling said "put the id on as a label and query it" and did not notice the id is INTERPOLATED INTO A SELECTOR — so a comma injects a term and the guard silently matches nothing.** Kubernetes selector terms are ANDed, so `kind=report-amendment,name=<id>,amendment=<user-supplied>` with a comma in the supplied value adds a term nothing satisfies: the already-decided query returns empty, the 409 never fires, and **R176's replay defect is re-opened with no error anywhere**. Closed by constraining `amendment_id` to the K8s label charset **in the zod schema at the edge**, not at the query site — a request carrying `,kind=nonsense` is a 400 with zero upstream requests. Three lessons. (1) **A selector is a query language and string interpolation into one is injection**, however innocuous the value looks. This codebase already had the tell: **R104** records Bob building its own selector as literally `"name=" + name`, and the same shape reappeared here in a ruling written by someone who had read R104. **A known injection shape in a dependency is a checklist item for every new call site, not a fact about that dependency.** (2) 🔴 **This injection fails CLOSED into a guard, which is the worst place for it** — no error, no log line, no failed request; the query simply matches nothing and the protection evaporates. An injection that breaks something is found immediately; one that quietly satisfies a `.length === 0` check is not. **Any guard implemented as "a query returned nothing" needs its inputs constrained where they enter, not where they are used.** (3) It is the third defect on one ticket originating in an orchestrator ruling rather than in its implementation (**R176**, **R177**, this) — all three from the *same* ruling, R167, and none found by review. **A ruling that adds a row, a label and a query is three interfaces, and it deserves the same adversarial pass a ticket gets.** |
| **R180** | 🔴 **THE FIXTURE IS LOAD-BEARING TEST INFRASTRUCTURE WITH ZERO TESTS, AND THE CLASS CANNOT BE FOUND BY THE METHOD THAT FOUND ITS FIRST TWO INSTANCES.** W21's verifier measured it directly, and the measurement is the entry: making the fake stop honouring `include_retracted` → **1618 green**; stop honouring `limit` → **1618 green**; and — the line that matters — stopping BOTH the fixture's `include_retracted` *and* the two production reads that pass `includeRetracted: true` → **1618 green**. That last state is where this codebase actually was an hour earlier, reproduced deliberately, with every test passing. **A three-line fixture regression silently re-converts every retraction defence in the file into decoration, and then conceals the production regression that follows it.** Why it outranks **R178**'s two instances rather than merely extending them: `limit` and `include_retracted` were each caught because a mutation of **production** code refused to die. 🔴 **A mutation of FIXTURE code dies nowhere, by construction** — mutation testing's whole mechanism is "change the code, watch a test fail", and a fake has no test above it to fail. So the discipline that has found every blocking defect in this project is structurally blind to its own foundation. **The fix is the `#text`/`KEEP_CONTENT` pattern from the sanitiser profile: test the fixture's own behaviour.** Three tests driving the fake through the client — a retracted row hidden without the flag, shown with it, `limit=1` returning one row — fail when the fixture regresses, which is what makes every downstream defence honest. Two further lessons. (1) **Fidelity gaps in a fake are all in one direction: permissive.** Three found in this fake (`limit`, `include_retracted`, the dataset download ignoring `?version=`), plus a fourth dimension where it defaults `limit` to 100 with no cap while Bob defaults to 20 and caps at 100. Nobody writes a fake that is *stricter* than reality, because that breaks tests immediately; permissiveness is the only failure mode that survives. (2) 🔴 **A REQUEST-SHAPE assertion outlives a permissive fake; a BEHAVIOURAL one does not.** Measured on the same defence: dropping `includeRetracted` from `readTemplate` kills *two* tests — `store_read_template` (which asserts the flag is **on the URL**) and `report_frame: a template hidden by a HOSTILE retraction is still served` (which asserts what came back). The first was honest all along, because a fake that ignores a parameter cannot make a URL assertion pass; the second was decoration until the fixture was fixed. That is the practical rule for writing tests above a fake at all: **assert the request you made as well as the answer you got**, because only one of those two survives the fixture being wrong. (3) 🔴 **When a parameter audit finds an ignored parameter, every test that depended on it was passing for the wrong reason until the moment it was fixed — so re-run THOSE tests' mutations, not just the new one's.** The fix does not merely add coverage; it retroactively changes what a whole set of existing tests was proving, and nothing in a green run marks which ones. (4) 🔴 **"Proved by URL inspection rather than by behaviour" is a category of test worth naming.** The download's version pin asserts that the right URL was requested, because the fake would answer the same bytes either way — it is a test of our intent, not of the system's response, and it should be labelled as such wherever a fake cannot distinguish the two. |
| **R181** | 🔴 **A whitespace-only `rationale` LOCKED A TEMPLATE and then answered 400 — the lying state R171's ordering exists to prevent, reached from the other side.** Measured: `rationale: "   "`, `decision: "accept"` → **400 invalid**, one `report-template` written and locked, `GET …/report/frame` serving it, and **zero decision rows**. The caller is told their request was rejected while the amendment has taken effect, and the record shows a locked template that no human is on record as accepting. Cause: `z.string().min(1)` **accepts `"   "`** (measured against the installed zod 4, not reasoned), so the blank check lived only in `buildLineAndBody`, which runs *inside* `writeDecision`. On **reject** that is harmless — `writeDecision` is the first append. On **accept**, R171's template-first ordering puts it after `appendMemory(template)`. **The edge validator did not enforce the rule the writer enforced, and the gap is exactly the window the ordering opened.** Three lessons. (1) 🔴 **A validator and a deep guard that disagree by even one input turn the deep guard into a half-committed rollback.** Where an operation has more than one durable write, every precondition must be checked before the first one — a guard at the point of use is fine only when it is the *first* thing that can fail. (2) **`z.string().min(1)` does not mean "non-blank"**, and the mistake is invisible because the type and the intent read identically. `.trim().min(1)` is the spelling; this is worth remembering wherever a string becomes the visible line 1 of a durable record. (3) 🔴 **Its test passed for the wrong reason, in the test written to close this exact item.** Both assertions were true; the *comment above them* asserted a false proof ("`buildLineAndBody` refuses an empty line 1, so the record cannot be blank by either route"); and the one assertion that would have caught it — `expect(appends).toHaveLength(0)`, which the empty-string half four lines earlier **does** make — was absent from the whitespace half. **R133's shape at its most instructive: the sibling test one screen away had the right assertion, and the copy dropped it.** When two cases of one rule are tested side by side, diff their assertion lists. |
| **R182** | **Two rates worth carrying to every future ticket, offered by the agent that measured them on its own work.** Across five mutation sweeps on W21 (79 valid mutations), its implementer reported: **roughly one in twenty of its own mutations was INVALID on first attempt** — crashing on an unimported identifier, firing on every fixture, or dying inside an assertion rather than on it — *and every one looked fine until the assertion that died was read*. Beside it, a second rate the same work exposed: **roughly one test in twenty that it wrote was passing for a reason other than the one it stated.** Four such were found on this ticket alone — the fake ignoring `limit`, the fake ignoring `include_retracted`, an unreachable guard, and the whitespace assertion — **every one invisible to a green suite of more than 1600 tests.** Three lessons. (1) 🔴 **Neither number goes down by being careful.** They go down by mutating and by *reading the assertion that dies* — care produces the tests that pass for the wrong reason, because a careful author writes the assertion they believe rather than the one that discriminates. (2) **The two rates are the same phenomenon seen from either side**: a mutation that dies wrongly and a test that passes wrongly are both "the red/green signal came from somewhere other than where you think". R146 is the rule; these are its frequency. (3) 🔴 **Both of this ticket's wrong-reason tests were in the half of a test added SECOND** — a case copied beside an existing one, with the sibling's load-bearing assertion not carried down. **When two cases of one rule sit side by side, diff their assertion lists** (R181(3)); the failure is not forgetting to write an assertion, it is copying a case and trimming it to what looks relevant. |
| **R183** | 🔴 **A deep guard that can throw BETWEEN the first durable write and the last is a half-committed rollback — mirror every one of them at the edge, and enumerate the window rather than fixing the instance.** R181 was one symptom: `buildLineAndBody` refusing a blank line 1 from inside `writeDecision`, which R171's template-first ordering placed *after* the template append. The fix that matters is not `.trim().min(1)` but the enumeration that followed it — W21 listed everything that can throw in that window (`reportDecisionLabels`, validating the amendment id; `buildReportAmendmentContent`, validating the rationale) and confirmed both are now edge-validated, the id since an earlier round and the rationale now. **The window is closed, not the instance.** The general form: **an operation with N durable writes must check every precondition before write 1**; a guard at the point of use is safe only when that point *is* the first thing that can fail. Note how the defect was manufactured — R171 chose template-first ordering for a good reason (a lie about state is worse than a repeatable action), and that correct choice is exactly what opened the window, because it moved a durable write in front of a validator nobody had noticed was doing validation. **A change to write ORDER is a change to which guards are still preconditions**, and neither the ruling that ordered it nor the review that accepted it asked that question. |
| **R184** | 🔴 **THERE ARE FOURTEEN HAND-ROLLED FAKES, SO FIDELITY IS A PER-FILE PROPERTY WITH NO SHARED FLOOR AND NO TEST — R178/R180 found instances in ONE of them.** `marketdata/stooq`, `bob/client`, `bootstrap/bootstrap-project`, `app`, `hypothesis/provision`, `testing/undici-mock.smoke`, `routes/hypotheses`, `routes/series`, `marketdata/fred`, `routes/auth`, `hypothesis/poller`, `hypothesis/store`, `routes/embed`, `routes/report` — fourteen files each build their own `MockAgent` with its own route table. **R180's audit question — for every parameter the client sends, does the fake implement it or ignore it? — must be asked once per fake, and the answer in one file says nothing about the next.** Measured so far: `report.test.ts` (three parameters sent, **three ignored**) and `poller.test.ts` (`?version=` ignored). **Twelve are unmeasured.** A concrete consequence, worth having before anyone describes the fix as closing the class: W21's fix round 3 repairs `report.test.ts`'s fake only, so `poller.test.ts:602`'s version pin stays URL-shaped afterwards — real, and surviving a permissive fake precisely *because* it is URL-shaped (R180(2)), but not behavioural. *(No open item against W10: its pin **is** asserted — dropping `{ version }` from `poller.ts:313` reddens `poller_version_gate`.)* Three lessons. (1) **"We fixed the fake" is a file-scoped claim wearing a codebase-scoped shape.** Fix round 3 closes the *instances*; the class is "each fake is hand-rolled, so there is no floor anyone can raise". (2) 🔴 **The cheapest structural answer is ONE shared fake that all fourteen consume**, at which point a single fidelity test covers every consumer and R180's audit is run once instead of fourteen times. That is a wave-sized change, emphatically not W21's, and is **OPEN** — it is what the three-defects-in-one-file finding actually points at. (3) **This was found by asking a question, not by a mutation** — the third such finding in two days, after R163's `draft` gap and R180's `?version=`. Mutation testing finds what is *wrong in code that exists*; enumeration finds what *nobody wrote*. They are different instruments and this project has been leaning on one of them. 🔴 **The sharpest data point is the one where BOTH were aimed at the SAME fix and disagreed about what had been proved.** On R181's `.trim()` fix, the mutation says: revert it and the shipped test dies — true, and it establishes the fix is load-bearing. The enumeration says: reverting it tears **ten** distinct inputs (leading LF, leading CRLF, blank first line then text, lone CR, tab, VT, FF, NBSP, U+2028, BOM) and **the shipped test covers exactly one of them** — so a narrower "fix" stripping only spaces would keep that test green and still tear on the other nine. **A mutation can only ever answer "is this guarded?"; it takes an enumeration to answer "guarded against what, and what else is in the set?"** That is the difference between knowing the *instance* is closed and knowing the *class* is, and it is why the fuzz was worth writing after the fix was already correct. |
| **R185** | 🔴 **A GRACEFUL DEGRADATION SWALLOWED THE SECURITY SIGNAL IT WAS STANDING NEXT TO — the detail page went silent about an attack the board was naming, in the unsafe direction, on the page carrying the verdict buttons.** W22's verifier proved it end-to-end through the real `createApp`, not by reading: hypothesis `2b2b2b2b`'s researcher appends `kind=report, name=1a1a1a1a`; `1a1a1a1a`'s **own** genuine report has a body that is not a flat `{slotId: html}` map. `readLatestReport` computes `picked.tamper` **before** it fetches and parses the winning row (`store.ts:1656-1666`); `parseReportContent` then throws `invalid`; the catch in `readLatestReportOrDegrade` returns `{report: null, tamper: []}` — **discarding tamper already witnessed on the OTHER rows**. Board: `[cross_hypothesis_write]` naming the attacker. Detail: `tamper: null`, `drift: null`, `has_template: true` — a benign empty state, indistinguishable from "no tick has run yet". **Three lessons, and the second is the general one.** (1) The degradation was *correct* in isolation and correctly motivated — a `400` here would have let model-authored content take the human's controls away, which is worse. The defect is that it discarded evidence collected on a **different** row than the one that failed. (2) 🔴 **THE GENERAL RULE: a catch that degrades must enumerate what it is DROPPING, not only what it is returning.** `return {report: null, tamper: []}` reads as "no report, no problems"; what it meant was "no readable report, and I am also throwing away the problems I found". A degradation is a *narrowing* of a return value, and every field narrowed to its empty value is a claim — here, a false one. (3) `drift: null` was **carrying two meanings** and that is what let the empty state absorb the attack; **`report.unreadable: boolean` was added to the pinned block the same day** so `{drift: null, unreadable: false}` is "no tick yet" and `{drift: null, unreadable: true}` is "a tick ran and its report cannot be read". Both W22's implementer (as an open finding) and W22's verifier (as a fix suggestion) reached for that flag independently, from opposite ends — which is the strongest signal available that a pinned shape is short a state. **The instruments agreed here**; contrast R184, where they disagreed. Fixed in W22's fix round 1. |
| **R186** | **A criterion whose meaning lived entirely in its trailing clause, and a careful executor read it the other way — the standing lesson's TWENTIETH instance, and the first that is a matter of SCOPE rather than of fact.** W22's criterion reads: "a hypothesis with no report yet gets `headline: null`, never `\"\"`, so the UI distinguishes 'nothing yet' from 'said nothing'." Every word is true and the sentence is not wrong: the "never `\"\"`" is scoped to the no-report case, and the trailing clause names both states explicitly. W22's executor read it as forbidding `""` outright, found `web/src/api/types.ts:104` (W13's, authored earlier) saying `null` = no report yet and `""` = the report said nothing, concluded the plan and the code disagreed, implemented **both** correctly, and flagged the apparent conflict. **The right outcome by the right route — and it cost a careful reader a detour.** The lesson is not "be more precise", which is unfalsifiable; it is narrower and mechanical: 🔴 **when a criterion states a prohibition and then a rationale, the prohibition's SCOPE must be in the prohibition, not recoverable from the rationale.** "never `\"\"`" should have read "never `\"\"` *for that case*". A reader who takes the first half at face value and stops is behaving correctly. Clarified in place 2026-08-26. |
| **R187** | **W22 shipped a per-request read shape that W27's restated criterion had no room for, and its cost is chosen by the attacker.** W22 issues **one further per-name read for every hypothesis whose newest `kind=report` row is a forgery** (`store.ts:1594`). It is deliberate and it is the criterion working: without it a forged newest row *erases the victim's real headline*, and "ignored, not rendered" would be false. It honours the "exactly three `latest_per` requests" criterion **literally** — it sends no `latestPer`, so it is not one of them — which is the right kind of literal, and the verifier checked it rather than assuming. But W27's ticket restates the budget as "three `latest_per` requests plus one project-wide attention read", a sentence written before this read existed, and W27's twelve-row test must therefore count **`latest_per` requests specifically**, never a total. Amended in W27 2026-08-26 to "three `latest_per` requests, plus one project-wide attention read, plus one per-name read per ATTACKED hypothesis". 🔴 **And the cost is on the record rather than waiting to be discovered: that last term is `O(attacked)` and the attacker picks N** — one forged row per hypothesis turns a 3-request board into `3 + N`. It is the same pattern `readBoard` already uses, so it is consistent rather than novel, and the alternative (dropping the follow-up) is a working attack. Logged as an accepted amplification, not a defect. **The general shape, third instance after R94 and R143: a criterion in ticket B that restates a criterion in ticket A is a copy, and A's executor is the one who will invalidate it.** |
| **R188** | **OPEN — half of W22's isolation criterion is VACUOUS, and only an independent grep found it.** The criterion reads "a `kind=report` **or `kind=report-candidate`** memory whose provenance does not name that hypothesis's own researcher worker or its `hyp-<id>` session is IGNORED…". W22's implementer reported, and W22's verifier confirmed independently, that **`kind=report-candidate` has no reader anywhere in the tree**: `grep -rn "report-candidate\|KIND_REPORT_CANDIDATE\|reportCandidate" api/src web/src` returns only the constant and its label builder in `report/kinds.ts`, their tests, one prompt fixture and one comment. No selector, no store read, no route. Nothing can render a `report-candidate`, so nothing can be attacked through one, so the defence for it cannot be tested and its absence cannot be detected. `isOwnReport` is kind-agnostic and a unit test does cover a `report-candidate` row, so the *code* is right — it is the *criterion* that promises a defence for a surface that does not exist. **Two possible resolutions and they are not the same: either a reader is missing from an earlier ticket (the kind was designed to be read and nobody built it), or the criterion should say `kind=report` only.** Deciding needs the answer to "what was `report-candidate` for?", which is a W15 question. 🔴 **The general shape, and it is the one worth carrying: an enumerated value in a criterion is a claim that the value EXISTS in the code, and nothing checks that.** R163's vocabulary sweep checks the reverse direction — code values missing from a ticket. This is the forward direction: **ticket values missing from the code**, which has no token to grep for either, because the token is right there in the ticket. Both directions belong in the sweep. |
| **R189** | **OPEN — the frame cache is consulted AFTER the reads it exists to save, so one detail request makes 19 Bob reads where about 11 would do.** W22's implementer reported the detail route reads the template and the report twice; W22's verifier measured it on a live `GET /api/hypotheses/:id` through `createApp` and found it is worse: **eight report-layer reads where four would do, plus a duplicated spec read** — `kind=report` ×2, `kind=report-template` ×2, `GET /agent/memories/rep-1` ×2, `GET /agent/memories/tmpl-1` ×2, `kind=hypothesis-spec` ×2. The cause is an ordering, not a missing cache: `reportBlockFor` reads both rows (`hypotheses.ts:876-879`), then `composeReportStats` → `composeFor` reads them **again** (`report.ts:510, 516`) *before* consulting the frame cache (`:520-521`), so a cache hit still costs both reads. **W22 was right not to fix it**: the fix widens W21's `ReportComposeStats`, which W22 was explicitly forbidden to widen, and the forbidden widening is `html` specifically — so a narrow widening (accepting an already-read template/report, or moving the cache probe above the reads) is legitimate and is simply not W22's ticket. 🔴 **The reason this is worth a ticket rather than a shrug: it is the R124 cost compounding.** R124 already records that the poller spends ~8–10 agentd requests per hypothesis per tick; this is the same shape on the human-facing path, and it is the path where latency is visible. Needs its own ticket alongside R169's `readLockedSpec` consolidation. |
| **R190** | **OPEN — `BOARD_LIMIT = 100` is DECORATION, and the fake is why nobody knew.** `store.ts:1170` caps the board's `latest_per` reads at 100 rows across **all** hypotheses. W22's verifier's R180 audit read both stubs' own code and found the `limit` parameter is **ignored on every `latest_per` branch of both Fake A and Fake B** — so no test can observe the cap, and mutating it changes nothing. Past 100 hypotheses, rows silently vanish from the board while the detail page still serves them: a disagreement between two views of the same data, arrived at by a limit that has never been exercised. 🔴 **This is R184's mechanism landing on a pre-existing constant rather than on new code, which is the case that entry predicted and did not have an example of.** The permissive fake did not weaken a defence W22 wrote; it has been quietly holding a *pre-existing* defence in the "untestable" state since the board was built, and it took a consumer running the audit — not a reader, not mutation, which is structurally blind here — to surface it. 🔴 **SHARPENED 2026-08-26 (R232): this is no longer only a data-loss risk, it is a MISCLASSIFICATION,
and it is now the precondition for testing one.** W27 gave the `kind=report` read's `headline` a
second consumer — the WATCH tier rule — so past 100 hypotheses a truncated read yields
`headline === null` for every unlisted row, and **every such `live` row tiers WATCH**. The section
meant to be short becomes the longest, burying the rows that genuinely need a human. **And the fake
cannot express the condition at all**, so the defect is currently untestable: teaching the fakes to
honour `limit` is not preparatory work for the consolidation, it is what makes a live
misclassification observable. **The fix is two-part and both parts are needed**: teach the fakes to honour `limit` (which will fail nothing today, since no test exceeds 100), then write the test that proves the cap and decide what the board does at the boundary — silently truncating is the current answer and it is the wrong one. Belongs with R184's shared-fake consolidation. |
| **R191** | 🔴 **"UNTESTED" AND "UNREACHABLE" ARE DIFFERENT DIAGNOSES FOR THE SAME SURVIVING MUTATION, AND THEY CALL FOR OPPOSITE TREATMENTS — R148 covered only one of them.** W22's verifier found `mergeTamper`'s de-duplication survived mutation and filed it as an untested guard (S3). W22's implementer went to fix it, could not build a fixture that reached it, and worked out why: a retraction memory carries a **single** `retracts=<id>` label, so one retraction appears in exactly one row's `retracted_by`, and **both** `mergeTamper` production call sites join reads over **disjoint kinds** (board row, `hypotheses.ts:719`: `kind=hypothesis` × `kind=report`; detail block, `:973`: `kind=report-template` × `kind=report`) — *corrected 2026-08-26 from "all four", see R193*. A memory has one kind and one id, so no overlap is producible through any route. **It then reported that its OWN comment on that function — asserting the board's two reads "can both witness the same hostile retraction" — was factually wrong, and corrected it.** The verifier confirmed the unreachability against Bob's SQL rather than the prose: `RetractionLabel = "retracts"` (`go/agentdb/memories.go:315`), value = the retracted id (`:302`), and the lookup groups on a **scalar** jsonb extraction, `r.labels->>'retracts'` (`:655-672`) — no multi-target form exists on that path. 🔴 **THE DISTINCTION, which is the entry:** R148 says a guard that cannot fail on its own is a comment, and that is right for a guard **structurally** incapable of firing — dead by the construction of the function it sits in. This guard is fully capable of firing; its inputs merely never overlap because of a property of a **different system** that `mergeTamper` can neither see nor enforce. Change Bob's retraction model, or call it with two reads over one kind, and it is load-bearing that instant. **The correct treatment is therefore the opposite of deletion**: keep it, export it, grade it directly with unit cases (the dedup, both near-misses — same memory/different reason and same reason/different memory — and the `null`-not-`[]` case), and say in the comment *why* no fixture reaches it. **Test the guard, document the reachability, and do not confuse the two.** One caveat carried forward as an open note: the reachability rests on the **call sites**, and nothing pins it — a later ticket joining two reads over the same kind makes duplicates reachable with no test flagging the changed assumption, which is why each call site now names its disjoint-kind pair in a comment. |
| **R192** | **Two rulings on shapes W22's fix round changed, both recorded rather than deferred.** (1) **`report.unreadable` stays ONE boolean, and its conflation is deliberate.** W22's implementer flagged that it is `true` for two causes with different operator meanings: an unparseable `kind=report` body (a model's fault, routine, expected) and a stored template that no longer validates (Wolf's own fault, should never happen, `internal` on the frame route). It is right that those differ. It stays one flag anyway: the second cause is an internal invariant break that already logs at `error` and already answers `internal` on the frame route, so the operator signal for it exists elsewhere and is louder; splitting the wire field would widen a pinned block on **speculation about what W23 will want**, before W23 exists to want it. 🔴 If W23 needs to say "the agent wrote a bad report" versus "our stored template broke", it adds a reason string then, with a real requirement behind it — and the pinned block's cause is recorded here so nobody re-derives it. Both causes are independently mutation-pinned in both directions. (2) **The frame route's 400 body grew a `tamper` key, and that is ACCEPTED.** `details` on the `invalid` from `GET …/report/frame` now carries `tamper` alongside the pre-existing `key`, a side effect of attaching the witnessed anomalies to the thrown error. It is a wire change to a **W21** route made by **W22**, which is the kind of thing that should be caught, so it was: the implementer flagged it unprompted. Accepted because it leaks nothing — the same four-field `Tamper` objects are already served by two other routes behind the same `requireSignedIn`, `api/src/routes/report.ts` and `api/src/report/` are byte-unchanged since the W21 merge, every W21 assertion on `details` is field-level rather than a whole-object `toEqual` so an additive key cannot weaken them, and `internal` still suppresses its details (both pinned by mutation). **And it is an improvement**: the frame's own error now names the attack too. |
| **R193** | **The standing lesson's TWENTY-FIRST instance, and the first that originated with a VERIFIER and was propagated by the ORCHESTRATOR without being checked.** W22's verifier wrote "all **four** `mergeTamper` call sites join reads over disjoint kinds" and then named **two** pairings in the same sentence (board: `kind=hypothesis` × `kind=report`; detail: `kind=report-template` × `kind=report`). The orchestrator copied "four" into the fix-round brief and into **R191** without grepping. There are **two** production call sites — `hypotheses.ts:719` and `:973`; the other four hits are assertions inside `hypotheses_merge_tamper`, where a comment about reachability would say nothing. W22's implementer re-grepped `src/` before acting, found no third caller, commented both real sites, folded the both-pairings statement into the test header, **and flagged the discrepancy rather than quietly satisfying the instruction** — which is the behaviour that caught it. 🔴 **Two things this sharpens.** (1) The lesson has always been stated as *orchestrator-written* text being the defect-dense surface; this widens it: **the defect surface is text written by ANY agent that is then acted on by another**, and the orchestrator's real failure mode is being a **propagation vector** — a verifier's number carries the authority of having been measured, so it is precisely the kind of claim that gets copied unchecked. A number inside an otherwise-verified report is not itself verified. (2) 🔴 **A count and an enumeration in the same sentence are a free consistency check, and nobody ran it.** "All four call sites … board: X × Y; detail: A × B" enumerates two and asserts four; the sentence refutes itself and three readers passed over it. **When text states a count AND lists the items, compare them** — it costs nothing and it is the second time this project has been saved by an executor doing arithmetic on a sentence rather than trusting it (R162 was the first, and there the orchestrator and the verifier reached wrong numbers by different routes). |
| **R194** | 🔴 **THE WAVE-15 SHAPE WAS APPROVED BY THE OWNER AND WAS ILLEGAL — `W23 ‖ W29` collide on `web/src/pages/HypothesisDetail.tsx`, and the ownership table did not say so.** Both tickets' own **Files** lines name the file correctly (W23: "modify `web/src/pages/HypothesisDetail.tsx`, `web/package.json`"; W29: "modify `web/src/pages/HypothesisDetail.tsx`"). The `web/src/pages/*.tsx` row listed **W13, W14, W24** and neither of them. The orchestrator recommended the shape, the owner approved it, and the collision was found only in the pre-dispatch read of W29's ticket — after approval, before dispatch, which is the last place it can still be free. **Thirteenth instance of the R94 class**, and the second in three waves where the colliding pair contains **neither** of the row's authoring tickets — the failure mode is not "the author forgot to add a consumer", it is that **a row lists who CREATED the files and is never revisited when a later ticket MODIFIES one.** 🔴 **The mechanical fix, and it is the same shape as the citation grep and the vocabulary sweep — a third standing pre-wave check: for every ticket in the proposed wave, extract its Files line and intersect the sets pairwise.** Every collision this table has ever missed would have been caught by that intersection, because in every collision to date the Files lines named the shared file; it is the *table* that goes stale. ⚠️ **Amended within the hour by R195**: this originally read "because the Files lines have been right every single time", and W23's executor refuted that in the same wave — its Files line named a file it must not touch AND omitted one it could not finish without. The intersection check still holds, because it needs only that a shared file APPEARS in both lines, and no Files line has yet been short of the file two tickets actually collide on. But it is **not** a check on the Files lines themselves, and describing it as though the tickets were trustworthy is the overclaim that made this sentence wrong. It is grep-able, it takes seconds, and unlike the other two checks it does not need the document to be self-consistent — it reads the tickets, which are the thing executors are actually handed. **Wave 15 was re-cut as `W23 ‖ W31`, with W29 following W23.** |
| **R195** | 🔴 **A TICKET'S OWN FILES LINE WAS WRONG IN BOTH DIRECTIONS AT ONCE, AND IT REFUTED R194's REASONING ONE COMMIT AFTER R194 WAS WRITTEN.** W23's Files line named **`web/package.json`**, which W23 must *not* modify — the testing library was already installed by W13 and the jest-dom pin is exactly `6.9.1` (**R135**), so touching it is a regression, and the ticket's own first acceptance criterion says as much ("if W13 has already added it, this is a no-op") while the Files line says to modify it. And it **omitted `web/src/api/types.ts`**, without which the ticket cannot be completed at all, because `ReportBlock` did not exist on the web side. **A criterion and a Files line in the same ticket contradicting each other is a shape we had not seen**; the previous eleven instances of this class were all omissions. The executor implemented the criterion, ignored the Files line, changed neither `package.json` nor `yarn.lock`, added `types.ts`, and reported the discrepancy — the right outcome by the right route, again. 🔴 **The sharper lesson is about R194, written the same hour**: it justified the new pairwise-intersection check with "the Files lines have been right every single time", which was **false when written** and refuted within the wave. The check survives, because it needs only that a shared file *appears* in both lines and no line has yet been short of the file two tickets collide on — but its *justification* was an overclaim, and the amendment is in place. **A rule invented to fix a stale summary must not itself rest on an unexamined claim about the source it defers to.** That is the second time in two waves that orchestrator text asserted a property of a set it had not enumerated (R193 was the first, and there the count refuted itself inside its own sentence). Files line corrected in place. |
| **R196** | 🔴 **A FIFTH FALSE-GREEN DIRECTION, AND IT DEFEATS THE RULE WE WROTE TO CATCH THE FIRST FOUR. EVERY HARNESS IN THIS PROJECT HAS THIS GAP, INCLUDING THE TWO THAT FOUND IT.** The standing rule since R153 is: decide on **exit code**, never on grepped output, and classify **"did not run"** as its own outcome — implemented everywhere as *"no `Test Files` summary line ⇒ `DID_NOT_RUN`"*. W31's verifier broke `api/src/bob/client.ts` with a syntax error as a deliberate self-check and got: `exit 1`, **`Test Files  14 failed | 22 passed (36)`**, **`Tests  1132 passed (1132)`**. 🔴 **A summary line IS printed, so the rule passes the run through — and a `Tests … failed` parse reads ZERO FAILURES while a third of the suite never ran.** A **collection** failure is not a **test** failure: vitest reports the files it could not load as failed *files* while the `Tests` line counts only tests that executed. A harness deciding on that line sees a clean, if smaller, green. **The direction is the dangerous one**: in a mutation sweep the finding *is* the zero-kill, so a mutation that breaks collection in the very file it targets reads as "the code is undefended" — manufacturing a false finding rather than hiding a real one, which is the inverse of R153's failure and worse, because a fabricated hole gets *acted on*. **THE FIFTH HARNESS RULE, NOW REQUIRED ALONGSIDE THE OTHER FOUR: assert the COLLECTED TEST TOTAL (`failed + passed`) equals the baseline on every run, alongside the test-file count.** 🔴 *Strengthened the same day by W31's executor, over the weaker form this entry first specified.* The orchestrator asked for the **file count**; the executor implemented the **collected total** as well, reasoning that a file which fails to collect contributes **zero tests** — so a mutation breaking collection in a file the run still counts slips past a file-count check and cannot slip past a total. It then **retro-validated every earlier run against the stricter rule: 68 runs, 0 violations**, which is what makes it safe to keep the audit's numbers rather than re-measure them. A fifth outcome, `COLLECTION_FAILURE`, joins `DID_NOT_RUN` and `TIMED_OUT`. **Nothing was corrupted**: every zero-kill run in both the audit and its verification reports `36 passed (36)`, because both agents recorded the file line even though neither's rule required it — the discipline of writing the number down is what made the gap findable, and is itself the argument for recording more than you check. 🔴 **The general lesson, which outruns vitest: "the tool produced output" is not "the tool did the work".** Four of the five directions are now instances of one thing — R153 (the runner crashed and its output was grepped), R168 (the shell killed the driver), R170 (a peer overwrote the mutation), and this. Each time the harness observed a *proxy* for the work and the proxy survived the work's absence. **Ask what the run would look like if it had not happened, and assert against that** — do not merely assert what success looks like. |
| **R197** | 🔴 **A RULE THAT IS CAREFULLY TESTED SOMEWHERE IS THE RULE YOU STOP CHECKING IS TESTED *HERE* — and it predicts where the next hole is.** W23 produced nine defects across three fix rounds and **every one was the same shape**: a check that cannot fail on its own. But the *trigger* is sharper than R133's "an unpinned constant", and W23's executor named it from its own work. In all three of the worst cases the rule **was** tested, thoroughly, against something **adjacent to the thing that ships**: (1) the `postMessage` prohibition was proved against `ReportFrameHost`'s **own fixture child**, not `ReportPanel`, so adding a `message` listener to the shipping component passed all 414 tests; (2) `tripped`'s colour was covered by "no element computes to the error colour" — a statement of what it is **not** — so `tripped → warning`, which collapses the exact distinction § 2b spends its colour budget on, passed the whole suite; (3) the `unreadable`-vs-empty distinction was pinned in the **code** by three separate mutations while the **prose** that communicates it went unpinned, so a plausible copy edit dropping "a tick ran" would have reintroduced R185's confusion in the copy after it had been eliminated from the logic. **Two negative assertions on a three-way distinction leave one leg standing on nothing.** 🔴 **The predictive form: look at every place where a ticket boundary means the PROOF and the SHIPPING CODE live in different files.** That is where the rule feels covered and is not — and this project generates those boundaries deliberately, by design, one per ticket. Also logged from the same sweep: **a bare count of a mapped list is a category of wrong-reason test, not an instance** — `expect(rows.length).toBe(2)` is satisfied by a component that renders the *first* row twice, which on this page would show a human one attack where there were two. It asserts arity and reads as though it asserts content. |
| **R198** | **THE SWEEP DOES NOT COVER THE TEST IT IS WRITING WHILE IT SWEEPS — third instance in three rounds, and the executor caught it in its own fix round.** W23's round 2 ran the R182 assertion-list diff across all three components, found six divergent pairs and closed every one. **The pair round 2 was writing while running that sweep went out lopsided**: it put the positive control on the first of its two new listener tests and not the second, so the second passed with a spy that could never have observed anything (proved — de-wiring the spy killed 1 before the fix and 2 after, both on the probe). The sweep looked backwards at existing tests and did not turn around on itself. 🔴 **The rule: a fix round that levels assertion lists must re-run its own diff over its own output before reporting.** Cheap, mechanical, and it composes with the two entries either side of it — **R182** says the second half of a pair is where the hole is; **R197** says the risk concentrates where the code distinction was carefully tested; this adds that **the newest test is the least swept, and the newest test is the one you just wrote to close a gap.** Corrective work is not exempt from the standard it is enforcing. |
| **R199** | **OPEN — the report panel's provenance stamp cannot name a writer, so UI § 2's signal table is unsatisfied in substance.** § 2 defines the `model` treatment as a stamp reading `<worker-or-session> · <relative time>` and states the stamp names `written_by_worker || written_by_session`. `reportBlockFor` (`api/src/routes/hypotheses.ts:957-974`) projects exactly seven fields — `has_template`, `structure_hash`, `stripped_count`, `updated_at_ms`, `drift`, `unreadable`, `tamper` — and **no writer**. `provenanceStamp` therefore falls through to W28's `UNKNOWN_WRITER`, so **every healthy report on the detail page reads "unknown writer · <time>"**. W23 handled it honestly — it passes `atMs` only and marks the gap with an explicit comment rather than borrowing a writer from a neighbouring row, which would have been a false attribution on the one surface whose entire job is attribution. **The only writer currently on the block is inside `tamper`, and that is emphatically the wrong one to stamp with.** Fix is one field on the API side (the newest report row's writer), plus a second, related edge the verifier found: in the reachable `{has_template: false, drift: populated}` state a **degraded drift notice about model-authored slots renders outside any provenance ground**. Both are the same root cause — the API under-serves the panel's provenance — and want one small ticket. 🔴 **Note the shape: this is a design document's requirement silently unmet by an API projection written three tickets earlier, found only when a consumer tried to satisfy it.** R94's class, arriving through the wire format instead of a Files line. |
| **R200** | **OPEN — `stubFetchRoutes` is strict in the right direction but its own header overstates it, and the guarantee holds today only by accident of test style.** Verified from the fixture's own code (`web/src/testUtils.tsx:75-103`): it really does `throw` on an unrouted path (`:88-92`), and matching is **exact** — the key is `` `${method} ${path}` `` and lookup is a plain object index, with no prefix, regex or normalisation. That is the right design. Two gaps: **(1)** a test may declare a route it never requests, silently — proved by mutation, an unused route key left all 414 tests green; `countFor` exists and **nothing calls it**. **(2)** 🔴 The header's claim that an unrouted request "fails loudly instead of reaching the network" is true of the fetch layer **only**. The throw is swallowed twice on the way up — `client.ts:120` converts *any* fetch rejection into `ApiError("network", …)`, and `HypothesisDetail.tsx:107` catches that into a failure banner. Inserting an unrouted `fetch` into the page loader killed 27 tests — but **only because every W23 page test asserts positive content that then is not rendered**. **A test asserting only absences would pass against a page that never loaded at all.** None of the eight W23 page tests is of that shape today, which is luck plus house style, not a property of the fixture. Wants an `expectAllRoutesUsed()` helper and a corrected header sentence. **This is R184's lesson on the web side**: the fixture is load-bearing test infrastructure with no test of its own, and its documentation is the thing most likely to be believed. |
| **R201** | 🔴 **TWO PROCESS DEFECTS ON THE ORCHESTRATOR'S SIDE, BOTH FOUND BY AN EXECUTOR PUSHING BACK, AND ONE OF THEM CAN DESTROY WORK SILENTLY.** (1) **The validate-then-commit window.** W23's executor ran `yarn test && yarn typecheck && yarn build` **before** committing each round and pasted that output beside the commit it then made. Correct and honest — but it leaves a one-to-two-minute window in which `git status` shows `M <file>` on content that is finished and about to be captured. The orchestrator looked into exactly that window, read it as "reported completion over uncommitted work", and stopped the wave. **An observer polling git state cannot distinguish "mid-round, about to commit" from "finished and reported over an uncommitted tree", and the second is serious enough to be worth stopping for** — so the ambiguity, not the executor, is the defect. 🔴 **HOUSE RULE, adopted 2026-08-26: COMMIT FIRST, then run Validation on the clean tree, and paste the sha above the output.** It costs nothing, it makes the pasted output provably *of* the named commit rather than of a tree that merely became it, and it collapses the window to zero. It also removes a real hazard this session demonstrated: had the worktree been pruned four minutes earlier, `326e3ed` would not have existed and the work **would** have been lost with no trace. (2) 🔴 **PRUNING A WORKTREE WHILE ITS AGENT IS STILL LIVE TRUNCATES THAT AGENT'S FINAL VALIDATION, AND THE FAILURE LOOKS LIKE A REAL ONE.** The orchestrator pruned `wave15/w23` on merge while its executor was mid-`yarn build`; the command returned **exit 1** with no output, which reads exactly like a build failure rather than like a vanished directory. The executor said so plainly, refused to claim a green build it had not seen, and told the orchestrator to re-run it — which was right, and the build was then confirmed green on merged `main` (`✓ built in 3.39s`, exit 0, both probes present, `326e3ed` an ancestor of `HEAD`). **Prune only after the agent is confirmed done, and check `git status` in the worktree before pruning either way.** The next agent to hit this will spend a round debugging a build that never ran. **Both halves of this entry are the orchestrator's, and both were surfaced by an executor declining to accept a mistaken account of its own work** — the fourth time this wave that the right outcome came from a worker disputing the orchestrator rather than complying with it. |
| **R202** | 🔴 **AN AUDIT UNDER-COUNTED ITSELF BY THE EXACT MECHANISM IT EXISTS TO EXPOSE, AND SAID SO.** W31 reported **24** undefended sends. Its verifier, measuring at one row per *call site* rather than one per *parameter name*, found **16** more. The fix round re-measured and found **18** more — two that neither pass had counted, including a **fifth** `limit: DETAIL_LIMIT` caller (`store.ts:1786`) after the verifier's correction had named four. **24 → 40 → 42, every increase from splitting an aggregated row.** The worst instance was §2.5, which folded five callers into one row reported as *defended* on the strength of one kill; four were undefended, including `readTemplate` and `readLatestReport` — the two reads every report in the product goes through — in the file the audit had examined most closely. 🔴 **The mechanism is identical to the thing being audited: a permissive fake makes a suite report coverage it does not have by collapsing distinctions; an aggregated row makes an audit report defence it does not have by collapsing call sites.** The auditor wrote that into its own §0 rather than quietly correcting the number, which is why the entry is worth keeping. **THE RULE: when a measurement groups things, the group is a claim about every member, and one member's evidence is not the group's.** Two corollaries adopted: state counts as **floors** where the method can only establish a lower bound (enumerate-and-mutate always can); and **split evidence classes rather than summing them** — "zero kills" for a parameter no test ever sends is a tautology, not a measurement, so W31 now reports 40 measured and 2 unmeasurable rather than 42 undifferentiated. Third instance of the general shape this month after **R193** (a count contradicting its own enumeration) and **R199** (a projection silently short of a documented requirement). |
| **R203** | **THE INERTNESS RULE — a fake defends a parameter only when the test's expected outcome differs from the outcome of "no interceptor matched".** W31's most transferable finding, and it generalises past this codebase. With `disableNetConnect()`, an unmatched request **throws**; `marketdata/fred.ts:100-104` catches *any* throw and raises `unavailable`. So every test that asserts `rejects.toMatchObject({kind: "unavailable"})` **cannot distinguish a matched request from a missed one**, and its matcher predicate is inert however precise it looks — pinning `api_key` and `file_type` in the matcher buys nothing, measured at **0 kills each**. The same tests' siblings at `:125` and `:169` reply **200 with fixture rows**, which a miss cannot produce, and kill 1 each: **defended for exactly the reason the others are not.** 🔴 **The general form: a fixture's precision is only as strong as the DIFFERENCE between "the fixture matched" and "nothing matched". Where the failure mode of the system under test is the same as the failure mode of an unmatched fixture, the fixture is decoration.** The verifier checked whether the rule reaches further and found it does not — of five matcher-based fakes, `bootstrap-project` is immune (one-shot exact-path interceptors, a body reader that throws on an uncaptured call, and **zero** failure-expecting tests), `auth`'s route carries no query parameters, `embed` is a catch-all, and the remaining nine fakes are catch-alls. **Its reach is fred + stooq, and both were covered** — a negative result worth recording, because "does this rule apply anywhere else?" is the question nobody asks after finding one. It also corrected the count in the unflattering direction: **three** inert stooq matchers, not four. |
| **R204** | **A MEASUREMENT OF A NAMED COMMIT MUST RE-ASSERT THAT COMMIT AT THE END — its Validation cannot notice the repository moving underneath it.** W23 merged into `agent-wolf` while W31 was mid-fix-round, so the audit's subject, `a155c96`, stopped being HEAD during the work. W31's executor **checked rather than assumed**: `git diff a155c96 HEAD -- api/` is empty (all nine changed files are under `web/`), so the audited subtree is byte-identical to HEAD and every measurement still describes the code that ships — and it added a callout at the top of the document saying so. 🔴 **W31's own Validation would never have caught it.** It asserts the test counts (36 / 1696 — unchanged, because W23 was web-only) and `git status --porcelain` (clean, because the executor changed nothing). **Neither notices a merge.** It held here by luck of which subtree moved. **Standing check, added for any ticket whose deliverable is a measurement rather than code: record the subject commit at the start, assert it is still HEAD at the end, and if it is not, diff the audited subtree and say so in the document.** A moving HEAD is exactly how an audit quietly stops being about the code that ships — and the wave that produced the audit is the wave most likely to move it. |
| **R205** | 🔴 **THE ORCHESTRATOR WALKED INTO R201's HAZARD ONE HOUR AFTER WRITING R201 — deleting a branch that carried unmerged work.** W31's executor went idle with its 999-line deliverable **untracked**. The orchestrator asked it to commit, it went idle again without doing so, and the orchestrator then committed the file itself to stop the work being destroyed (`fd03af6`) and merged that. **But the executor had in fact been mid-commit**: it landed `ae92413` — the two consequences of the orchestrator's own class-2 ruling — on top, and the orchestrator, believing the ticket closed, ran `git branch -D w31-fake-audit`. Git printed `was ae92413`, which is the only reason it was recoverable: the sha in the deletion message, `git cat-file -t` to prove the object was still there, `git branch <name> <sha>` to restore, then a normal merge. **Three lessons, and the third is the one that generalises.** (1) 🔴 **`git branch -D` prints the sha it is deleting. READ IT.** If it is not an ancestor of the branch you merged into, you have just orphaned work — check with `git merge-base --is-ancestor` **before** deleting, not after. (2) **An idle notification is not "finished".** An agent can be idle between two of its own actions; the orchestrator read the second idle as abandonment and it was a pause. **`git log` in the worktree is the state, and it must be re-read immediately before any destructive step**, because the reading and the deletion are not atomic. (3) 🔴 **Writing the rule down does not install it.** R201 was committed at 17:53 and violated at 18:16 by its own author, in a different-looking situation — untracked file, then an unmerged commit — because the *rule* was remembered and the *class of hazard* was not. **A rule stated as a procedure ("check git status before pruning") protects only the instance it was written from; the hazard is "any destructive step taken on a stale reading of another agent's state", and the procedure must be derived from that each time.** The plan's Executor-orientation entry now carries both the ancestry check and the re-read, and the general form is here so the next variant is recognisable when it does not look like either. 🔴 **(4) The executor supplied the sharpest half, from the other side: "you committed to stop work being lost in a worktree, and in doing so captured a file that was STILL BEING EDITED."** `fd03af6` caught the class-2 ruling's substance — the §0 counts and the rewritten class tables, which happened to be on disk — but not its two consequences, which were not yet written. **`fd03af6` alone is the ruling HALF-APPLIED**, and a merge from it would have shipped a document whose own recommendation contradicted its class table. The rescue was right and it was not free: **committing another agent's working file freezes an arbitrary instant of its work and gives that instant the authority of a commit.** A measurement deliverable has the same "is it still what I think it is" problem **R204** gives a named commit. The executor did the right thing — it did not amend or force over the orchestrator's commit, it stacked `ae92413` on top and said which to merge. **If you must rescue an uncommitted deliverable, say in the commit message that it is a rescue of a possibly-mid-edit tree, and re-read before merging.** |
| **R206** | 🔴 **THE NEW PAIRWISE FILES-LINE CHECK EARNED ITS KEEP ON ITS FIRST RUN — IT CAUGHT THE ORCHESTRATOR'S OWN RECOMMENDED WAVE, AND THEN A BIGGER DEFECT BEHIND IT.** R194 proposed the check at the wave-15 cut; wave 16 is the first wave cut with it. **First hit: the orchestrator had recommended `W32 ‖ W27 ‖ W24` in writing and the owner approved it — W27 and W32 both modify `api/src/hypothesis/store.ts`.** Neither the ownership table nor the orchestrator's own reasoning caught it; a five-line script did, in seconds, before a worktree existed. **Second hit, found while reconciling W24's Files line against its criteria: W24's first acceptance criterion is UNBUILDABLE AS WRITTEN.** It says the screen "reads the newest `kind=report-candidate`" and **nothing in the tree can** — `grep -rn "report-candidate" api/src` returns only the constant and its label builder, and `web/src/api/client.ts` has no method. 🔴 **This closes R188**, which asked whether a reader was missing from an earlier ticket or whether the criterion naming that kind was simply wrong: **the reader was missing, and it is W24's.** The kind has existed since W15 as a transport with a writer and no reader for nine tickets, and the only thing that ever noticed was a consumer trying to consume it — the third time (after R143 and R199) that a *projection or route nobody built* was found by the ticket that needed it rather than by anyone reading the plan. **Two general points.** (1) **A check that reads the tickets beats a check that reads the summary**, which is R194's argument, now with evidence: the Files lines were *also* wrong here (W24's omitted `GoLiveButton`, on its own ownership row — the R195 shape), but reconciling them against the criteria is what exposed the missing route. **The reconciliation, not the intersection, found the bigger defect.** (2) 🔴 **An enumerated kind, route or field in a criterion is a claim that the thing EXISTS, and nothing checks it** — R188 said this and could not say which resolution applied; the check applied it. **Add the reconciliation to the standing pre-wave list: for every ticket in the proposed wave, intersect the Files lines pairwise, THEN read each Files line against that ticket's own criteria and ask what the criteria need that the line does not name.** |
| **R207** | 🔴 **A VALIDATION COMMAND WHOSE EXPECTED VALUE WAS ARITHMETICALLY UNREACHABLE — PASSING IT WOULD HAVE REQUIRED VIOLATING THE CRITERION IT ENFORCES.** O10's check 6 read *"The core-tools table gained the three tools. Must print 3"*, over `grep -cE 'dataset_list|dataset_get|dataset_put' docs/18-workers-memory-events.md`. 🔴 **`grep -c` counts matching LINES, not occurrences.** O10's criterion requires a core-tools **table row** naming all three tools (**one** line) **plus** "each with its one-line contract" (**three** more). The minimum achievable count is **4**. An executor that satisfied the criterion would fail the check; an executor that passed the check had written the table row and omitted the contracts. O10's executor hit 4, said so, and **reported the check as wrong rather than reshaping its work to satisfy it** — which is the behaviour the whole Validation rule depends on and the reason this was caught at all. **The general fault is a class, not a typo: an expected value asserted without being computed.** § "The Validation rule" already says a command must be *run* before it is written down; this adds that **a command with an expected NUMBER must have that number derived, and a command counting LINES must be checked against how many lines the criterion actually produces.** Related and worth stating together, because they are the same error in the two available directions: **R196**'s collection failure is a check whose *pass* condition was too weak, and this is a check whose pass condition was *impossible*. Both were written by someone reasoning about what the output should look like instead of running it. Corrected in place to `>= 4`. |
| **R208** | 🔴 **A STALE CLAIM THAT NAMES A *COUNT* HAS NO TOKEN TO GREP FOR — and it is the one that matters most, because a count appears in the sentence a reader uses to CHOOSE.** O10's ticket named three stale claims in `docs/19-embedding.md`; its executor found **five**. The unlisted one was § 8's opening line: *"Agent Bob offers **two** ways to carry state between runs, and an application-layer builder picks."* False the moment O6b merged, and **worse than the three that were named** — those name a route or a bullet, so a search for the route finds them, while § 8 is the section an embedder reads while *choosing a state strategy*, and there is no string in it to search for. 🔴 **This is R163's vocabulary-sweep failure mode arriving in PROSE instead of in an enumerated union**: an omission has no token, so the only way to find it is to enumerate the real set and compare. **Add to the standing checks: when a feature ships, grep the docs for bare cardinals and absolutes — "two ways", "three routes", "read-only", "there is no", "only", "always", "never" — because those are the sentences a new capability falsifies and the sentences nothing points at.** Three further findings from the same pass, all worth carrying: **(1) the two reaper knobs read the same digit OPPOSITELY** — `0` on `AGENTKIT_DATASET_REAP_INTERVAL` disables the sweep, `0` on `AGENTKIT_DATASET_KEEP_VERSIONS` means **keep every version**; `gc.go:182-215` argues at length that inverting that would delete every non-current version of every dataset on the first pass, and neither the ticket nor the design said so. **(2) The documented default was spelled in a syntax the parser REJECTS** — both the ticket and § "Dataset MCP tools" write `AGENTKIT_DATASET_MAX_BYTES` as **64MiB**, while `parseDatasetMaxBytes` accepts only a plain integer and makes a suffixed value a **boot error** by design; an operator copying the doc would fail to start. Documented as `67108864` instead. **A default written in a form the code cannot parse is not a documentation defect, it is a latent outage.** **(3) The design's line citations have drifted** (`notRetractedSQL` cited at `memories.go:284-288`, actually `:317-341`; `MemorySearchResult` at `:113-121`, actually `:128-144`) — the facts hold, only the numbers moved. **Cite SYMBOLS, not line numbers, wherever the symbol is stable**; the executor did exactly that in the doc it wrote, which is why its citations will still resolve next month. |
| **R209** | 🔴 **R196's SECOND LIVE CATCH, BY A DIFFERENT ROUTE — THE HARNESS'S OWN COPY WAS INCOMPLETE, WITH NO MUTATION APPLIED AT ALL.** R196 describes the collection failure as *a mutation breaking collection in the file it targets*. W32's executor hit it on **run zero**: it snapshotted with `git archive f23bd52 api` — a **pathspec**, because `api/` looked self-contained (own `package.json`, own `tsconfig.json`, own `vitest.config.ts`, no test importing outside it). It is not. `api/tsconfig.json` line 2 is `"extends": "../tsconfig.base.json"`, which lives at the **repo root** and was not in the archive; the repo is also a yarn workspace, so vitest itself lives in the root `node_modules`. esbuild could not resolve the tsconfig, **all 36 files failed to TRANSFORM**, and the runner printed `Test Files 36 failed (36)` / `Tests  no tests`. 🔴 **The FILE COUNT MATCHED THE BASELINE while zero tests were ever collected** — the exact thing rule four waves through. Under rule four all ten mutations would have been admitted with an **empty kill attribution**, manufacturing ten "this code is undefended" findings on a ticket whose entire deliverable is a kill table. Rule five classified it `COLLECTION_FAILURE` on **run one**, with the cause named. **Two honest qualifications the executor volunteered, and they sharpen the entry.** (1) **This variant is LOUDER than W31's.** W31's printed `Tests 1132 passed (1132)` — a real number from a real partial run, which fools any harness that merely asks "did any test fail". This one printed `Tests no tests`. What the two share is the half that matters: **the file count matched while zero tests ran.** (2) **Rule five was not the only signal — it was the FIRST.** The closing control also fires on a pristine tree returning exit 1, and the empty kill list was a third hint. The value rule five added was *detection on run one, before ten fabricated results existed, with the cause named*; on a longer sweep the closing control would have said only "something is wrong", eleven runs late. 🔴 **TWO NEW HARNESS RULES FOLLOW, BOTH ADOPTED:** **(a) The FIRST run of any copy-based harness must be the CLOSING control, never a mutation** — assert the copy reproduces the baseline before trusting anything it says. The executor's pre-flight checked the copy was byte-identical to *pristine*, which it was, and **that proves nothing about whether pristine is COMPLETE**. **(b) `git archive <sha> <path>` is NOT a safe way to snapshot a workspace package — take the whole repo.** A tsconfig `extends`, a workspace-root `node_modules` and a `.npmrc` all sit *above* the package and all break silently, and the missing file is referenced by a path that points **out of** the subtree, so the subtree itself can never reveal the gap. |
| **R210** | **W32's other four findings, ruled and recorded.** **(1) 🔴 A vitest test timeout is NOT a bound on a non-terminating loop** — it fails the *test* but cannot cancel the running promise, so the loop keeps hammering until an **external** wall clock kills the process (measured: exit 143 at 180s under `timeout -k 5 180`; 2.8 GB RSS when W31 hit it unbounded). Added to W32's Notes, because the next executor will otherwise assume `testTimeout` is the bound and reproduce the hang in full. **(2) The ticket said "the fix is two lines" twice while its own Scope line said "the tests are the work".** It landed as 68 lines of implementation and 136 of test. Two sentences of orchestrator text contradicting each other inside one ticket — corrected in place. **(3) 🔴 THE CEILING IS A NUMBER AND THE PLAN NOW CARRIES IT.** Checking the budgets **after** the short-page exit — which is correct, so a walk that legitimately converges on its fiftieth request is not refused for reaching the cap on it — means the largest converging walk is 49 full pages plus a short one: **9 999 sessions at the default page size of 200**. A project exceeding that would throw `internal` on **every board read**. Judged two orders of magnitude past reach (Bob's port pool caps *concurrent* sessions at 100, and the archive loop reclaims idle ones), and a hard ceiling was preferred to a soft one — **but a ceiling nobody wrote down is how a limit becomes a surprise**, which is R190's lesson about `BOARD_LIMIT` arriving before the ticket is even merged. **(4) `internal` for the edge guard is RULED CORRECT**, not merely accepted: `sessionPageSize` is reachable from no HTTP route — only `store.test.ts` sets it — so a bad value is a Wolf bug, and `invalid` would answer **400 to a client who could not have caused it**. The taxonomy question is "whose fault is this", not "what shape is the value". **And one deliberate omission, also ruled correct:** the executor did **not** add explanatory comments to the three other `for (;;)` loops it cleared, because W32's Files line names two files and **R110** says an executor honours its Files line literally. The enumeration belongs in the plan, and it is in W32's Notes. |
| **R211** | 🔴 **THE INVERSE DOCUMENTATION ERROR — A CLAIM *ABOUT* THE CODE THAT THE CODE DOES NOT MAKE — AND NOBODY LOOKS FOR IT.** Every documentation check this project has run asks "is something missing?" O10's verifier asked the other question and found **two instances inside the brand-new document**, written by an executor whose 35-row verification table was independently confirmed **16 of 16**. (1) `docs/20-datasets.md:27` attributes `COALESCE(MAX(version), 0)` to `CurrentDataset`; that function is `Order("version DESC").First()` (`agentdb/datasets.go:324-327`) and the SQL lives in `CreateDatasetVersion` (`:227-230`). The *behaviour* described is true of both; the **attribution** is not. (2) `:132` says `row_count` is never negative because "the shrink guard **divides** by it"; the guard performs **no division** — `2*newRows >= current.RowCount` (`mcp_datasets.go:668`), written that way *specifically to avoid a ratio*, which the same document then states correctly forty lines later at `:150`. 🔴 **Both survived an author who verified every value against the source**, because the author was checking *values* — defaults, status codes, boundaries — and these are claims about *mechanism*. **A verification table catches "is this number right"; it does not catch "does this function do what I said it does".** The general rule: **when a doc names a symbol, open that symbol — not the behaviour it describes.** A sentence can be behaviourally true and mechanically false, and the mechanically false half is what a reader debugs against. Two further instances of the same family from the same pass: § 1 of `docs/19-embedding.md` is titled **"The three credentials" and there are now four** — the dataset download token, which the same middleware accepts (`cmd/agentd/auth.go:156-167`) and which is 🔴 **the only credential in the tree that ever travels in a URL**, where proxy logs and `Referer` headers capture it; the hazard list gained **no entry** for it. And doc 20's own opening cross-reference sends the reader to doc 19 for three credential definitions, one of which (the session token) **is not in that table**. **R208 said a stale claim naming a COUNT has no token to grep for; this adds that a stale claim naming a MECHANISM has none either — the only check is to open the thing it names.** |
| **R212** | **OPEN — two environment-variable tables now enumerate an incomplete "subset" by name, and neither is false.** `docs/18-workers-memory-events.md` § 11 and `docs/15-standalone-stack.md:366-368` both list "the product-layer subset" **by name** and both now omit the three dataset variables (`AGENTKIT_DATASET_MAX_BYTES`, `AGENTKIT_DATASET_REAP_INTERVAL`, `AGENTKIT_DATASET_KEEP_VERSIONS`). Neither is a lie — both call themselves a subset, and § 11 already delegates other variable families to docs 15 and 19 — but **an enumerated subset with no stated boundary is indistinguishable from an exhaustive list to the reader using it**, and both were found only because O10's executor and its verifier each swept past their own Files line and reported what they saw without touching it. Left open deliberately: O10's Files line named neither, and R110 says an executor honours its Files line literally, so fixing them inside O10 would have been the scope creep the rule exists to prevent. 🔴 **The right fix is not to append three rows** — it is to give each table a one-line statement of what it covers and where the rest lives, so the next variable family does not silently make a third list stale. Wants a small follow-up ticket alongside **R199**'s writer projection and **R189**'s cache-before-read. |
| **R213** | 🔴 **A RETRACTED FACT SURVIVING AS SOMEBODY ELSE'S JUSTIFICATION — the completion of R157, offered by O10's executor after it had already fixed the instance.** R157 said: when a value **moves**, grep for the *value*, not the section that held it. This adds the harder case. `docs/19-embedding.md` § 5.1 did not restate "the two secrets are the same" **for its own sake** — it restated it as **the reason** a design decision was made ("its `sid` is deliberately empty *because* an embed token carrying one would be a working credential"). 🔴 **A reader treats a justification as still load-bearing, so a stale one is more dangerous than a stale assertion**: it does not merely misinform, it makes a live defence look like an accident of history. And it is the hardest to find — a grep for the hazard's name finds nothing, a grep for the section finds nothing, and the sentence never mentions the mechanism it depends on. **THE RULE: when you retract a claim, grep for the claim's CONSEQUENCE, not only for the claim.** Two corollaries from the same round, both adopted: 🔴 **(a) grep the Go COMMENTS too** — `go/cmd/agentd/embedtoken.go:158-172` still carries the identical retracted fact in the identical pre-split wording, citing lines that no longer exist, and is the **likely source** of the doc's sentence. Prose and comments drift together when the prose was written by reading the comment, and `git grep -n "same secret" go/` finds it in one command. **(b) A CITATION IS A SECOND CLAIM AND NEEDS ITS OWN CHECK.** O10's verification table had the row *"current = highest version"* verified and **true**, while the `path:line` beside it pointed at the wrong function — a table that checks *values* passes straight over a mis-attributed mechanism. **Cite `symbol` plus `file:line`, never a bare line**, because a symbol is checkable by grep and a bare line is not; then the mechanical check exists: for every citation, grep the cited symbol and confirm the file:line contains it. **That single check would have caught five of this ticket's nine findings.** The need is not hypothetical — five citations in `docs/19-embedding.md` were broken by **this plan's own inserts**, every one still pointing at *something*, so nothing looked broken to a reader who did not open the file; and in the sharpest case **O8's insert pushed a citation onto a line O8 itself added**, leaving the document pointing at the dataset reaper while claiming to describe console login. |
| **R214** | 🔴 **R205 AGAIN, ONE HOUR LATER, WITH THE CHECK R205 PRESCRIBED ACTUALLY RUN — AND IT DID NOT HELP, BECAUSE IT WAS RUN AGAINST THE WRONG THING.** R205 says: before `git branch -D`, confirm ancestry with `git merge-base --is-ancestor`. At O10's close the orchestrator **ran exactly that** — `git merge-base --is-ancestor 27570ca HEAD` — it passed, and the branch was deleted. **`27570ca` was the sha the AGENT REPORTED. The branch tip was `31e91e9`**, a third commit the agent landed after sending its report, applying the orchestrator's own F11 ruling. Recovered again from the sha `-D` prints. 🔴 **THE FIX: check the BRANCH, not a remembered sha — `git merge-base --is-ancestor <branch-name> <target>`.** A reported sha is a *claim about the past*; the branch name is the present. This is the third instance in one session of the same root cause (R201, R205, this), and the pattern across all three is now unmistakable and worth stating as the lesson rather than the procedure: **every one was a destructive step taken on a reading that was TRUE WHEN TAKEN and stale when acted on.** R201's reading was a `git status` from before a commit; R205's was an idle notification read as "finished"; this one was a sha from a report that was accurate at the moment it was written. **The interval between reading and acting is where the work is lost, and it cannot be closed by adding another check — only by making the check name a MOVING reference (a branch, a worktree path) instead of a FROZEN one (a sha, a status, a notification).** Three saves, three times only because `-D` prints the sha it discards; a `git worktree remove --force` would have printed nothing. 🔴 **THE MERGE-SIDE HALF, from O10's executor, and it is the dangerous one.** The same stale-reference error runs in the opposite direction at merge time: the orchestrator diffed `b59b7ae` — the sha O10 reported at the end of round one — concluded four items were outstanding, and sent them again; they had landed an hour earlier in `b2bb7d8`. **Twice in one ticket, in opposite directions, and the branch name would have answered correctly both times.** The executor named the general case better than the instance: *"a reviewer's ruling landing after the thing it rules on is already in trunk."* Here the addendum was written against `b59b7ae`, answered at `b2bb7d8`/`27570ca`, and the merge took the latter — **the merge was ahead of the review that produced the ruling.** Cosmetic that time. **The general case is not: a ruling can be a security decision, and merging past it puts a state in trunk that a reviewer explicitly rejected, with nothing recording the disagreement.** 🔴 **GUARD, ADOPTED: before merging, run `git log --oneline <branch>` and compare the tip against the sha the review quoted. If they differ, read what arrived in between BEFORE merging, not after.** |
| **R215** | 🔴 **AN OMITTED FILES-LINE ENTRY HID A COLLISION FROM THE COLLISION CHECK — the second time in two waves that the RECONCILIATION half of R206's check found what the INTERSECTION half could not.** At the wave-16 mid-point the orchestrator went to start **W29** alongside the two tickets still in flight. The pairwise intersection said **clear on all three pairs**. The reconciliation said otherwise: W29's criteria require the artifacts panel to fetch `GET /api/hypotheses/:id/artifacts`, and **`web/src/api/client.ts` is the only module in `web/` that calls `fetch`** (its own ownership row says so) — so `client.ts` and `types.ts` are unavoidable, they were **absent from W29's Files line**, and **W24 is editing both right now.** `W24 ‖ W29` read as legal *because the file that collides was not written down*. 🔴 **This is the failure mode that makes an intersection check insufficient on its own: it can only compare what the lines SAY, so an omission is invisible to it — and an omission is precisely what the R94/R195 class produces, thirteen times and counting.** The reconciliation is not a nicety bolted onto the intersection; **it is the half that supplies the intersection's inputs.** Run it first. **The mechanical form, now stated: for each ticket in the proposed wave, read its acceptance criteria and ask "what file must change for this sentence to be true?" — then check that file is on the line.** For a UI fetch the answer is always `client.ts`; for a new wire field it is always `types.ts`; for a new route it is always `app.ts`. Those three have accounted for most of the class. W29 is held until W24 merges — which is the outcome the check exists to produce, and it cost seconds. |
| **R216** | **THE RECONCILIATION CHECK RUN AHEAD OF TIME, WHILE BLOCKED — AND ITS MOST USEFUL RESULT WAS A NEGATIVE ONE.** With wave 16 serialised (W27 held behind W32, W29 behind W24) the orchestrator ran R206's reconciliation on the two *unstarted* tickets rather than idling. **X1's Files line is correct as written** — it creates only `e2e/` files, and the three ownership rows that list X1 (`api/src/config.ts`, `.env.example`, `docker-compose.yml`) do **not** need it, because earlier tickets already landed what its criteria ask for: `WOLF_WEB_PORT` is already honoured at `docker-compose.yml:123`, `WOLF_TEST_LOGIN` is already parsed (`api/src/config.ts:337-360`), the dev-login route is already built and gated (`api/src/routes/auth.ts:159-174`), and **B6's "asserted absent from the production build by test" is already done** (`api/src/routes/auth.test.ts:279`). All four written into X1's Notes with citations, so its executor does not spend a round rediscovering them — and one of the four would otherwise have been *rebuilt*, since a ticket that says "X1 does this" reads as work to do. 🔴 **Two things worth carrying.** (1) **A check that returns "nothing to fix" is not a wasted check if you write the result down** — the ownership table is *deliberately* over-inclusive (it lists who *might* touch a file), so a ticket appearing on a row is a **question**, not an instruction, and answering it once is cheaper than every executor answering it separately or, worse, honouring it. (2) 🔴 **The pre-flight's real limit was stated rather than glossed:** every mechanism it verified is proved *in unit tests against a fake*, and X1 is the first thing in this project that runs the real stack — so the note tells its executor to treat all four as **unverified in a container** until seen working there. That is R41's point (`docker compose config` proves a declaration and nothing else; W1's nginx bug survived a pass for exactly that reason) and it is why the reachability proof is assigned to X1 at all. **Verifying a precondition and verifying it in the environment that will run it are different claims, and a pre-flight can only make the first.** |
| **R217** | **THE CRITERION WAS RIGHT AND ITS STATED REASON WAS FALSE — and the implementer's code was correct anyway, because it reasoned from the code rather than from my sentence.** W32's criterion 1 demanded a page cap **and** a row cap, justified as *"a server returning a full page of duplicates exhausts rows before pages."* W32's verifier checked it: **`rows` and `pages` advance in lockstep regardless of duplication.** At the default `SESSION_PAGE_SIZE = 200` the **page** budget fires first, at 10 000 rows, so `SESSION_INDEX_MAX_ROWS` is **unreachable on every production path today** — it bites only for `sessionPageSize ≥ 409`, which no caller supplies. **Both bounds were delivered as demanded, and the implementer's own docstring (`store.ts:1235-1240`) states the true relationship correctly** — so it had already noticed and had quietly written the right thing down beside the wrong instruction. 🔴 **The rule: a criterion carries TWO claims — WHAT to build and WHY — and only the first is graded.** An executor satisfies the *what*; a verifier grades the *what*; the *why* is checked by nobody, and it is the half that propagates, because the next ticket to reason about page walks would have inherited a false model of how the counters move. **This is R213's shape one level up**: there, a retracted fact survived as somebody else's justification in a document; here, a false justification rode into a ticket beside a correct instruction. **Both are the same failure — a justification is load-bearing to readers and graded by no one.** The keep is that the *decision* survives the correction: a bound that is unreachable today but correct is exactly what you want in place before a caller passes a larger page size, so the criterion stands and only its argument is struck. **Do not delete a requirement because its stated reason was wrong; re-derive the reason and check whether the requirement still follows.** |
| **R218** | 🔴 **A SIXTH ROUTE TO A MANUFACTURED FINDING: THE MUTATION THAT NEVER LANDED — and the landing gate as most people would write it does not catch it.** W32's executor hit it during its fix-round re-sweep. Its M2 anchor quoted a block **including the `details` line it had just deleted in the same fix round**, so the occurrence guard counted zero matches and raised `ANCHOR_REFUSED`, stopping the sweep. **It failed LOUDLY only because the harness counts occurrences before substituting.** Written the ordinary way — `s.replace(old, new)` with no count check — the substitution is a **silent no-op**: the file is unchanged, the suite runs green, and the result reads **`SURVIVED` — "this code is undefended"**. That is R196's manufactured-finding direction reached by an entirely different route: not a collection failure, but a mutation that never happened. 🔴 **AND THE LANDING GATE DOES NOT NECESSARILY SAVE YOU.** A no-op replace leaves the tree **byte-identical to pristine**, so the differing-file set is **empty**. A gate that asserts *"the differing set **equals** `[the intended file]`"* fails correctly. A gate that asserts *"no **other** file changed"* — which is the natural English of the rule and how it would most likely be written — **passes an empty set**, and admits the run. **The rule is therefore amended in the header: assert the intended file DID change, not merely that no other file did.** Two further points from the same report. **(1) A mutation harness's anchors go stale the moment the code under test changes, and a FIX ROUND is exactly when that happens** — the post-fix re-sweep is both the run most likely to hit a stale anchor and the run an executor is most tempted to treat as a formality. **(2) The invalid-mutation rate stays uninformative for the reason W31 established**: the occurrence guard converts a would-be-invalid mutation into a *refusal* rather than a *run*, so the number measures the guard, not the mutations. 🔴 **The dangerous case is the anchor that is a SUBSTRING still matching something else after the edit** — that one applies, in the wrong place, and runs. **That** is what R182 counts, and no guard catches it; only reading the landed diff does, which is R170's rule arriving from a third direction. |
| **R219** | **A GATE BUILT IN ONE TICKET AND WIRED ON ONLY ONE OF ITS TWO SURFACES — caught by a verifier reading a comment, not the code.** W24 built `GoLiveButton`'s second prop and wired it on the Go Live review screen, proving all four combinations and mutating each condition separately. But `web/src/pages/HypothesisDetail.tsx:162` still renders the same button with `specValidation` only, **so the detail page's Go Live button is enabled with no template accepted** — and the comment sitting on that line already asserts the opposite: *"The spec half of the Go Live gate (W24 adds the template half)."* 🔴 **W24 was right not to touch it.** `HypothesisDetail.tsx` belongs to another ticket this wave, and reaching in would have been exactly the collision **R194** exists to prevent — this is the ownership rule working, producing a *known* gap instead of a silent conflict. **W22's server-side `422` backstop means it is not a hole**, only a button offering an action the server refuses; assigned to **W29**, which owns the file next. **Two things generalise.** (1) 🔴 **A comment that says another ticket "adds" something is a promise, and nothing checks that it was kept** — it read as satisfied because the ticket named in it *did* land. **When a ticket's scope is trimmed by a file-ownership rule, the trimmed half needs an explicit owner written into that owner's ticket the same day, or the comment becomes the only record and it lies.** (2) The verifier found it by **reading a comment against the code it describes** — the R211 lens (a claim about the code that the code does not make) applied to a **forward-looking** claim rather than a retrospective one. R211 covers "this function does X" when it does not; this is "another ticket will do X" when it has not. **Same check, same failure, different tense.** |
| **R220** | 🔴 **THE OPENING CONTROL FOUND THE DEFECT — the first time in this project that a harness's own setup step, rather than its results, was the finding.** W24's verifier needed an opening control on the web side: a mutation it *expects* to go red, proving the harness can report a failure at all. It picked `CODE_ORIGINS_HEADING` → `"XX"`, ran it, and got **GREEN**. Its control had failed — and the reason its control failed **was defect D1**: two of the three security headings on the Go Live review screen have **no assertion anywhere in the repo**. It re-ran with a different mutation to establish the control, then reported the first as the defect. **The rule's stated purpose is to validate the instrument; it validated the instrument by failing, and the failure was the measurement.** Worth stating because it changes how a green opening control should be read: 🔴 **an opening control that goes green is not "pick another mutation" — it is a finding about the code you chose, and it must be recorded before you move on.** The temptation is exactly the opposite, because at that moment the harness is unproven and the natural instinct is to find *something* that reddens so the real work can start. **What D1 actually is:** setting `OTHER_ORIGINS_HEADING = "Origins allowed to execute code"` leaves **457 tests green**, so the screen shows the *non-executable* host list under the *executable* label — the one distinction that entire screen exists to draw for a human approving remote code. Every **value** is correct and pinned; only the **words telling the human which list is which** were unguarded. **Fourth consecutive ticket with the R182/R198 shape** — three side-by-side cases of one rule, the first getting the load-bearing assertion and the two written beside it trimmed, with the sweep looking outward instead of at the three lines it had just written. |
| **R221** | **`node_modules` must be HARDLINKED into a verification copy, never symlinked** — `api/src/import-boundary.test.ts` resolves realpaths, and a symlink makes the resolved path escape the repo, so the boundary test fails for a reason that has nothing to do with the code under verification. Recorded because it is the third distinct way this project has produced a **wrong** result from a correctly-made copy, and all three are the same lesson from different angles: **R209** — `git archive <sha> <path>` loses a root `tsconfig` an `extends` points out of the subtree, so every file fails to *transform* while the file count still matches; **R218** — a mutation whose anchor no longer matches silently no-ops, leaving the tree byte-identical and reading as `SURVIVED`; and this. 🔴 **The unifying rule: a verification copy is a claim that the copy behaves like the original, and that claim needs its own evidence.** W24's verifier drew the right conclusion and ran it — **its first run was the identity control, before any mutation**: unmutated copy, both suites green at the baseline counts, differing set empty. That is R209's rule doing its job on a second ticket, and it is what makes everything the sweep says afterwards worth reading. |
| **R222** | 🔴 **R198 IS RECURSIVE, AND THE FIX ROUND'S OWN OUTPUT IS THE LEAST-SWEPT CODE IN THE TICKET.** W198 said the sweep does not cover the test you are writing while you sweep. W27's executor was told to re-run its assertion-list diff **over its own output** before reporting — and it found **three** gaps, one of them a **wrong-reason test it had written during its own mutation sweep**. `hypotheses_board: an attention read that FAILS costs the third clause and nothing else` **configured no attention request at all**, so the clause its title claims to cost was never exercised; it proved only the half after "and nothing else". 🔴 **And a mutation still killed it** — the rethrow 500s the whole board — **which is precisely how it hid**: a red you have not attributed *to the right assertion* is not evidence, and R146's rule needs that sharper form. It now has a **working-read control twin**, without which "the failure costs the clause" is unfalsifiable, since a board that never tiered by request at all would satisfy it too. The sharpest of the three: `store_state_history`'s forged- and Wolf-retraction cases were left thinner than the hostile-retraction case that the executor's **first** self-sweep had thickened — **the fix round's own output went unswept by the fix round.** **The rule, stated once and for all: the assertion-list diff must be run over the output of every round INCLUDING the round that is running it, and the last thing a fix round does is sweep itself.** Six consecutive tickets have now shipped some form of this defect; this is the first where the executor caught its own before reporting, and it caught it only because it was told explicitly to look at its own work rather than at the code it was verifying. |
| **R223** | **Two W27 rulings, both accepting a deliberate deviation from the ticket's literal wording, and both recorded because the reasoning is the useful part.** **(1) `challenge_reason` is gated on `status === "challenged"`, which R143 does not say — and the gate is RIGHT.** R143 said "project the rationale as `challenge_reason`", unqualified. But a `confirmed` row's `rationale` is **the free text a human typed with their verdict**, and serving that under a name the UI renders as a *machine's* reason would put a person's words in a machine's voice — on the one page whose entire job is telling the two apart (UI § 2's two channels). It is already carried on the `verdict` block. The gate also means the extra full-content read is paid **only for challenged hypotheses**. 🔴 **The general point: a projection's NAME is a claim about provenance, and reusing a field because its shape fits is how a human's words end up rendered as a system's.** If the human rationale is wanted on the wire it needs a second, differently-named field. **(2) `poller.ts` production code is unchanged, and the ticket's Files line implied an edit that would have been WORSE.** `stale` is derived inside `evaluationSummaryLine`, beside `tallyConditions` — the one call site the writer and its tests share. Computing it at the poller's call site instead would be a **second source of truth** for a number the board reads. The ownership row is still right to serialise W10 → W27; only the ticket's wording implied a change that should not be made. **A Files line is a permission, not an instruction — an untouched file it names is not automatically an under-delivery, and the executor that says why is doing the job.** |
| **R224** | **OPEN — a `live` hypothesis with no trusted state row and no tamper tiers `holding`, and `holding` is COLLAPSED BY DEFAULT.** § 4's fourth rule is "everything else", W27 implemented it literally and tested it, and that is correct against a pinned table. But the plan's standing doctrine is that **an anomaly is RENDERED, never dropped**, and this row is an anomaly — a session present in the index with **no `kind=hypothesis` memory at all** — landing in the one section a human does not see without expanding it. In practice such rows almost always carry tamper and rule 1 catches them first; the exposed case is the genuinely empty one. **Left open rather than deviating from a pinned table**, which is the right call by the executor and the reason it is here rather than in the code. 🔴 **The decision needed is whether § 4 gains a fifth rule** — something like *"`live` with no trusted state row → `needs_human`"* — or whether `holding` stops being collapsed when it contains a row with no state. **Note the shape**: the tier table is exhaustive by construction ("everything else"), which guarantees every row lands somewhere and thereby guarantees that an anomaly lands somewhere **plausible**. An exhaustive fallback is what stops rows vanishing; it is also what stops them being noticed. |
| **R225** | 🔴 **A LIMIT CAN BECOME A DEFECT WITHOUT ITS LINE CHANGING — because a later ticket changes what the data is FOR.** `readDetailRows` has always read `limit: DETAIL_LIMIT` (50). Harmless for eight tickets, because only the **newest surviving row** was ever used. W27 made those same rows the **entire state-change timeline** on the detail page — so a hypothesis with more than 50 state rows now **silently loses history, with no anomaly and no caption**, against the standing doctrine that an anomaly is rendered rather than dropped. **Not one character of the read changed.** Its *meaning* did. **This is R190's shape arriving from the opposite direction**: there, a cap was decoration because no fake honoured `limit`; here, a cap is correct, honoured, and newly wrong because its consumer changed. 🔴 **THE RULE: when a ticket gives an existing read a NEW consumer, re-derive every bound on that read against the new use.** A `limit` is a claim about what the caller needs, and the claim is re-made silently every time a caller is added. Two corollaries worth having together: **every silent truncation needs to be either rendered or written down** — R190 is open precisely because nobody wrote its cap down, and W32's ceiling went into the plan for the same reason; and **a Files line that says "modify `X.ts`" does not warn you about the reads in `X.ts` you did not touch.** Found by W27's verifier, which asked what the new consumer needed rather than whether the code W27 wrote was correct. |
| **R226** | **THE THIN HALF OF A PAIR WAS THE FIRST ONE THIS TIME, AND THAT IS WHY THE SWEEP MISSED IT.** R182 established that when two cases of one rule sit side by side, the hole is usually in the half written **second** — measured, twice, and it is what R198's self-sweep looks for. W27's verifier found the inverse: of three side-by-side `challenge_reason` cases, **the FIRST omits an assertion both later siblings carry**. Behaviour is still pinned by the siblings, so the defect is tidiness — **but the direction is the finding.** An executor applying R182 looks at the newest case against the older ones and finds nothing, because the older one is the thin one. 🔴 **THE RULE, sharpened: compare EVERY member of a group against every other, not the newest against the oldest.** R182's "usually the second" is a *prior*, not a procedure, and a prior used as a procedure searches only where it expects to find. **Seven consecutive tickets have now shipped some form of the assertion-list defect**, and the fact that the shapes keep varying — second-thin (R182), unswept-by-its-own-fix-round (R222), first-thin (this) — is itself the argument for running the diff mechanically over all pairs rather than by judgement about which half is likely wrong. |
| **R227** | 🔴 **A TEST WHOSE TITLE NAMED A DISTINCTION ITS BODY NEVER EXERCISED — invisible to every reading, findable only by mutation.** W27's review round 2 wrote a test called *"truncation is reported off the ROWS READ, not off the rows that survived"*. The mutation that counts **survivors** instead of rows read **survived it**. The reason is the entry: **every fixture had survivors equal to rows read**, so the two implementations returned the same number on every case that existed, and the assertion could not tell them apart. **The distinction lived in the title and nowhere in the body.** Note what this defeats: reading the test finds nothing wrong (the title says the right thing and the assertions are true); reading the code finds nothing wrong (the implementation is correct); and R133's check — *would this assertion still hold with the thing deleted?* — **also passes**, because the thing was not deleted, it was replaced by something that happened to agree. 🔴 **Only a mutation asks the question, and it asks it by making the two expressions disagree.** The discriminating case turned out to be **the attack itself**: a full page with enough rows rejected to put survivors under the cap is what padding a hypothesis's page with forged rows produces — so the survivor-counting implementation would push real history off the end **and hide that it had**. **THE RULE: a test's title is a claim about what its body discriminates, and nothing checks that claim.** When a test's name contains "not", "rather than", or "instead of", it is asserting that two implementations differ — **build the fixture where they actually do, or the title is decoration.** This is the third and subtlest member of one family found in three consecutive review rounds of one ticket: a missing assertion (findable by reading), a test asserting half its title (findable by a careful diff), and this (findable only by mutation). **The trend is the argument for mutating every round, including the rounds that only fix tests.** |
| **R228** | 🔴 **A NEGATIVE ASSERTION CANNOT SEE A FIELD NOBODY THOUGHT TO FORBID — and the test that reads as the strongest on the page was the one that could not fail.** W29's route logs a line, and its criterion is that **no `download_url` ever reaches the log**. The test captured `pino` output and asserted the joined lines contained no `download_url`, no `downloadUrl`, no `blobPath`, no API key. The executor mutated the log line to dump the **whole record**, expecting the leak — **and nothing appeared, because the client's mapper had already dropped both fields upstream.** So every "does not contain" assertion was green over data that could never have contained the thing, **and would have stayed green while the log line silently grew a field nobody sanctioned.** 🔴 **This is the most dangerous shape in the codebase: a credential-absence test that cannot fail reads as the strongest guarantee on the page, so an auditor sees it and stops looking.** The fix is the general one — **pin the payload's own KEY SET exactly**, not merely what must be absent — which is R220's lesson (every *value* pinned, the *shape* unguarded) arriving on a security assertion. Two corollaries the same ticket produced. **(a) The fixture must carry the forbidden value or the criterion is untestable**: Bob sends no `download_url` on this route today, so the fake supplies one deliberately — **without it an allow-list and a passthrough are indistinguishable.** **(b) Every prohibition needs a positive half**: a route that logs *nothing at all* satisfies every "must not contain" trivially, so the test also asserts the line exists and carries its three intended keys. **A test made only of negatives passes hardest when the feature is missing entirely.** |
| **R229** | 🔴 **A NEW DOOR INTO R133's FAMILY: a route that is mounted, reachable, and SILENTLY DEGRADING — and the page's own tests pass over it.** R133 recorded that nothing in 1018 tests noticed if `app.ts` never mounted W11's routers. W29 found the subtler cousin in `App.test.tsx`: its detail-route case **did not stub the new artifacts fetch**, `stubFetchRoutes` threw on the unrouted path as designed, **the client caught the throw and the component rendered its degraded marker** — and every route assertion in the test still passed. **The page was broken in the test and the test did not care.** The difference from R133 matters: there, a route was *absent* and a discriminating probe existed (a signed-in request with a malformed id, answered 400 before any upstream call, where an unmounted route 404s). Here the route is *present and working*; what is broken is a **fetch the page makes**, and the failure has a designed-in graceful path — so the page renders, just wrongly. 🔴 **The rule: a component whose failure mode is "degrade gracefully" cannot be tested by asserting the page rendered.** Every test that renders a page making a new fetch must either stub it and assert the **success** state, or assert the degraded marker **on purpose**. The generalisation, which is R200's lesson from the other side: *"an absence-only test passes on a page that never loaded"*, and this is *"a page-level test passes on a page whose sub-component silently failed"* — both are cases where **the assertion's subject is coarser than the thing that broke.** Closed with a positive assertion so it cannot recur. |
| **R230** | **"`unavailable` → 503" was a claim about the KIND read as a claim about the STATUS, and the plan repeated it twice.** W29's executor found it and **asserted the real behaviour rather than changing it**, which was the right call. Verified in the source: `DEFAULT_STATUS` (`api/src/errors.ts:28`) maps a kind to a status **only when no status is supplied**, and `defaultErrorFor` (`api/src/bob/client.ts:255-270`) **always supplies one — the upstream's** (`{ status, upstreamBody }`). So a Bob **502** reaches the browser as **502 carrying `kind: "unavailable"`**. The behaviour is correct and deliberate: the upstream status is information a caller may want, and the *kind* is what carries the retryable/not decision. The defect was purely in the description. 🔴 **The general shape, and it is the third instance this month: a mapping stated in one direction reads as a bijection.** "`unavailable` → 503" is true of the default and false of the general case, and a reader who branches on status rather than kind inherits the wrong model — the same failure as **R217** (a criterion's stated *reason* being false while the criterion held) and **R213** (a retracted fact surviving as somebody else's justification). **All three are load-bearing prose that nothing grades.** Clarified in § "Shared error taxonomy" with the rule stated plainly: **branch on the kind, not the status.** W10's poller already does. |
| **R231** | 🔴 **`cmd1 && cmd2 && cmd3 || echo "<specific claim>"` ASSERTS A SPECIFIC CLAIM ABOUT A COMMAND IT NEVER RAN — R196's lesson in shell-plumbing form, and the orchestrator uses this pattern constantly.** W27's executor chained `git cat-file … && git log … && git merge-base --is-ancestor e5d4c04 main` with `|| echo "NOT an ancestor"`. An **earlier** command in the chain failed, short-circuited the rest, and the `||` fired — printing **"e5d4c04 is NOT an ancestor of main"** when the merge-base check had **never executed** and the commit *was* an ancestor. It caught this only by re-running each check as its own command before believing any of them. 🔴 **The general form: a `||` branch reports the failure of the CHAIN, while its message names the LAST command — so every such line is a claim about a command that may not have run.** This is the same family as **R196** ("the tool produced output" is not "the tool did the work"), **R218** (a mutation that never landed reading as SURVIVED) and **R209** (a copy that could not compile reading as a correct file count): in all four, **an observation was taken as evidence for a step that never happened.** The rule: **when a shell line's output makes a specific factual claim, that line must contain exactly one command.** Compound chains are fine for *doing*; they are not fine for *concluding*. Directly relevant here because the orchestrator's own ancestry check before `git branch -D` — the guard from **R214** — is exactly this shape, and would have printed a confident false answer under the same conditions. |
| **R232** | 🔴 **R225 APPLIED TO ITS OWN TICKET, AND THE ANSWER WAS WORSE THAN THE ONE ALREADY LOGGED — a truncated read that now MISCLASSIFIES rather than merely losing rows.** Given R225's rule (*when a ticket gives an existing read a new consumer, re-derive every bound on that read against the new use*), W27's executor ran it over the rest of its own work and **corrected its own earlier report**. It had told the orchestrator that past `BOARD_LIMIT = 100` "rows vanish from the board and from their tier counts". Re-reading the code, the truth splits three ways: the `kind=hypothesis` read **loses nothing** (a hypothesis whose state row did not fit fails to settle, so `readBoard` falls through to its per-name follow-up and renders correctly, at one extra request each — a **performance** cliff); the `kind=evaluation` read goes **silently null** (no follow-up exists for an *absent* entry); and 🔴 **the `kind=report` read, whose `headline` W27 gave a SECOND consumer — the WATCH tier rule — now MISCLASSIFIES.** No entry ⇒ `headline === null` ⇒ every such `live` row tiers **WATCH**. **Past 100 hypotheses the board floods the section that is meant to be short with rows whose only "signal" is that a read was truncated, burying the ones that genuinely need a human.** That is a different and worse failure than rows going missing, and **W27's own tier rule introduced it** by giving an existing field a second reader. **Two things this establishes.** (1) R225's prescription is not a formality — running it over already-shipped work in the same ticket found a defect the ticket's own author had mis-stated an hour earlier. (2) 🔴 **The fix is blocked on R190's first half and cannot even be TESTED today**: both stubs ignore `limit` on every `latest_per` branch, **so the fake cannot express the condition**. Teaching the fakes is not preparatory work for the consolidation — it is the precondition for testing a live misclassification. Fix shape when it lands: have `readReportSummaries` and `readEvaluationSummaries` report truncation the way `readHypothesisWithHistory` now does, and make the WATCH rule decline to fire on a null headline **from a truncated read**. |
| **R233** | **THE OPEN LEDGER AT 47 OF 49 — written down in one place, because a plan closes and a log does not.** Every ticket but the two end-to-end ones is merged, so what follows is what this project knows about itself and has not fixed. 🔴 **Ranked by what would bite a real deployment first.** **(1) `BOARD_LIMIT = 100` is decoration AND now misclassifies** — R190 as sharpened by **R232**: the fakes ignore `limit` on every `latest_per` branch so the cap is unexercisable, and W27 gave the `kind=report` read's `headline` a second consumer, so past 100 hypotheses every unlisted `live` row tiers **WATCH** — the section meant to be short becomes the longest and buries the rows that need a human. **The fake fix is the precondition for testing it, not preparation for a consolidation.** **(2) Bob's `ListArtifacts` is unbounded end to end** — no LIMIT, no pagination, no cap in Wolf's route, every row rendered, on data that grows daily for a hypothesis's whole life (**R225**'s shape). **(3) `readSessionIndex({sessionPageSize: 0})` still loops forever** — W32 capped the walk; a caller's non-positive page size is refused, but the zero case was recorded and not closed. **(4) The fourteen fakes** — 42 undefended sends measured, 7 security-class, and **the six retraction tests come first**, because four of the seven are *missing fixtures* no shared fake can close (**R184**, W31). **(5) `kind=report-candidate` had no reader for nine tickets** until W24 built one (**R188/R206**) — the class, not the instance, is the open item: an enumerated kind in a criterion is a claim that something reads it, and nothing checks that. **(6) Two documentation halves that disagree with the code** — Bob's published `source` union intersects its Go source on exactly `'auto'` (**R229**-adjacent, Bob's defect), and two env-var tables enumerate an incomplete "subset" by name (**R212**). **(7) `report.unreadable` conflates two causes** (**R192**), **`ApplyTopology` is a second producer of empty provenance reachable by an embed token** (O10 § 9), and **`getSessionArtifactFile` is unreachable with its 501 branch untested**. 🔴 **The meta-item, and the one to act on first: EVERY entry above was found by a consumer, not by a reader.** R188 by the ticket that needed the kind; R190 by the audit that tried to test the cap; R225 by the ticket that gave a read a new consumer; R229 by a page test that passed over a broken page. **Nothing in this project has ever been found by reading the plan.** That is the argument for the three standing pre-wave checks — they are the only mechanism here that reads *ahead* of a consumer instead of behind one. |
| **R234** | 🔴 **X1's OWN CRITERION PINNED A STRING THAT CAN NEVER APPEAR IN AN X1 RUN — a `run.sh` written to it fails every time.** The criterion required the boot line `[agentd] ANTHROPIC_API_KEY unset → MOCK model proxy (set it for a real agent)` as proof of mock mode. `go/cmd/agentd/modelproxy.go:58-63` branches on whether a mock **script table** was configured: `table == nil` prints that line (`:59`); **otherwise** it prints `SCRIPTED mock model proxy (N rule(s))` (`:62`). **They are exclusive, and X1 always configures a table** — the ticket's own criteria require one. So the ticket demanded proof that its own design makes impossible. `README-stack.md` carried the same defect, documenting only the scriptless form. **This is R207's shape** — an expected value asserted without being produced — **but worse in one respect: R207's number was merely unreachable, and this one is unreachable *because of a different criterion in the same ticket*.** The two criteria were each written correctly and are jointly unsatisfiable. 🔴 **The general rule: when a ticket pins an exact output string, that string must have been OBSERVED under the ticket's own conditions, not copied from a document describing a different configuration.** Corrected in both places, and the replacement is a positive plus **two negatives** — assert the `SCRIPTED` form, and assert that neither `real model proxy →` nor `subscription mode →` appears — because a positive alone would pass on a boot line whose wording changed, and the thing actually being proved is *that nothing can bill*. |
| **R235** | 🔴 **THE GATEWAY PROBE READS THE WRONG ROUTING TABLE — every `mcp__wolf__*` call from inside a session would time out with no error and no log, and only running the product could ever have found it.** `api/src/config.ts` discovers DinD's gateway from its **default route**; under compose that is the **outer compose-network gateway**, while the address a session container actually uses is DinD's inner `docker0`. X1 measured it from **inside a nested container**: `172.17.0.1:8100/mcp` → **401** (reachable, credential refused); `172.26.0.1:8100/mcp` → **exit 7** (cannot connect) — and `config.ts` had discovered `172.26.0.1`. 🔴 **§ "Local topology and networking" NAMES the correct address, and then prescribes a probe that reads a different route.** The document and the code it specifies disagree, two paragraphs apart. **R43 warned that the VALUE drifts and prescribed discovery as the fix; the defect is that the DISCOVERY IS WRONG** — a different failure from the one guarded against, inside the same function, which is why the guard's existence made it look handled. **Two things generalise.** (1) 🔴 **The two addresses AGREE on a plain Docker install and differ under compose** — so a host-side check passes, a unit test passes, and only a reading taken from inside a nested container disagrees. **A probe can only be verified from the place its answer will be used.** (2) The failure is **silent by construction** — ⚠️ **though NOT for the reason first written here; see R238.** This originally said "a wrong gateway yields a timeout, not a refusal", and **X1's own measurement, recorded in this same entry, contradicts it**: `curl exit 7` — could not connect, **immediately**. ⚠️ **and the first correction over-corrected — see R244: "immediately"/"fast" is not supported either**, since exit 7 covers both a 4ms refusal and a multi-minute black hole. **The invisibility comes from WHERE the failure lands, not from latency** — on the far side of the container boundary, where nothing Wolf logs can see it. Cut as **W33**. X1 is unaffected: its rig sets `WOLF_MCP_URL` explicitly, which the design says wins, and that escape hatch is the only reason the bug was survivable. |
| **R236** | 🔴 **A DISCOVERY SPEC MUST NAME THE ROW, NOT THE FILE — "read X from Y" is under-specified whenever Y holds more than one plausible answer, and that under-specification is INVISIBLE on a default install.** § "Local topology and networking" told `api/src/config.ts` to *"read the default route from inside DinD's netns (`ip route show default`, or `/proc/net/route`'s destination-`00000000` entry — wolf-api shares that namespace, so this reads DinD's own table)"*. The ASCII topology two paragraphs above **names `172.17.0.1` correctly**. The bullet is the defect, and the parenthetical is why it survived review: **it is TRUE** — wolf-api genuinely does read DinD's own table — so a reader checking the sentence finds it accurate and stops. **DinD's own table has two gateways, and the sentence silently assumed the interesting one was the default.** Under compose it is not: the default route is the **outer compose-network gateway**, unreachable from a session container. **The failure is silent by construction** (a wrong gateway times out; it does not refuse), so nothing surfaced until X1 ran the product. 🔴 **And the reason no test caught it: the two candidates AGREE on a plain Docker install and differ only under compose** — a host-side check passes, and a unit fixture whose two addresses agree **cannot fail**, which is precisely the fixture the original tests had. **Three things follow, all adopted.** (1) **Name the row.** Corrected in place. (2) 🔴 **A spec that prescribes a probe must state what the probe must NOT match**, because the wrong candidate is usually the more obvious one — the corrected bullet says "never the destination-`00000000` default route" and says why. (3) **A guard's existence made the gap look handled**: R43 warned the *value* drifts and prescribed discovery as the remedy, so every later reader saw a considered mechanism rather than an open question — **the defect was in the remedy, inside the same function, and that is the hardest place for a reader to look.** |
| **R237** | 🔴 **THE FILES-LINE CHECK COMPARES TICKETS TO TICKETS, SO IT CANNOT SEE A SECOND CONSUMER OF A CHANGED CONTRACT.** W33 changed a function's behaviour, and `api/src/mcp/server.test.ts:406-425` broke — its route-table fixture carried **only** a default route and asserted `mcpUrlSource === "discovered"`, which under the fix correctly resolves to `fallback`. **No ticket owned that file**, so the pairwise intersection (**R194**) and the criteria reconciliation (**R206/R215**) both pass it silently: the first compares *ticket* Files lines against each other, and the second reconciles a ticket against *its own* criteria. Neither asks who else depends on the seam being changed. 🔴 **THE EXTENSION, ADOPTED AS A FOURTH STEP: a ticket that changes a FUNCTION'S CONTRACT must have its Files line reconciled against every CALLER of that seam — not only against other tickets.** Mechanically: grep the changed symbol, and grep the *fixture shape* it consumes, because the caller that breaks is usually a test carrying its own copy of that shape rather than one calling the function directly. **The right treatment was the one taken**: the fixture was extended to be a realistic table and **its assertions were left alone** — the shape this project watches for is a *test weakened to accommodate a fix*, and adding a row a real table always carries is the opposite of that. **Note the symmetry with the bug being fixed:** the broken test's fixture had one gateway where reality has two, and the broken *code* assumed one gateway where DinD has two. **The same over-simplification appeared in the production code and in an unrelated test's fixture, and neither could see the other.** |
| **R238** | 🔴 **THE PLAN DREW A CONCLUSION FROM A PREMISE ITS OWN RECORDED EVIDENCE CONTRADICTED — in the same entry, three lines apart.** R235 said *"a wrong gateway yields a **timeout**, not a refusal, so there is no error to surface"*, and W33's criterion 4 required that wording in a code comment. **X1's measurement, quoted in R235 itself, is `curl exit 7` — could not connect, immediately.** A timeout is `exit 28`. ⚠️ **The first correction then said "the address fails FAST" — also an inference, corrected in R244.** W33's executor rewrote the comment to say so; its verifier flagged the contradiction with the plan and ruled the executor right. **Both are correct and the plan was wrong.** 🔴 **The conclusion survives and the reason changes, which is the interesting part**: the failure IS invisible — but **not because it is slow. Because of WHERE it lands**: on the far side of the container boundary, where nothing Wolf logs can see it. **A plausible mechanism was substituted for the real one, and the substitution was invisible because the CONCLUSION was right.** That is the shape worth carrying: **R217** recorded a criterion whose stated reason was false while the criterion held; this is the same failure inside a *log entry*, and worse in one respect — **the refuting evidence was already in the entry, transcribed correctly, and simply not read against the sentence beside it.** The rule: **when an entry states a mechanism AND quotes a measurement, check the measurement against the mechanism.** An exit code is a fact; "times out" is an interpretation, and nothing grades interpretations. |
| **R239** | 🔴 **R237's HOLE EXISTS IN THE VALIDATION LINE TOO, AND R237 DOES NOT CLOSE IT.** R237 extended the Files-line reconciliation to every **caller of a changed seam**. W33's verifier found the identical gap one field down: **W33's Validation was `yarn test src/config`, which runs ONE file and never touches `api/src/mcp/server.test.ts`** — the probe's second consumer, and a file W33 had to edit. 🔴 **Eight of that verifier's nineteen mutations kill a test that lives only in that file**, so **a regression there passes the ticket's own gate**. A Files line and a Validation line are two independent claims about scope, and fixing one leaves the other stale. **THE RULE: when a ticket touches a seam with more than one consumer, its VALIDATION must cover every consumer, not just the file the change lives in.** A filtered run is a scope claim, and the safe default is the whole suite — the filtered form exists to make a *tight* loop fast, not to define what "passing" means. Corrected in place to the whole api suite. **Generalise it to the standing checks: the pre-wave pass now reconciles a ticket's Files line against callers of the seam (R237) AND its Validation line against the same set.** |
| **R240** | **A COMMENT THAT CANNOT NAME THE MECHANISM CORRECTLY SHOULD NAME THE OUTCOME INSTEAD — the inverse of R236, and it survived the same defect by being less specific.** While correcting the documentation around R235, W33's executor checked `docker-compose.yml:27-33` and found it **already right and needing nothing**: it says wolf-api *"discovers the DinD bridge gateway itself at boot rather than trusting a hard-coded 172.17.0.1"*. **It names the BRIDGE, never the ROUTE.** R236's bullet named a file and a namespace precisely and was precisely wrong; this comment named an outcome vaguely and was safe. 🔴 **The corollary, in the executor's words and adopted verbatim: "a comment that cannot name the row correctly should name the OUTCOME instead of guessing at the mechanism."** This is **not** an argument for vague comments — R236 stands, and a discovery *spec* must name the row. It is an argument about **where a claim is load-bearing**: a spec instructs, so it must be exact; a comment orients, and **a guess at a mechanism is a claim that nothing grades**, which is strictly worse than an honest abstraction. Two related instances from the same round: the same false *"times out"* claim was found a second time in `api/src/index.ts:50-53`, **sitting directly above the log line an operator is told to check** — the second thing a debugger reads while chasing exactly this failure, telling them to look for a slow request that will never come. 🔴 **A false diagnostic hint is worse than no hint, because it costs the debugger the time it claims to save.** And the search that finds these is **R213's**: grep for the **claim** (`time out`, `hang`), not the mechanism — the restatements never mention the thing you would search for. |
| **R241** | **A TEST THAT PINS DOCUMENTATION PROSE IS R180's CLASS WITH EXACTLY ONE DEFENDER — ITSELF — and W33's executor said so rather than letting it look stronger.** To stop `.env.example`'s corrected description drifting back, W33 added a case asserting substrings of the file. **It is test infrastructure with nothing above it**: a legitimate rewrite — new wording, same meaning — reddens it, and **the cheapest fix is to weaken the assertion**, at which point it silently becomes decoration and nobody finds out. A mutation of the *document* dies nowhere else. The mitigation taken is the right one available: **three short, meaning-bearing phrases rather than whole sentences, each independently load-bearing** — three separate mutations (restoring the old sentence, dropping the empty-value paragraph, softening "WINS OUTRIGHT") each kill it **alone**, which is what shows it is not passing on one lucky substring. **Recorded because the honest framing is the deliverable**: there is no better answer available, and pretending the pin is stronger than it is would be the actual defect. 🔴 **Also worth keeping: the same executor disclosed that this round was NOT test-first** — it edited the document first and wrote the pin second, so the pin never went red on a first run — and stated precisely what that costs: *"red-first would also have caught a pin that asserts nothing at all, whereas the reversion mutation only proves it responds to that one sentence."* **That distinction is exactly right, the two extra mutations are the correct substitute, and most executors would have let the mutation table imply a red-first run.** |
| **R242** | 🔴 **WITHDRAWING A FINDING IS AN ACTION, AND IT NEEDS THE SAME RE-MEASUREMENT THAT MAKING IT DID — the failure direction is invisible, because nobody re-checks a withdrawn finding.** X1's executor found that `README-stack.md` documented only the scriptless mock-mode boot line. **That finding was correct.** The orchestrator, relying on a verifier that had read the *already-corrected* file, told it the README was fine. It conceded — **and did not re-run the grep it had run at the start of the ticket.** Then it went further and reported *why* it had been wrong: *"I only ever read line 128's block and asserted the README was incomplete without checking further down the file."* 🔴 **That sentence was a fabrication, and the executor said so itself, unprompted, on re-measuring**: *"I invented a plausible mistake for myself in order to reconcile a contradiction between my own evidence and an authority's claim."* **Two failures, and the second is worse than the first.** The concession cost a true finding — recoverable. The invented explanation **manufactured a false entry in the record about how the error occurred** — and a wrong account of a mechanism is exactly what this log exists to prevent. **THE RULE, in the executor's words: re-run the command before you agree.** It costs one grep; the exchange cost four messages. **And note where the pressure came from**: not from evidence, but from an *authority* contradicting evidence the agent itself held. It resolved the contradiction by doubting its own measurement, which is the wrong default — **a correction from a reviewer is a measurement like any other and gets checked like any other.** Generalises the moving-target rule (**R204**, re-assert a commit at the end of a measurement) in two directions the log did not have: it applies to **documents** identically, and it applies to **conceding**. |
| **R243** | **A CONTROLLED COMPARISON FOR R240, FOUND BY A CLAIM SWEEP: two comments about the same failure, written in the same era, under the same log entry — the one that named a MECHANISM went stale and misled; the one that named an OUTCOME survived a root-cause change untouched.** W33's sweep for the false *"time out"* claim turned up `api/src/mcp/server.ts:83-90`, a docstring written under **R43** saying that reading `WOLF_MCP_URL` directly would mint URLs that *"fail inside a container with no obvious cause"*. **It is still true after W33 changed the root cause**, because it never guessed at *why*. One file away, `api/src/index.ts:52` — same author-era, same subject — said the calls *"time out with no obvious cause"*, and was **wrong on the mechanism and sitting directly above the log line an operator is told to check.** 🔴 **This is evidence for R240's corollary rather than an argument for it**: same failure, same period, two comments, and the difference in survival tracks exactly the difference between naming an outcome and guessing a mechanism. **And the asymmetry in cost is the point**: the outcome-naming comment lost nothing by being less specific; the mechanism-guessing one actively misdirected a debugger who was already stuck. Also worth keeping from the same sweep: **the executor enumerated every other match rather than pattern-matching them away** — real `AbortSignal.timeout` clients, Bob's 30-minute idle timeout, W32's unbounded page walk, the `unavailable` docstring, and two uses of "hang" meaning "hang a flag on" — **so the one true positive is reported against a list of read-and-cleared negatives, not against silence.** |
| **R244** | 🔴 **R238's OWN FIX CONTAINED R238's OWN DEFECT, IN THE OPPOSITE DIRECTION — and it took a MEASUREMENT to see it, twice.** R238 caught the plan claiming a wrong gateway *"yields a timeout"* when X1's recorded evidence was `curl exit 7`. The correction — mine, and the code comment written to it — said the address **"fails FAST … immediately"**. 🔴 **That is not supported either, and W33's verifier proved it by running three probes rather than reasoning:** a **connection refused** returns **exit 7 in 4ms**; curl's **own** timeout is **exit 28**; and a **black hole** returns **exit 7 after minutes**. **Exit 7 means "could not connect" and says nothing about latency** — a kernel connect timeout after unanswered SYNs surfaces as a connect failure, indistinguishable from a refusal by exit code alone. **And the slow shape is the MORE likely one here**: `172.26.0.1` is a real gateway on a network the container is not on — *routable but dead*, so it black-holes rather than refusing. X1 recorded the code and not the elapsed time, so **we do not know which it hit, and neither claim was ever available.** 🔴 **The operational cost of the over-correction was worse than the original error**: *"do not read a slow request as this bug"* tells a debugger who hits the black-hole shape to **rule out the correct diagnosis**. The old text made them wait for a timeout that might not come; the new text made them disbelieve one that might. **THE RULE, and it is the general form of R238 rather than a second instance: state the observation, and mark the inference AS inference.** An exit code is an observation; "fast", "immediately", "times out" are all inferences from it, and **the fix for a bad inference is not a better inference — it is to stop at what was measured and say so.** The actionable sentence in both comments — *compare this line's `mcpUrl` against DinD's own `docker0` address* — **is true regardless of timing and needed no timing claim to stand**, which is the tell that the claim was never load-bearing in the first place. |
| **R245** | **R240's COROLLARY, SHARPENED BY ITS OWN EVIDENCE: the surviving comment survived because it STOPPED AT WHAT WAS KNOWN, not because it was vague.** R243 recorded a controlled comparison — two comments about one failure, same era, and the outcome-naming one survived a root-cause change untouched. W33's verifier read the survivor rather than taking the account, and issued the qualification that makes the rule safe to use: `api/src/mcp/server.ts:84-89` says minting URLs directly *"fail inside a container with no obvious cause"* — **that is an OBSERVED OUTCOME, and it stopped there.** 🔴 **Read as "prefer vaguer comments", R240 does harm**: the next author writes something unfalsifiable and calls it discipline. **The transferable rule is narrower and better — STATE THE OBSERVATION, AND MARK THE INFERENCE AS INFERENCE.** `server.ts` observed an outcome and stopped; `index.ts` and `config.ts` observed an exit code and then guessed at latency, **and the guess is what broke, twice, in both directions** (R238, R244). **The discipline is the virtue; the vagueness is a side effect.** One more thing the verifier noted and it is the strongest part of the evidence: `server.ts`'s claim survived a root-cause change that **invalidated the surrounding mechanism entirely** — a stronger test than "it was never wrong", because it stayed load-bearing across the exact event that broke its neighbours. |
| **R246** | 🔴 **"AN IDLE NOTIFICATION IS NOT FINISHED" HAS A TWIN: A COMPLETED REPORT IS NOT FINISHED EITHER, IF YOU HAVE SINCE SENT NEW WORK.** W33's executor sent a complete round-4 report. The orchestrator then **asked it for a round 5**, and later pruned its worktree — **on the strength of the report, not on the state after the work it had itself just requested.** The tree vanished **mid-sweep**: the executor had committed round 5, and was rebuilding pristine for its mutation run when `cp -al` failed and then the directory itself was gone. **Nothing was lost, and only because it had committed first (R201)** — the four minutes between its commit and its sweep were, once again, the window. **This is the third distinct shape of one root cause today** — R201 (a `git status` from before a commit), R205 (an idle notice read as "finished"), R214 (a sha from an accurate report) — **and the first where the staleness was created by the orchestrator's OWN next instruction.** The reading was true when taken; sending new work is what invalidated it, and the sender is the one party guaranteed to know that. 🔴 **THE RULE: after you send an agent more work, every prior signal from that agent is stale by construction — including its own completed report. Re-read the worktree, not the transcript.** Two things the executor did that are the model response. **(1) It refused to present the numbers it had.** It had a green `yarn test`/`yarn typecheck` from *before* the commit and would not paste them as a clean-tree result, because that is exactly the dirty-tree-then-commit shape R201 exists to eliminate: *"close to zero is my estimate, not a measurement, and this ticket has now been a five-round lesson in the difference."* **(2) It substituted the strongest available proof and labelled its limit** — stripping every comment and blank line from both versions and hashing to **identical**, then saying plainly that this proves the executable content unchanged and **does not prove the file still parses**, since a comment edit can break a build. The orchestrator then took the measurement on merged `main` (37 files / 1834 tests, typecheck 0) and the estimate became a number. |
| **R247** | **A FOURTH ROUTE INTO THE FALSE-GREEN FAMILY, EARNED ON ONE TICKET: THE TREE VANISHED MID-RUN.** When W33's worktree was pruned underneath its harness, the run printed `identity DID_NOT_RUN exit=1 (no Test Files summary line)` — **not a green, and not a kill.** That joins **R196** (collection failed while the *file count matched* the baseline), **R209** (the copy could not compile because a subtree archive lost a root `tsconfig`), and **R218** (the mutation never landed, leaving the tree byte-identical). 🔴 **Four distinct causes, one shape: the tool produced output, and the output was not evidence that the work happened.** All four were caught by the same discipline rather than by four separate checks — **decide on exit code, classify "did not run" as its own outcome, and assert the baseline counts on every single run** — which is the argument for the five rules being a *set* rather than a menu. **And note where they were earned**: not from a paper about testing, but from one ticket's worth of things going wrong in ways nobody predicted. The family is still open; there is no reason to think four is the number. **The generalisation that has survived all four: ask what the run would look like if it had NOT happened, and assert against that — never merely against what success looks like.** |
| **R248** | 🔴 **THE STALE READING REACHED THE BRIEF ITSELF — I HANDED AN AGENT A BASE SHA THAT WAS THREE COMMITS BEHIND ITS OWN BRANCH, AND IT NEARLY REDID WORK ALREADY DONE.** X1's re-brief named `1932301` as the tip. The branch was at `01eb65d`; `0dda26c`, `cc31581` and `01eb65d` already existed, and **`cc31581` and `01eb65d` had already fixed BOTH jobs the brief assigned.** The agent noticed, verified rather than rebuilt, and told me — but the cost was only avoided because it checked; a compliant agent would have re-implemented two fixes and produced a plausible green over duplicated work. 🔴 **This is the stale-reading family (R201/R205/R214/R214b/R246) arriving at a new place: not a merge, not a prune, but the BRIEF — the one document whose whole purpose is to be the fresh agent's only source of truth.** And the mechanism is specific to how this orchestrator works: **the sha was carried in my head across a context compaction, and a remembered sha does not age — the branch does.** R214's rule was "check the branch, not a remembered sha" before *merging*; **the same rule binds before *briefing*, and briefing is the earlier and cheaper place to apply it.** THE RULE: **a brief states a base sha only after `git rev-parse` has produced it in the same minute** — or, better, states the BRANCH NAME and tells the agent to read the tip itself, because a branch name cannot go stale. **The tell that this class is systemic rather than careless: five of six instances were committed by the party who had every reason to know better and every fact needed to avoid it — what was missing each time was not knowledge but a RE-READ.** |
| **R249** | 🔴 **A GUARD CAN FIRE CORRECTLY AND STILL BE DESTROYED BY ITS CALLING CONTEXT — the truncation alarm works, and a retry loop turns it into a parenthetical inside a message blaming a different subsystem.** X1's verifier proved `assertNotTruncated` is not dead code by lowering `LIST_LIMIT` to 1 and watching it fire against a real listing. But the call site at `hypothesis-lifecycle.spec.ts:138` sits inside `waitFor` (`e2e/helpers/x1.ts:288-313`), whose `catch` stashes the message in `last` and **retries to the deadline unless the throw is an `UnretryableError`**. So the run burned the **full 300s** and surfaced the truth as a sub-clause of *"timed out after 300s waiting for a completed tick session"* — **five minutes late, and pointing at the tick pipeline rather than at the listing cap.** 🔴 **The message survived; the DIAGNOSTIC did not.** This is a new shape for the false-green family (R196/R209/R218/R247), and the inverse of all four: those were runs that did not happen reporting as if they had; **this is a check that DID happen, correctly, reporting as something else.** The rule those four share — *decide on exit code* — does not help here, because the exit code is right. **THE RULE: a guard that classifies a condition as PERMANENT must be thrown as unretryable at the point it is raised, or the first retry loop it passes through re-classifies it as transient.** A retry is a claim that the condition may resolve on its own; a truncated listing never will. The mechanism already exists in that same file and is already used elsewhere — **which is the tell: the author knew about the distinction and did not carry it to the new guard**, the ordinary way a defence is built once and then not extended. Verifier's own limit, stated unprompted and worth keeping: because `describe.configure({mode:"serial"})` aborts the file, the teardown-path call at `:367` never ran in the mutation — **it proved the guard on the sessions path and READ that `allSchedules` calls the identical helper, and said which of the two it had seen with its own eyes.** |
| **R250** | **AN INFRASTRUCTURE READING IS AN OBSERVATION AND MY ALARM WAS AN INFERENCE — R244/R245's rule holds outside the code, and the thing that settled it was ARITHMETIC, not a grep.** `docker ps` showed `agent-bob-agentd-1` **UP 4 MINUTES** while every sibling read **UP 12**. I flagged it to X1's verifier as a possible crash mid-run, and said that if agentd had died during a "green" run then that green was suspect. **The observation was real and the inference was wrong.** `e2e/run.sh:517-518` restarts agentd **on every invocation by design** — the mock script table is bind-mounted, so the *file* is live but the *process* is not, and the rig then waits up to 60s for the boot line. 🔴 **What makes this worth an entry is HOW it was closed.** `RestartCount=0` and zero panics are both consistent with a crash that compose's restart policy never handled, so neither is decisive alone. The verifier **counted boot lines**: the log held exactly **4** `model proxy` lines when I asked (1 initial `up -d --build` + 3 restarts = 3 prior invocations), and exactly **8** after its own 4 runs. **One boot line per run, no remainder.** That is a *positive* account of every restart, where a panic grep is only an absence — and an absence cannot distinguish "nothing went wrong" from "the evidence was not where I looked." **THE RULE: to clear a suspicious count, reconstruct the count — do not merely fail to find its alarming explanation.** The same discipline as R234's positive-plus-two-negatives, applied to infrastructure rather than to a boot string. |
| **R251** | 🔴 **THE 'NEVER LOG A CREDENTIAL' RULE IS WRITTEN ABOUT THE TRANSCRIPT, AND THE CREDENTIAL WENT TO DISK — where an ordinary `tail` of a shared directory publishes it into the transcript anyway, by an author who never handled it.** Hunting for a verifier's run output, I tailed the newest files in the shared session scratchpad. One was a curl cookie jar an agent had written while seeding the board, and a **live `wolf_session` cookie printed straight into my context** — not by the agent that created it, and not by any rule-breaking command. **`tail` is not a credential operation, which is exactly why it was safe to run and exactly why it was not.** 🔴 **The rule as written names the ACT (print, log, echo, commit) and the shared scratchpad defeats it by decoupling the act from the exposure: the writer never printed anything, and the reader never handled a secret.** Two things generalise. (1) **A shared scratchpad is a publication channel with a delay.** Anything an agent leaves there is readable by every later agent and by every routine listing, so a credential written there is disclosed the moment somebody looks — which is what a scratchpad is *for*. (2) **The product was NOT at fault, and checking that mattered**: `grep -rn 'cookie-jar\|--cookie' e2e/` returns nothing outside `node_modules`, and the rig's own secrets go to `e2e/.x1-secrets.env` under `umask 077` + explicit `chmod 600`, gitignored, verified `-rw-------` on disk — **the rig's discipline was better than the agents'.** Withdrawn as an X1 defect and kept as an **operating rule: a credential an agent needs on disk goes to a mode-0600 file outside the shared scratchpad and is shredded at the end of the task** — which is what was done with this one. 🔴 **AMENDED after the verifier measured what I had only inferred, and the real mechanism is worse than the one I wrote.** (a) **WHY the agents improvised: `e2e/` offers no shell-side sign-in at all.** The rig authenticates only through Playwright's in-memory request context (`e2e/helpers/x1.ts:325`, `page.request.post(… /api/auth/dev-login)`) — no `storageState`, no curl login, nothing a shell can call. **Every agent that needs to drive Wolf from a shell therefore invents its own auth, and invents it at the default 0644.** The rig did not leak; **the rig's missing door is what made every agent cut its own.** (b) **The exposure window is not bounded by the file, and deleting the jar does not contain it.** Verified in source: `SESSION_MAX_AGE_MS = 12 * 60 * 60 * 1000` (`api/src/auth/session.ts:33`); **there is no server-side session store and no revocation anywhere** (`grep -rn 'revoke\|sessionStore' api/src/auth/ api/src/routes/auth.ts` → no matches); and `POST /api/auth/logout` (`api/src/routes/auth.ts:215`) calls `clearSessionCookie` and nothing else — **it clears the BROWSER's copy and has no effect on a captured one.** Worse, `WOLF_SESSION_SECRET` is generated **once** into `e2e/.x1-secrets.env` and reused by every later run (`e2e/run.sh:216`, `if [ ! -s "${SECRET_FILE}" ]`); only `--rotate-secrets` replaces it. **So a captured cookie survives `docker compose down`, a stack restart, and the run's own cleanup — because the secret that signs it comes back off disk unchanged.** 🔴 **THE SHAPE: I logged the leak and reasoned about the FILE; the file was never the amplifier. A credential whose validity is a pure function of a long-lived secret cannot be contained by deleting copies of it** — the only containment is rotating the secret, and nothing in the run lifecycle does. **This is R244's error committed in the safe direction**: my entry's conclusion (a real leak, agents at fault, rig clean) was right, and the mechanism I attached to it was a guess that a measurement replaced. Two fixes follow, both deferred as their own ticket rather than smuggled into a verification ticket: a documented `run.sh --login <jarpath>` doing `umask 077` + `chmod 600` + a trap-based `rm`, which removes the incentive to improvise; and either a shorter `SESSION_MAX_AGE_MS` for the dev login or rotating `WOLF_SESSION_SECRET` per run, **so a leaked cookie dies with the run that minted it.** |
| **R252** | **THE VERIFIER FOUND THE LEAK-RECOVERY PATH WORKING UNDER CONDITIONS NOBODY HAD EXERCISED, BY READING WHAT ITS OWN FAILING RUN DID — evidence that arrived as a side effect of a mutation, not from a test written for it.** The `LIST_LIMIT=1` mutation run failed at 6.2 minutes and, while failing, **leaked 7 sessions** (the project went 5 → 12). Cleanup then took it back to **exactly 5**, deleting only the ids its own manifest recorded and leaving all five verifier seeds alone. 🔴 **That is the manifest-scoped cleanup working on a NON-EMPTY project, after a FAILING run — the precise pair of conditions it exists for and the precise pair no test covers**, since every passing run self-empties the board (see X1's notes) and no spec deliberately fails. **The observation is only available to somebody who reads the cleanup footer of a run they expected to fail**, and the shape worth carrying is that **a deliberately-broken run is a free experiment in the failure path** — the mutation was aimed at one guard and incidentally exercised the recovery machinery under load. Two further items from the same report, both minor and both recorded so they are not rediscovered: `--reclaim-orphans` was verified **by reading, not by running** (explicit-only at `run.sh:421`, schedules deleted at `:443` before sessions at `:457`, `--help` guard at `:382` carrying an explicit blast-radius warning) — stated as a reading because it was one; and the chosen `a[href="/hypotheses/<id>"]` locator works while a per-row `board-row-<id>` testid was already available and is more direct — **cosmetic, not a defect, and labelled as such rather than filed as a finding.** |
| **R253** | 🔴 **THE ORCHESTRATOR CAN MAKE A FILES LINE STALE AT DISPATCH TIME, AND THE STANDING CHECK CANNOT SEE IT — because the check runs BEFORE the brief, and the brief is what breaks it.** W26's Files line reads *"create `e2e/features/report-layer.spec.ts`"* and its Validation reads `./e2e/run.sh report-layer`. **I then briefed the implementer with two more files** — `e2e/helpers/x1.ts` (R249's `UnretryableError` fix, deliberately assigned here rather than reopening a closed ticket) and `e2e/mock/build-script.py` (the hostile-slot and undeclared-slot rules the criteria require). Both plan lines were accurate when written and are now wrong, **and the Validation line is the worse of the two: `./e2e/run.sh report-layer` does not exercise the helper change at all**, so the ticket's own gate is blind to a file the ticket now owns — **R239's hole, re-cut by hand.** 🔴 **What generalises is the ORDERING.** The pre-wave checks (R194/R206/R215/R237/R239) all run against the plan *before* a wave is dispatched, and every instance they have caught was a defect the plan already contained. **This one did not exist until I sent the brief** — the same structural blind spot as R246 (a completed report goes stale because you sent more work), arriving one layer up: **the party who invalidates the document is the party who wrote the instruction, and is therefore the only party who can know.** THE RULE: **assigning a file that a ticket's Files line does not name is an edit to the ticket, and must be written back to the plan at dispatch — not at merge.** Recorded here at dispatch rather than fixed silently at merge, because the point of the entry is that the gap was open for the duration of the work. **And the tell that it is a real class rather than one slip: the correction had to be sent to the implementer as a WARNING NOT TO TRUST ITS OWN TICKET, which is the exact failure mode the plan exists to prevent.** |
| **R254** | 🔴 **TWO RULES THIS PROJECT ENFORCES EVERYWHERE ARE IN DIRECT CONFLICT DURING A MUTATION RUN, AND I ISSUED ONE WITHOUT CHECKING WHETHER THE OTHER WAS MID-FLIGHT — obeyed literally, it would have committed a BYPASSED SANITISER to the branch.** Reading W26's worktree I found `api/src/report/sanitise.ts` modified — a production file no brief had assigned — and sent a checkpoint asking what it was, **while separately instructing "commit before you validate, then paste the sha above the output" (R201).** I then read the diff myself rather than waiting for the answer (R242), and it was `const clean = html;  // MUTATION: sanitiser bypassed` — **not scope creep at all, but the exact discipline I had demanded**: break the defence, watch the sanitiser leg go red, prove the test is not decoration. 🔴 **R201 exists because a validation run against a dirty tree is a claim about a tree that no longer exists. The mutation discipline exists because a test that passes against unsanitised input is decoration. Both are right, and between the mutation and the restore they demand opposite things** — one says the tree must be committed, the other requires it to be deliberately, temporarily wrong. **THE ORDERING I FIRST WROTE — mutate → observe red → restore → verify clean → commit → validate — IS SAFE AND TOO COARSE, AND W26's IMPLEMENTER REPLACED IT WITH A BETTER ONE ON EVIDENCE.** 🔴 **It serialises the two activities, which means a SEVEN-mutation campaign cannot commit at all until the last revert — and that unbroken uncommitted window is precisely the exposure R201 exists to close.** My rule shrinks the window for a single mutation and *widens* it for a campaign. **The narrower rule that satisfies both constraints: NEVER LET A MUTATION REACH THE INDEX.** Stage by explicit path, never `git add -A`, never `commit -a`; the proof afterwards is `git diff --name-only <base> HEAD`, which must return nothing outside the ticket's own directory. **Then commit timing and mutation timing stop competing entirely** — the tree may be deliberately wrong while the index and the history never are. **Adopted; my ordering is superseded.** And the implementer had already done exactly this, unprompted, while my instruction was in flight: it staged four `e2e/` paths by name **rather than restoring first, because restoring mid-iteration would have handed the running rig an unmutated sanitiser halfway through the run measuring that mutation** — destroying the evidence and producing a green that means nothing. **A second conflict I had not seen, inside the one I had.** 🔴 **Its own framing is the part worth keeping: *"not luck, but it was one habit away from being luck — if I had reached for `git commit -am` your instruction would have committed a bypassed sanitiser exactly as you feared."* A correct outcome produced by a habit rather than by a rule is a near-miss, and reporting it as one is what turns it into a rule.** R201 as written never covered this because every prior instance was a plain dirty tree, not an intentional one. **What generalises past this pair: a dirty tree has two causes that look identical to `git status` and mean opposite things — unfinished work, which must be committed before it is measured, and a live mutation, which must never be.** `git status` cannot tell them apart; **only the diff can**, which is why reading it before instructing was the step that saved this. 🔴 **And the class: a correct rule applied at the wrong moment does damage neither rule predicts.** This log has many entries about rules that were absent or stale; **this is the first about two that were present, current, and mutually exclusive** — the failure was in the orchestrator's sequencing, not in either rule, and no amount of sharpening either one would have caught it. |
| **R255** | 🔴 **THE NO-BILLABLE-CALL GUARD FAILED OPEN, AND IT WAS FAILING OPEN ON THIS MACHINE THE WHOLE TIME — a defence that was inert for weeks while every run it protected reported green.** W26's verifier could not start `./e2e/run.sh` at all: *"agentd printed no model-proxy line within 60s — refusing to run anything that could be billable"*, when agentd **had** printed it. The cause is five lines of the form `printf '%s\n' "${logs}" | grep -qF 'PAT'` under `set -euo pipefail`: **`grep -q` exits at the FIRST match without draining the pipe, `printf` takes SIGPIPE, the pipeline reports 141, and `pipefail` propagates it — so a MATCH READS AS A NO-MATCH.** Reproduced independently before fixing (R242), 202,018-char log, match on line 1: **pipe form status=141, guard skipped; here-string form status=0, guard fires.** 🔴 **The lethal half is line 549, `if printf | grep -qF 'real model proxy →'; then fail` — the guard that REFUSES TO RUN against a real, billable model. When the dangerous line is present and the log is long, the guard does not fire and the run proceeds silently.** The defence and its negation share one broken mechanism: **the same bug that made the mock-mode assertion too strict made the billable check too permissive**, and only the strict half was ever noticed, because the permissive half's failure mode is silence. **It is SIZE-TRIGGERED at the 64KiB pipe buffer, which is why it was invisible**: a fresh stack has a short log, every green in this project was taken on one, and it arrives with age rather than with a change — **no commit introduced it on the day it started biting.** 🔴 **AND THE HONEST MEASUREMENT, which is worse than "we got away with it": this stack's agentd log is 165,959 bytes — 2.5x past the trigger — so the guard WAS failing open here, now, during the runs that verified X1 and W26.** It let nothing through only because `docker logs` records **24 mock boots and ZERO `real model proxy →` or `subscription mode →` boots**: no real key was ever configured. **The outcome was safe for a reason entirely unrelated to the defence.** That is the R180/R184 shape at its purest — a guard that had become decoration — but arrived at by *environmental drift* rather than by a permissive fixture, which is a route this log did not have. **THE RULE: a guard whose failure mode is silence must have a control that fails LOUDLY when the guard's own mechanism is broken.** Six sites converted to here-strings, plus an opening control that fabricates a >64KiB string and refuses the run if the match mechanism cannot find a sentinel in it — **and the control was proved able to fail by swapping the here-string back to the pipe form (exit 1), because an opening control that cannot go red is the very thing it exists to catch.** |
| **R256** | **THE LAST TICKET'S LAST CHANGE WAS THE ONLY ONE NOBODY VALIDATED, AND IT WAS THE ONE MOST WORTH VALIDATING — a 182-line rewrite of the assertion layer, stopped mid-proof.** W26's implementer was asked to close three assertion gaps and **did not write three patches**: it replaced the inline assertions with **one `expectReportState()` every leg calls**, in which `tamper` is an **EXACT set**, so `[]` is a positive claim that a healthy hypothesis carries no anomaly — *"there is only one place to forget it, and every caller pays."* 🔴 **That is the correct structural answer to a divergence defect and it is the first time this project has been given one**: seven prior tickets closed the instances and left the shape intact. It was then stopped mid-mutation, leaving a live `add(forgedRowTamper(row))` in a worktree with no owner — **reverted by the orchestrator, and the near-miss is that nothing but the R254 index rule kept it out of the branch.** The orchestrator re-ran the proof rather than merging on the strength of a well-argued diff: clean run **10 passed / exit 0**; mutation applied behind a landing gate asserting the differing set EQUALS exactly one intended file; **exit 1, leg 3 red, `Expected Array [] / Received Array ["forged_row"]`** — the exact-set assertion catching the exact defect it was written for, with the offending entries and their provenance dumped in the message. Reverted, tree clean, index never touched. **THE RULE, and it is the one this whole log argues for in miniature: the most persuasive change is the one most likely to be merged unproven.** A patch nobody believes gets tested; **a rewrite whose reasoning is visibly right invites the reader to substitute the reasoning for the evidence** — and the better the argument, the stronger that pull. **The correct response to "this is obviously right" is to run it**, which cost one mutation and four minutes. |
| **R257** | 🔴 **THE TRUST MODEL WAS ENFORCED ON ONE OF ITS TWO INPUTS. Status transitions are driven by memories AND by datasets; memories were provenance-checked at every read, and datasets were not checked at all.** Found by an adversarial review on a different model, and it is the shape this log has been circling all along: **the defence was complete within the channel everyone was looking at.** `isTrusted`/`pickSurvivingRow` make a container-written memory unable to become state — attacked from several directions, it holds. But the poller reads `<id>-<slug>` (`poller.ts:289`), evaluates the spec's conditions against those numbers, and **Wolf itself appends the `challenged` transition** (`poller.ts:476`). Bob's `dataset_put` stamps provenance from the token (`mcp_datasets.go:631-632`) and imposes **no name ownership**; `dataset_get` is scoped to the **project**, not the session. Measured from inside a session container: a write to `hyp-victim-fable-probe-rate`, a name that session did not own, returned `version: 2` and read back identically. 🔴 **So the attack is: hostile container writes tripping values into a PEER's dataset name → the peer's poller reads them → Wolf writes a genuine `challenged`. NO TAMPER FLAG FIRES, because the state row really was Wolf's.** The forgery happened one step upstream, in the channel nobody guarded. **THE RULE: enumerate every input that can move a trusted output, and check that the defence covers the SET — a guarantee proved on one input is not a property of the system.** The tell that this was missable: `store.ts` is dense with trust reasoning and **all of it is about memories**, so a reader auditing the trust model finds thorough, correct code and stops. |
| **R258** | **THE REVIEW'S FINDING WAS RIGHT AND BOTH ITS PREMISE AND ITS PROPOSED FIX WERE WRONG — and taking either on trust would have produced a bigger change than needed and a broken product.** The report said provenance *"never crosses to the reader, so Wolf couldn't defend this even if it tried"*, and prescribed a Bob change to surface the field. **Measured: `GET /agent/datasets/{name}` already returns `created_by_worker`/`created_by_session` (`httpapi/datasets.go:121-122,144-145`), and `client.ts:495-496` ALREADY PARSES BOTH into `DatasetMetadata`.** Wolf had the data on exactly the path the poller uses and simply never looked — **so the correct fix touches no engine code at all.** (The premise is true of the *MCP* `datasetHandle`, which carries no provenance; the review generalised from the tool surface to the HTTP surface.) 🔴 **And the prescribed check — compare the SESSION — would have broken every hypothesis within a day.** Ticks run in MANY sessions over a hypothesis's life; the poller sweeps old ones and keeps a handful (`sweepTickSessions`), so a session id is not stable. **The stable binding is the WORKER**, and it is exactly as forgery-resistant: `caller.Worker` is read from the session ROW server-side (`mcpserver.go:105-110`), never supplied by the caller. **THE RULE, and it is R242 pointed at an incoming finding rather than an outgoing one: a finding and its fix are two claims, and the credibility of the first does not transfer to the second.** A correct, well-evidenced bug report is exactly the thing most likely to have its remedy adopted unexamined. |
| **R259** | 🔴 **A FIXTURE FIELD NOBODY READS DRIFTS TO VALUES REALITY CANNOT PRODUCE — in two fakes, differently, and neither could be caught until a real check existed.** Adding the ownership check broke seven tests that had nothing to do with it. **Cause: both dataset fakes invented a writer.** `poller.test.ts` hard-coded ONE worker for every dataset, so `2b3c4d5e-basket` claimed to be written by `researcher-1a2b3c4d` — the wrong hypothesis's researcher, a combination the real system cannot produce. `report.test.ts` was worse: a bare `"researcher"`, **a worker name Wolf never mints** (they are all `researcher-<id>`). Both were correct-looking, both were inert, and **the failures were not the new check misfiring — they were the fixtures being wrong and finally being read.** This is **R180/R184's family reached from the opposite direction**: those were fakes too PERMISSIVE, letting a defence become decoration; these were fakes simply FALSE, harmless only for as long as the field went unexamined. **THE RULE: a fixture field that no assertion and no production path reads is not neutral — it is an unverified claim about the world, and it will be wrong when something finally depends on it.** The fix in both was the same and is the general one: **derive the value from the data rather than hard-coding it** (`researcher-${name.split("-")[0]}`), so the fake cannot express a combination reality forbids. **And the diagnostic worth keeping: when a new check reddens tests in unrelated files, the first hypothesis should be that the FIXTURES are wrong, not that the check is** — here it was true both times. |
| **R260** | 🔴 **I TOLD THE OWNER NO TOOL EMITTED THE QUESTION-CARD MARKER. ONE DID, HAD ALL ALONG, AND SHIPS IN THE SANDBOX AS A BUILTIN.** The claim rested on one grep: `ask_question\|AskUserQuestion\|askQuestion\|request_human_attention` over `sandbox/src`. Every term was a guess at what the tool would be called, and **the actual name — `ask_user`, the same string the reducer's own gate matches on (`agentEventReducer.ts:243`) — was not among them.** `sandbox/src/tools/builtin/ask_user.ts` is 58 lines, is one of four builtins `DefaultToolRegistry` ships, and is granted to every session by default. The renderer, the marker protocol, the SSE event, the card and the tool ALL existed; **the only missing piece was a line in `prompts/interviewer.md` telling the model to use it.** So the reported gap was 100% real as an observation (the interviewer did ask in prose) and 100% wrong as a diagnosis, and the fix I proposed off it — build a second `ask_user` in Wolf's MCP server — was written, tested, mutation-proved, and then **deleted**, because it duplicated a builtin and would have put two identical tools in front of one model. **THE RULE: a grep that returns nothing proves your search terms wrong before it proves the code empty — and when the thing you are looking for has a canonical name in a CONSUMER you already read, search for that string, not for synonyms of the concept.** The reducer's gate literally spelled the tool's name; I read that line, quoted it in the report, and still searched for four other things. |
| **R261** | **THE FEATURE WAS PRESENT, REACHABLE, AND UNUSABLE FOR HALF THE QUESTIONS AN INTERVIEW ASKS — because of `.min(2)`.** `ask_user`'s `options` was a REQUIRED array of at least two entries, so "what price level would prove you wrong?", "how many days?", "which ticker?" — every question whose answer is a number, a date or a ticker — **could not be expressed as a card at all.** The model's only legal move for those was prose, which is the exact behaviour the card exists to replace, so an interviewer told to use the tool would have used it for the easy half and reverted to a wall of text for the rest. `allow_freetext` also defaulted to `false`, so a card's option list was a trap by default. The fix is small (`options` optional, and `allow_freetext` defaults to `options.length === 0`) and one rule in it is load-bearing: **with no options the text box is FORCED on, overriding an explicit `false`**, because no buttons and no text box renders a card the user can neither click nor type into — it reads as "the agent is waiting on me" while offering no way to answer. A rule that can be argued out of is not the rule. **THE RULE: when a capability goes unused, check whether its SCHEMA permits the use you expected before concluding nobody knew about it.** A validation floor is invisible in the description, invisible in the UI, and indistinguishable from the model choosing not to bother. |
| **R262** | 🔴 **THE STOOQ DEFECT WAS NOT "AN EMPTY SERIES". THE CHALLENGE PAGE'S OWN JAVASCRIPT WAS PARSED INTO A DATA ROW AND WOULD HAVE BEEN STORED AS AN OBSERVATION.** Every previous description of this — in `stooq.ts`'s header, in R56, in my own report to the owner — said the HTML was swallowed and produced no rows. **Measured on the recorded page (`__fixtures__/stooq-challenge-page.html`, captured 2026-09-07):** the page's inline `<script>` contains commas, so `parseStooqCsv` skipped line 0 as a header, split line 1 on commas, and took column 0 as a TIMESTAMP and column 4 as a VALUE. `normalise` emitted `timestamp,value\n(async()=>{const c="AAAAAGqe___…",e.encode(c+n))\n` — **one structurally valid data row, `rows: 1`, a working download URL, and a `dataset_put` that would have written it as a real versioned observation.** The difference matters: an empty series is a visible symptom (a flat chart), whereas a garbage row is a *populated* series that any reader would treat as data. **THE RULE: measure the failure before writing the fix's justification.** I had the fixture and the parser in front of me and wrote "returns `[]`" into a file header, a test name and a commit message from the prose that had been handed down; one `tsx -e` invocation corrected all three. A defect description inherited from earlier notes is a claim, not evidence. |
| **R263** | **THE YAHOO CONNECTOR'S TESTS ARE NOT FIXTURE-BACKED, AND SAYING SO IS PART OF THE DELIVERABLE.** The owner's instruction was "tests with real recorded fixtures for both". Yahoo answered HTTP 429 (`Too Many Requests`, 18 bytes, `text/html`) to every request from this IP for the whole ticket — `query1` and `query2`, with and without a browser User-Agent, with a cookie jar, across ~20 minutes of retries — so `yahoo.test.ts` tests OUR logic against the observed response *shape*, and its header says that in the first paragraph rather than in a footnote. What IS recorded: the 429 body, and Stooq's challenge page. **The temptation worth naming: I had the shape from a live call earlier the same session (1,261 `GC=F` bars, adjusted-close present, last close 4476.60) and could have written a plausible `yahoo-chart-gcf.json` from it.** That file would have been indistinguishable from a recording, would have passed review, and would have been a lie in a directory whose README's first line is "Fixtures are recorded real responses, not hand-written." **THE RULE: when the recording cannot be made, ship the honest test plus the exact command to record it later — never the fixture you could have written.** `__fixtures__/README.md` § Yahoo carries the four `curl` lines and the list of what a `yahoo-recorded.test.ts` must then pin; a background recorder was left retrying so the gap closes the moment the throttle lifts. ⚠️ **AMENDED the same day, and the amendment matters more than the entry.** There was no throttle. The 429s were a User-Agent blocklist rejecting the browser string the connector itself was sending — see **R264**, which is the real lesson. This entry's RULE survives intact and the fixtures were recorded within the hour once the actual cause was found; what does not survive is its framing of the obstacle as external and unfixable. **Leaving it uncorrected would make this log evidence for a conclusion the next entry disproves**, which is the failure mode a Discovered Issues Log exists to avoid. |
| **R264** | 🔴 **I SHIPPED A CONNECTOR THAT COULD NOT MAKE A SINGLE SUCCESSFUL CALL, AND DIAGNOSED THE EVIDENCE OF THAT AS SOMEONE ELSE'S RATE LIMIT.** `yahoo.ts` defaulted to a full Chrome `User-Agent` because the endpoint answered 429 without one, and I wrote "it requires a browser-style User-Agent or it returns HTTP 429" into the file header, the tool docs, a test name and a commit message. **Measured properly (2026-09-07, same URL, five trials, both orderings): the full Chrome string gets 429 FIVE times out of five, and `agent-wolf/0.1 (+url)` gets 200 five times out of five.** `Mozilla/5.0` alone: 200. `curl/8.5.0`: 429. `python-requests/2.32.3`: 429. It is a **User-Agent BLOCKLIST wearing a 429**, it is deterministic, and my default was on it. Every `series_fetch` against yahoo would have failed forever. **The error I made was not the wrong guess — it was never running the control.** I had one hypothesis ("no UA → 429, so it wants a browser UA"), the first thing I tried confirmed it, and for the next 40 minutes I read every subsequent 429 as a per-IP throttle: I wrote a background retry loop, told the owner gold had no working source, tried cookie priming, tried `query1` vs `query2`, and re-probed on a timer — all of it inside the assumption. The disconfirming test was **one curl with a different UA**, cost nothing, and I only ran it because R263 had me poking at the "throttle" again for the fixtures. 🔴 **AND THE FALSE DIAGNOSIS BECAME A DELIVERABLE**: "Yahoo is rate-limiting this IP, gold has no live price source" went into a status report, a commit message and a Discovered Issues Log entry as fact. **THE RULE: when an external system rejects you, vary YOUR side of the request before concluding anything about theirs — and treat a first-try confirmation as the weakest evidence there is, not the strongest.** A 429 is a *claim* by the server about why it said no, and a blocklist has every reason to lie about it. The corollary that actually stings: R262's rule was "measure the failure before writing the fix's justification", written the same afternoon, three log entries earlier — and I then wrote a justification from an unmeasured premise anyway. |
| **R264a** | **The fix is also the honest thing, and that is not a coincidence.** The replacement default identifies the software truthfully (`agent-wolf/0.1 (+repo url)`), which is what the *measurements* favour — but it is independently the only defensible thing to send an unofficial endpoint whose operator never agreed to serve us. Yahoo blocking `curl` and `python-requests` while allowing an honest custom UA reads as a deliberate policy: **generic scrapers out, identified clients in.** Spoofing Chrome takes away their ability to say no to Agent Wolf specifically, and it is the move that makes the next outage undebuggable, because a disguise's failure looks like a rate limit — which is precisely how this entry happened. Written into `yahoo.ts`'s header: if the honest UA is ever blocked, the answer is a different data source, not a better disguise. |
| **R265** | **THE RECORDED FIXTURES IMMEDIATELY FALSIFIED TWO CLAIMS IN THE CODE THEY WERE RECORDED FOR — and a third one I made while reading them.** Within minutes of the fixtures existing: (1) `yahoo.ts`'s header asserted "not every instrument has an adjusted column (futures and crypto do not)" — `GC=F`, `GLD` **and** `BTC-USD` all carry `indicators.adjclose`, so the fallback branch is defensive, not the routine path for two of three asset classes; (2) `searchUnit`'s doc said Yahoo's search "does not reliably carry a currency", implying sometimes — the key is **absent from all six hits**, so the string branch is dead code kept for a hypothetical future. Then (3) I wrote a test asserting the `currency` key was *present and null*, because I had inspected the fixture with Python's `dict.get()`, **which returns `None` for a missing key exactly as it does for a null value** — two different facts collapsed into one output, and the test failed on `Object.hasOwn` and corrected me. **THE RULE: a fixture is not documentation until you have asserted against it, and the tool you inspect it with can erase the distinction you care about.** `.get()`/`?.`/`??` are all lossy about absence-vs-null; when the difference matters, ask the question that can only have one answer (`hasOwn`, `in`, `Object.keys`). The wider point is R263's argument arriving as evidence: the shape tests were 38 green tests over a shape that was wrong in two places, and no amount of mutation testing on OUR logic could have found it. |
| **R266** | 🔴 **THE PROVIDER LIST WAS WRITTEN OUT THREE TIMES; I FOUND TWO, WIRED THEM, AND 1,944 TESTS PASSED OVER A FEATURE THAT COULD NOT COMPLETE A SINGLE REAL FETCH.** Adding `yahoo` meant extending `SERIES_SOURCES` (`mcp/tools.ts`) and `METRIC_SOURCES` (`hypothesis/spec.ts`) — both of which I found, because the owner's instruction named them. The third copy was a bare `z.enum(["fred", "stooq"])` inside the download token's payload schema (`mcp/seriesdownload.ts:63`), and no test in that file ever enumerated sources: its happy paths used `fred` and `stooq` and its four rejection cases were about signatures and expiry. **So `series_fetch` minted a valid `yahoo` download URL, reported `rows: 25` of real gold, and the download route then refused its own token with `{"kind":"forbidden","message":"invalid or expired token"}`.** That message is the byte-identical 403 the route uses for every rejection — deliberately, so it is not an existence oracle — which meant the symptom pointed at HMAC or clock skew, not at a stale enum. It cost the whole diagnosis. **Found only by pulling a real gold CSV end to end through the deployed stack**, which I had already told the owner was verified on the strength of `series_fetch` returning 200. **THE RULE: a green suite over a feature nobody has driven end to end is a statement about the tests, not the feature — and when you extend an enum, the thing to search for is the VALUES, not the constant's name.** `grep -rn '"stooq"'` would have found all three copies in one second; I grepped for `METRIC_SOURCES` and `SERIES_SOURCES`, which found the two that had names. The structural fix is `marketdata/sources.ts` holding the list once, with `METRIC_SOURCES` now literally `[...SERIES_SOURCES, "derived"]` (what it always meant), and a `it.each([...SERIES_SOURCES])` over the download route so a fourth provider extends the coverage automatically instead of needing to be remembered. |
| **R267** | **THE MODEL EVERY SESSION RUNS ON WAS ONE UNPINNED STRING LITERAL, A YEAR STALE, WITH NO UI AND NO TEST.** The owner asked what model the chat uses, because there is no selector. Answer: `claude-opus-4-5`, from `sandbox/src/config.ts`'s `z.string().default(...)`, and **that is the only default in the stack** — `go/runner.go:2750` writes `DEFAULT_MODEL` into the container env *only* when a session explicitly names a model, and agentd's `SessionEnv` is a **fixed three-key map** (`cmd/agentd/modelproxy.go:142,156`), so a `DEFAULT_MODEL` set on the HOST or in compose **never reaches a session container**. The owner's first instinct — "let's make that happen in the environment" — could not have worked, and the reason is worth writing down: an env var that looks like a knob is not one unless something forwards it. **The wider point is the shape of the thing:** the single decision with the largest effect on both cost and quality of every agent turn was a bare literal in a config schema, unmentioned in any doc, unasserted by any test, and unchanged since the fork — so it had drifted from "our best model" to "a year-old model" without anyone deciding anything. **THE RULE: a default that encodes a judgement needs a test, because a test is the only artefact that makes the next reader treat it as a choice rather than as furniture.** `config.test.ts` now pins it (including a named case for the value it replaced, so a revert reads as a revert), and the comment above it says in one line what changing it costs. Bumped to `claude-opus-5` by owner decision 2026-09-07; verified live end to end — `Model: claude-opus-5` at boot, `query() with model: claude-opus-5` in the harness, `subtype=success` and the requested one-word reply back from the real API. |
| **R268** | 🔴 **THE PROMPT TOLD THE MODEL TO READ SOMETHING IT CANNOT SEE, AND THE RESULT WAS A GOOD INTERVIEW THAT COULD NOT BE TAKEN LIVE.** `prompts/interviewer.md` said the candidate memory's `name` label is "this hypothesis's bare id (the session you are running in is named `hyp-<id>`; use the id without that prefix)". **A session container is given `SESSION_ID` — Bob's 32-hex id — and `SESSION_TOKEN`, and nothing else. The session's NAME never reaches it** (`go/runner.go:2735-2751`; `SessionEnv` is a fixed map). So the instruction was unfollowable, the model did the only thing left and invented a slug from the thesis (`gold-m2`), Wolf looks candidates up by `name=<id>`, found none, and the user's Go Live button never appeared. What he saw instead was the server's honest-but-now-false sentence *"no spec has been proposed yet — the interview has to deposit a hypothesis-spec-candidate memory first"*, while two perfectly good candidates sat in the database under a name nothing reads. **THE RULE: an instruction that refers to state is only as good as the model's access to that state — so when you write "you can see X", go and check that it can.** The tell was available the whole time and I walked past it twice: I had already read `sandbox/src/config.ts`'s env list and printed the container's environment (`SESSION_ID=<set>`, `SESSION_TOKEN=<set>`) an hour earlier while answering a question about the model, and did not connect it. Fixed Wolf-side by putting the id in the interview's FIRST MESSAGE, attributed to Wolf so a thesis claiming a different id reads as user text (§ 6.2.4); a Bob `SESSION_NAME` would be cleaner and touches the engine and every embedder. |
| **R269** | 🔴 **A 27-RULE STRICT SCHEMA WAS SPECIFIED TO THE MODEL AS ONE PARAGRAPH OF PROSE, USING FIELD NAMES THAT DO NOT EXIST.** The same interview deposited a spec with **thirteen** validation errors. The prompt said conditions name "a metric, a statistic, a comparison and a threshold" — the schema's fields are **`stat`** and **`op`**, and V7 rejects unknown keys at every level, so `statistic`/`comparison` were four errors on their own (two missing, two unrecognised). The prose never mentioned `unit`, `id`, `sustained_days` or `meaning` at all. **The model followed the prompt exactly and was punished for it.** The natural experiment sitting right beside it is the argument: the SAME interview's report template validated first time, because `report-authoring.md` gives a full contract, every rejection rule restated, and a worked fixture — while the spec got a paragraph. **Same model, same turn, two deposits, one detailed spec and one prose sketch: the detailed one was correct.** Fixes in three layers, and the third is the one that matters: the real field table; a worked example **with a test that validates it** (an example that does not validate teaches the model to fail and looks fine in review); and `spec_validate`, an MCP tool that runs the same extractor and the same validator the Go Live gate runs, so the model checks its own work before depositing. **THE RULE: for a strict machine-checked artefact, a prompt is a hypothesis about what the model will write and a validator is a fact — give the model the fact.** Cross-field conditionals (`reference` forbidden for `level`, required otherwise) and whole-document rules (weights sum to 1.0; a metric ≥ 0.25 must be named by a condition) are not reliably one-shot from ANY prose, however good. |
| **R270** | **BOTH FAILURES WERE INVISIBLE FROM BOTH ENDS, AND THAT IS THE PROPERTY WORTH NAMING.** The model believed it had finished — it wrote a correct-looking summary to the user listing the metrics, the invalidation rule and the horizon, and told them to go and click Go Live. The user went and looked, and the button was not there. **Neither party had any signal.** The deposit succeeded (memories are append-only and accept anything), the labels were syntactically fine, the spec was well-formed JSON, and nothing in the system's job was to compare what the interview wrote against what the reader would look for. This is the same shape as R255 (a guard failing open), R259 (a fixture field nobody reads) and R262 (a parser that turned a web page into a data row): **the system's components were each behaving correctly and the gap was between them.** The general remedy is the one applied here — move the check to where the writer is, so the writer is told, rather than leaving the reader to infer silence. A validator the model can call is worth more than a validator the server runs, not because it catches more, but because it catches it while someone can still act. |
| **R271** | **THE SIGN-IN PAGE ADVERTISED AN OFFLINE BYPASS TO EVERY VISITOR, AND THE UI TEST THAT PROVED THE AUTH GATE WORKED WAS LEANING ON IT.** The page rendered a Google button, an `or` divider, and an email/password form captioned *"Test-stack sign-in. Available only where the API was started with WOLF_TEST_LOGIN"* — shown unconditionally, because `VITE_GOOGLE_CLIENT_ID` is a build arg and `WOLF_TEST_LOGIN` is a **server**-side setting the browser cannot see. So a stack with the fallback removed still showed the form and still named the variable that turns it on. It was never exploitable (`api/src/routes/auth.ts` mounts the route only when the variable is set, `loadConfig` refuses it under `NODE_ENV=production`, and the allowlist gates both doors regardless) — but a product whose entire claim is that it does not trust things should not put a password box on its own front door, and should not tell a reader the name of the switch. **Hidden by owner decision on 2026-09-08, not deleted:** every region in `web/src/pages/SignIn.tsx` is commented and tagged `DEV-LOGIN-UI`, and the three tests that drove the form are commented alongside it with the same tag, so they come back together or not at all. The capability is untouched — `POST /api/auth/dev-login`, `devLogin()` in `api/client.ts` and their own tests are all still green — and **Playwright never used the form**: `e2e/helpers/x1.ts` and `e2e/layout-probe.mjs` both POST straight to the route, which is why hiding it cost the rig nothing. The second half is the more interesting half: `web/src/App.test.tsx` asserted the auth gate by looking for `dev-submit`, so **two tests about whether an unauthenticated visitor is stopped were anchored to a debug control.** Hiding the control would have reddened them, and the tempting fix — put the form back — would have been the test dictating the product. Repointed to a new `data-testid="sign-in"` on the page root, which is what they always meant. **THE RULE: when a test reaches for a nearby handle instead of the thing it is testing, it quietly becomes a vote against ever changing that handle.** In place of the three removed tests is `google_is_the_only_door`, which asserts no password input and no `<form>` at all, so a differently-named revival is caught too — mutation-checked by uncommenting the JSX region and watching it fail (`expected <div> to be null`). A comment does not stay commented on its own. `./stack wolf up --test-login`'s help text now says it mounts the route alone and that the page shows no form. |

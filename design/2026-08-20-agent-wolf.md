...
- **Acceptance criteria:**
  ...
  - The failure mode is **named in a comment**: a wrong answer here produces a timeout inside a
    container with no error anywhere, which is why it was invisible until something ran.
- **TDD:** yes.
- **Validation:** `cd api && yarn test src/config && yarn typecheck` — **state the test-file count.**

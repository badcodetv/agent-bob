...
> the TEST-FILE COUNT equals the baseline on every run** — the fifth, because a *collection*
...
**40 are done.** W13 and W14 landed 2026-08-24; W30, W17, W19, W21, W25 and **W22** landed 2026-08-26,
so the report chain now runs out to **W23** and **W27**, which W22 unblocks and which are independent of
each other. W29 is ready too — and was held out of wave 14 because it and W22 both modify `api/src/app.ts`.
...
🔴 **Orchestrator: never prune a worktree while its agent may still be live**, and run
`git status --porcelain` inside it before pruning regardless. A prune mid-command returns exit 1
with no output, which reads exactly like a genuine failure — one agent lost a `yarn build` this way
and correctly refused to report a green it had not seen.
...
### W31: The test-fake fidelity audit   [Status: pending | Model: opus]
...
- **Files:** create `design/2026-08-26-agent-wolf-fake-audit.md`. **No other file in either repo may
  be modified** — not a fake, not a test, not a source file. That constraint is the ticket.

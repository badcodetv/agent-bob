...
- [ ] done
- Notes: **Hand-off from W30 and W17 (2026-08-26), read before starting.** (1) 🔴 **`frame-src` and `form-action` are absent from the CSP skeleton entirely**, and an origin that is only ever *navigated to* currently becomes a permitted **script** source — `remoteOrigins` is one list and the skeleton substitutes it into `script-src`/`style-src`/`img-src`/`font-src` alike. A `meta[http-equiv=refresh]` target and an `object > param` value are navigation and plugin hosts, not code. **R146** is why this matters: a navigation is governed by none of the four directives the skeleton names. (2) `ParsedTemplate` now carries **`remoteOrigins`** (deduplicated, sorted, origins only) *and* `scriptSrcs`, and they are **not** the same question — `scriptSrcs` means "remote code and stylesheets a human is approving **as code**", `remoteOrigins` is the superset of everything the document will fetch. A `srcdoc`-hosted `<script src>` is in **both**; a meta-refresh target and an `object > param` value are in `remoteOrigins` **only**. `remoteOrigins` is a strict superset of `scriptSrcs`'s hosts, so W24 can render "everything else" as a set difference. (3) `validateTemplate` (W17) returns `parseTemplate`'s `ParsedTemplate` **whole**, so `remoteOrigins` reaches you untouched — no wiring needed. (4) 🔴 **The report panel's height contract and the `postMessage` prohibition are already implemented** by W14's `ReportFrameHost`; W23's `ReportPanel` is its **child**. Do not re-decide either. (5) 🔴 **A slot declared on `<style>`, `<title>`, `<textarea>` or `<xmp>` is a breakout site** and `validateTemplate` currently **accepts** one — the exploit fails today only because of a DOMPurify regex, not because of anything we wrote (**R150(3)**). Either refuse it here or in W30; do not assume the library keeps covering it.

...
| **R131** | 🔴 **THE SUBAGENT PATTERN DELIVERED IMPLEMENTATIONS BUT NOT REPORTS, AND THE ORCHESTRATOR HAD TO HOLD THE GATE BY HAND.** In wave 8 all three spawned agents — W11's implementer (twice), W11's verifier, and O12's implementer — went idle without producing their final report. The **work was never lost**: each had committed to its branch, and the verifier left its worktree clean at the original HEAD with no stashes. But the reports are the deliverable that carries the per-criterion evidence, the mutation results and the **guess log**, and none of that arrived. The orchestrator therefore verified both tickets directly: re-ran each ticket's Validation, ran the full suites, and ran **nine mutations against W11** and **two against O12**, all of which were caught. Both tickets passed with zero fix rounds, so the *outcome* was sound. Two costs stand. (1) **The guess logs are gone.** Waves 4–7 produced 82, 114, 115 and 39 guesses respectively and they are a primary deliverable of this plan — wave 8 produced none, and "the plan was complete" is not a safe reading of that. (2) **It does not scale**: hand-verification consumes orchestrator context that the pattern exists to protect. Before the next wave, either establish why reports are not arriving, or plan on the orchestrator gating personally and budget the context for it. **Do not read a silent agent as a failed agent** — check the worktree first; in every wave-8 case the code was finished and correct. |

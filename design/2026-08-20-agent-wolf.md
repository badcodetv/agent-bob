...
| **R134** | 🔴 **W28 CANNOT INSTALL THE TARBALL THE WAY THIS PLAN SPECIFIED, AND O12 PROVED IT BEFORE W28 WAS CUT.** O12's implementer tested the `file:` tarball form under **yarn 1** — what `agent-wolf` uses at its workspace root — and found two failures: a **clean** install after any change to the tarball dies with `error Integrity check failed for "@agentkit/chat-ui"` (CI's exact situation), and a **dirty** install silently serves a **stale cached tarball** that neither `yarn install --force` nor `yarn cache clean` refreshes. It also ruled out the directory form `"file:../../web"`: yarn 1 raw-copies the whole directory **including `web/node_modules`**, giving the consumer a second React, and both `files` and `.npmignore` are ignored. In `examples/web` O12 therefore used a Vite alias onto `web/dist/index.js` with **one entry per subpath** (alias keys match exactly, so a bare key misses `/pure`) plus `tsconfig` `paths` onto `web/dist/*.d.ts`. **For W28**, a different repository that cannot alias across repos, the vendored tarball stays right — it is a committed artefact that changes only on a deliberate republish — **but the version MUST be bumped on every republish**, or yarn serves the cache and Wolf silently builds against stale components with nothing failing. Now a W28 criterion. O12 also fixed the package version at **`0.1.0`**, so W28's vendored filename is `agentkit-chat-ui-0.1.0.tgz`. |

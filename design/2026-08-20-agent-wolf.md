...
| **R136** | 🔴 **O12's SUBPATH EXPORTS DID NOT FIX THE BARREL COST, AND FIVE TICKETS ARE ABOUT TO PAY IT FIVE TIMES.** R125 predicted the barrel would make the tier line notional; O12 answered with `.`, `./pure` and `./components`. Measured by W28's verifier: the `web` suite is **1.43s** for the five files that do not touch the package and **37.76s** for the single file that does — **~36 seconds of pure module import**. `dist/components/index.js` is 46 re-exports including `AgentChat` and `ChatHistoryDrawer`, so importing `ArtifactPanel` pulls the whole library, and the `exports` map hard-blocks a deep import (`ERR_PACKAGE_PATH_NOT_EXPORTED`) — **a consumer cannot opt out.** W29, W13, W14, W23 and W24 all import from `./components`. The fix is one line in `agent-orange/web/package.json` — a `"./components/*": "./dist/components/*.js"` wildcard export — plus a version bump and a re-vendor, which is also the first real exercise of R134's republish procedure. **Do it before W13 is cut**, or every UI ticket's test run carries the tax and people stop running the suite. |
